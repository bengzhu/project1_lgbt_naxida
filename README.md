...
当前正式版本：`2.3`（图片任务取消后，只有 sandbox source 已发布且文件仍存在时才显示重试；尚未落盘的取消、clear 和 translated 不误显 Retry）。v2.2 的 Store-owned 图片 transfer/run isolation、v2.1 的 Koharu trusted geometry coverage gate 和 v2.0 的稳定一对一最大基数匹配仍保留。仓库尚无真实 Koharu 四件套和 Speech corpus，因此不声称 OCR、翻译或识别质量提升；真实四件套仍等待 `ci-fast` 运行态证据。日常开发合入 `smalldata_test`，不合并到 `main`。
