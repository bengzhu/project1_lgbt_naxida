...
当前正式版本：`3.40`（图片 OCR 修正 sheet 让可保存性、未保存修改、是否重译和传给既有 Store 的文本共享同一首尾空白规范化值；`trim` 后仍等于当前原文时仍是“确认无误”，可以直接取消或下滑关闭。多行 OCR 输入保留可访问的键盘“完成”动作，取消、打开忽略确认或保存前也会先收起键盘；滚动同一 `Form` 时可交互式收起键盘，便于查看下方操作。v3.40 再在保存并重译期间锁定目标 OCR 输入框，避免异步完成时覆盖保存后新增的文字；该锁定只跟随既有 block-scoped correction state，不改变 Store 输入、保护、只重译目标 block 或关闭后焦点交接。v3.36 的开发控制台 Koharu readiness 摘要仍只读已有 probe report，不触发新探针、不读写 artifact，也不改变普通图片 OCR、翻译、覆盖图或质量基线）。仓库仍没有真实 Koharu 四件套、Speech corpus 或真实竖排图片 corpus，因此不声称 OCR、翻译或识别质量提升。云端 receipt 传播规则（v3.26）仍适用；日常开发合入 `smalldata_test`，不合并到 `main`。
