...
当前正式版本：`3.8`（免费模式在打开系统照片或文件选择器前显示图片翻译 Pro 门槛，不再让用户完成无效选择流程）。v3.7 的图片输入语言拒绝无跨页副作用、v3.6-v2.2 的 Retry、Koharu gate 与图片生命周期能力仍保留。仓库尚无真实 Koharu 四件套、Speech corpus 或真实竖排图片 corpus，因此不声称 OCR、翻译或识别质量提升。日常开发合入 `smalldata_test`，不合并到 `main`。
