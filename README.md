...
v3.106 让普通图片 OCR 与 Developer Console 漫画探针筛选器的 VoiceOver value 统一读出当前筛选类别、显示数量和总数量；图片筛选在有风险块时额外读出已复查／剩余数量，避免用户只听到分类而不知道当前结果范围。该 View-only 改动不新增 Store／持久化、不改变 OCR、翻译、renderer/export、探针报告或 Koharu active gate；候选云端验收完成后再补充 receipt，真实四件套仍缺失，不声称 OCR、翻译、识别或 Koharu 质量提升。
