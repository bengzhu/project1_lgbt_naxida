...
当前正式版本：`3.15`（完整图片预览中的 OCR 覆盖块可直接点选或取消，并打开局部放大；当前“待复查”筛选隐藏被点块时会回到“全部”，保持列表、位置与前后导航一致）。v3.14-v2.2 的定位、局部放大、选择、预览、交互、Retry、Koharu gate 与图片生命周期能力仍保留。仓库尚无真实 Koharu 四件套、Speech corpus 或真实竖排图片 corpus，因此不声称 OCR、翻译或识别质量提升。日常开发合入 `smalldata_test`，不合并到 `main`。
