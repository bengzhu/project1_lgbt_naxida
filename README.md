...
v3.109 让普通图片 OCR 的识别结果筛选在切换类别后将 VoiceOver 焦点交给第一个可见 OCR 结果行；继续保留隐藏选中项清理、revision-scoped 焦点和无结果空态回退，筛选与焦点仍为 View-only。新增 `scripts/test-v3109-image-filter-result-focus-contract.py`，不新增 Store／持久化、不改变 Vision OCR、翻译、renderer/export、探针报告、Koharu active gate、metrics 或 `output`。候选 full `31064198524`、PR #173 fast `31064487760`、merge fast `31064532453` 均通过；候选 SHA `b3a58afda5cbc0ffddfd337d1787a306a8ba2f36` 的 Xcode/JUnit `10/10` 成功，merge SHA `1a422b2cd90f955895acea644e435fe86899a328` 复用候选 full receipt，探针默认 skip，Koharu readiness 仍为 `manifestMissing / stopUntilArtifactsProvided`，不声称 OCR、翻译、识别或 Koharu 质量提升。
