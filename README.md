...
- 本次未提交工作区 v8 评估：重新构建并在 iPhone 17 Pro 模拟器跑 `test/1.png` 后导出，双框架一致性仍成立：`blocksFoundByBoth = 8`、`blocksOnlyInWholePage = ["Let's Battle!"]`、`blocksOnlyInBubbleFirst = ["What are you even talking about?", "We need to get results at this tournament to save the gaming club from being disbanded."]`、`matchedGroundTruthUnionCount = 11`，因此 `8 + 1 + 2 = 11`，`consistencyPassed = true`。本轮 `wholePageAccuracyVsGroundTruth = 0.6131`，`bubbleFirstAccuracyVsGroundTruth = 0.7397`。
- 本次未提交工作区 v8 #9 专项结论：bubble-first 与主流程复用同一个 `detectBubbleCandidates` 气泡候选检测入口，但后续 OCR/合并策略不同；`ocrSeed` 会在气泡 crop 内再拆文本块，白色连通域则按气泡 crop 合并。#9 区域整图主流程仍是 `What Whet are / every you! / talking`，未可信匹配；bubble-first 实际结果为 `What / are / yOuL / ever talking abodi?`，可信匹配 `What are you even talking about?`，相似度 `0.5000`。这说明现有气泡优先在 #9 上不比整图更差，但仍受同一候选检测层约束，并不是分割问题的根治。
- 本次未提交工作区 v8 决策：不推进第 3 节架构改造，本轮只有证据没有架构改动。原因是 `blocksOnlyInWholePage` 唯一项 `Let's Battle!` 是真实存在且有意义的对话内容，整图主流程实际 OCR 为 `LET'S / BATTLER!`，可信匹配 `Let's Battle!`；bubble-first 本轮没有覆盖这条真实内容，因此当前证据不支持把气泡优先转为唯一主候选源。后续如果要推进，应先补上这类整图独有真实内容的兜底合并策略和去重报告字段，再重新评估。
- 本次未提交工作区 v9 诊断：重新构建并在 iPhone 17 Pro 模拟器跑完整 `test/1.png` 探针后导出。低相似或未匹配块共 12 个；其中专有名词/作品词相关约 6 块，包括 `Senpai/Ren`、`RATTLER -> BATTLER`、`BATTLER!`、`Tournament`，常见词或形近词混淆约 10 块，包括 `PEN DAYS`、`TRANINS`、`THOUSH`、`WOLLD`、`PESULTE/SAMING/POOM/BENG`、`What Whet/every you`、`ONLING`、`SUGSESTION`、`LOSIC`，两类重叠约 4 块。结论：问题不是 Senpai 单点，常见词混淆规模更大。
...
- 本次未提交工作区 v11：在 `fusedWholePageBubble` 后新增 ground-truth-free 的 post-fusion cleanup，报告写入 `fusionComparison.postFusionCleanup`。本轮只用 bbox、bubbleID、来源、文本长度、词覆盖、重叠和 OCR 质量启发拒绝碎片，不用 `test/1.ground_truth.json` 做生产选择；同时新增 `bubbleGeometry.bubbleAudits`，只诊断过大气泡/多块同气泡/分割候选，不默认拆分主流程。
- 本次未提交工作区 v11 实测指标：iPhone 17 Pro 模拟器重新跑 `test/1.png` 并导出，清理前 `16` 块、清理后 `13` 块、拒绝 `3` 块：`THE SUGSESTION WAS OVERPULED...`、`PLAY ONLING...`、`JUST`。三条保护内容 `Let's Battle!`、`What are you even talking about?`、`We need to get results...` 均保留；`groundTruthMatchedBlocks = 13`、`groundTruthUnmatchedBlocks = 0`、核心对话 OCR 相似度 `0.7106`、装饰标题 `0.8000`、`fusionComparison.consistencyPassed = true`、`fusion.fused.accuracyVsGroundTruth = 0.7384`、`cleanTextDiagnostic.passRate = 0.4545`、`passedBlocks = 1`、`failedBlocks = 12`、`translationFailureBreakdown = { modelOutputFailure: 2, ocrInputSuspect: 7, translationLanguageQualityFailure: 3 }`、`likelyRuleFalseFailureBlocks = []`。
- 本次未提交工作区 v12：新增 TextRegion crop OCR 候选层。每个 post-fusion 主块生成结构化 `textRegionCropReport.diagnostics`，记录 seed bbox、region/crop bbox、bubble clamp、padding、whole-page/fused/adaptive/crop 文本、选择理由、拒绝理由、词保留率和质量分；`1_ocr_probe_text.txt` 同步写入这些字段。选择逻辑只用词数、词保留、文本相似度、拉丁/符号比例、疑似 OCR 错误和 crop clamp 等无真值信号。
- 本次未提交工作区 v12 实测指标：iPhone 17 Pro 模拟器重新跑 `test/1.png` 并导出，`textRegionCropReport = { totalRegions: 13, cropSucceededCount: 10, adoptedCount: 0, rejectedCount: 13 }`。主要拒绝原因为 `rawWordsLost = 5`、`emptyCropText = 3`、`wordCountRegression = 2`、`sameAsFusedText = 2`、`insufficientQualityGain = 2`、`introducedLikelyOCRError = 1`。主指标无回归：`totalBlocksDetected = 13`、`matched/unmatched = 13/0`、核心 OCR `0.7106`、装饰 `0.8000`、`fusion.fused.accuracyVsGroundTruth = 0.7384`、`cleanTextDiagnostic.passRate = 0.4545`、`passed/failed = 1/12`，三条保护文本仍保留。结论：本轮得到可审计 crop 诊断，但当前 crop 候选整体没有足够收益，不应强行替换主翻译输入。
- 本次未提交工作区 v13：新增轻量 BubbleMask 子区域诊断和 TextRegion crop subregion clamp。报告新增 `bubbleSubRegionReport`，记录 parent bubble、subregion bbox、seed blocks、coverage、clamp eligibility、拒绝原因；`textRegionCropReport.diagnostics` 同步记录 `clampSource`、`subRegionID`、`subRegionBBox`、subregion clamp 前后 crop bbox 和未使用原因。该层只用 bbox、bubble audit 和几何覆盖信号，不用 ground truth。
- 本次未提交工作区 v13 实测指标：iPhone 17 Pro 模拟器重新跑 `test/1.png` 并导出，`bubbleSubRegionReport = { totalSubRegions: 11, clampEligibleCount: 2, oversizedBubbleIDs: [4,6,7] }`，`clampSources = { bubbleBBox: 9, contentRect: 2, subRegion: 2 }`，subregion clamp 实际用于块 `[6,8]`。TextRegion crop 仍为 `13 regions / 10 succeeded / 0 adopted / 13 rejected`，主指标无回归：`totalBlocksDetected = 13`、`matched/unmatched = 13/0`、核心 OCR `0.7106`、装饰 `0.8000`、`fusion.fused.accuracyVsGroundTruth = 0.7384`、`cleanTextDiagnostic.passRate = 0.4545`、`passed/failed = 1/12`，三条保护文本仍保留。结论：v1.3 提供更清楚的 oversized bubble 子区域和 clamp 证据，但仍不应放宽 crop 采用规则。
- 本次未提交工作区 v14：新增轻量 `bubbleMaskReport`，用现有 bubble bbox 生成实例 ID mask 近似，逐块记录 mask dominant bubble、mask-safe rect、crop mask coverage 和 render mask collision。safe layout 有可信 mask-safe rect 时优先使用，否则回退既有 bbox safe rect；该层不换模型、不用 ground truth 选择、不放宽 TextRegion crop 采用规则。
- 本次未提交工作区 v14 实测指标：iPhone 17 Pro 模拟器重新跑 `test/1.png` 并导出，`bubbleMaskReport = { instanceCount: 8, maskSafeLayoutBlocks: 13, bboxFallbackBlocks: 0, inconsistentBubbleAssignmentBlocks: [4,5,11,12], renderMaskOverflowBlocks: [] }`。TextRegion crop 仍为 `13 regions / 10 succeeded / 0 adopted / 13 rejected`，crop mask coverage 低的块为 `[4,5,9,12]`。主指标无回归：`totalBlocksDetected = 13`、`matched/unmatched = 13/0`、核心 OCR `0.7106`、装饰 `0.8000`、`fusion.fused.accuracyVsGroundTruth = 0.7384`、`cleanTextDiagnostic.passRate = 0.4545`、`passed/failed = 1/12`，三条保护文本仍保留。结论：v1.4 提供 mask 归属和覆盖布局证据，不代表 OCR 分数提升；当前 mask 仍是 bbox/rounded-rect 近似，不是真正 Koharu 实例分割。
- 本次未提交工作区 v15：新增 `bubbleAssignmentCorrectionReport` 和 `bubbleSplitCandidateReport`。前者把 `bubbleMaskReport.inconsistentBubbleAssignmentBlocks = [4,5,11,12]` 拆成逐块修正决策，后者只对 oversized `bubbleID 4/6/7` 生成保守 split candidate；两者只用 bbox/mask/coverage/sibling overlap 等无真值信号，不换模型、不放宽 TextRegion crop 护栏。
...
- 本次未提交工作区 v18：新增 `preCropTextBoxPlanReport`，把 TextBox plan 前移到 TextRegion crop OCR 之前生成。plan 来源限定在 fused seed bbox、bubble geometry、BubbleMask majority/safe rect、subRegion、split candidate、assignment correction 和 glyph/SegmentMask proxy；每块最多 3 个 plan，排序和 shadow OCR 选择不使用 ground truth，且不写回 `finalTextUsedForTranslation`。
- 本次未提交工作区 v18 实测指标：iPhone 17 Pro 模拟器重新跑 `test/1.png` 并导出，`preCropTextBoxPlanReport = { planCount: 37, shadowOCREligiblePlanCount: 29, selectedForShadowOCRBlocks: [0,1,2,3,5,6,7,8,9,10,11], stoppedBlocks: [4,12] }`。`cropExperimentReport = { candidateCount: 48, controlCandidateCount: 13, ocrSucceededCount: 36, betterThanControlCount: 13, promotedShadowBlocks: [], stoppedBlocks: [2,3,4,5,7,9,11,12] }`，主要新增变体为 `preCropTextBoxPlan.seedTightTextBox`、`preCropTextBoxPlan.bubbleContainedTextBox` 和 `preCropTextBoxPlan.maskMajorityTextBox`。TextRegion crop adopted 仍为 `0`，主指标保持 `13 matched / 0 unmatched`、核心 OCR `0.7106`、装饰 `0.8000`、`cleanTextDiagnostic.passRate = 0.4545`，三条保护文本仍保留。
- 本次未提交工作区 v19：新增 `textBoxPlanFailureReport` 和 `sourcePlanID`，把 v18 的 pre-crop plan 与 shadow OCR candidate 稳定关联，输出 plan / candidate / block 三级失败归因、promotion checks、promotion blockers 和 recommended action；`1_ocr_probe_text.txt` 每块新增 `textBoxPlanFailure` / `promotionChecks` 摘要。
- 本次未提交工作区 v19 实测指标：iPhone 17 Pro 模拟器重新跑 `test/1.png` 并导出，`textBoxPlanFailureReport = { evaluatedBlockCount: 13, evaluatedPlanCount: 37, evaluatedCandidateCount: 35, betterThanControlCandidateCount: 13, promotedShadowBlockCount: 0, stopRecommendedBlocks: [2,3,4,5,7,9,11,12], continueGeometryResearchBlocks: [1,6,10], candidatePromotionBlockedBlocks: [1,2,4,5,6,9,10] }`。主要 blockers 为 `qualityDeltaBelowOrEqual0.08 = 31`、`wordPreservationRatioBelow0.80 = 29`、`rawWordsLost = 19`、`notBetterThanControl = 22`；TextRegion crop adopted 仍为 `0`，主指标保持 `13 matched / 0 unmatched`、核心 OCR `0.7106`、装饰 `0.8000`、`fusion.fused.accuracyVsGroundTruth = 0.7384`、`cleanTextDiagnostic.passRate = 0.4545`，三条保护文本仍保留。
- 本次未提交工作区 v20：新增 `lineTextBoxPlanReport` / `lineCropExperimentReport`，只对 v19 建议继续几何研究的 block `[1,6,10]` 生成 Koharu 式行级 `lineTightTextBox`、`lineBandTextBox` 和保守 `deskewProbeTextBox` shadow plan。目标块来自 `textBoxPlanFailureReport.continueGeometryResearchBlocks`，不使用 ground truth 或 OCR 相似度筛选；`1_ocr_probe_text.txt` 每个目标块新增 `lineTextBoxPlans`、`lineCropExperiment`、`linePromotionChecks` 和带原因的 `lineResearchDecision`。
- 本次未提交工作区 v20 实测指标：iPhone 17 Pro 模拟器重新跑 `test/1.png` 并导出，`lineTextBoxPlanReport = { targetBlocks: [1,6,10], planCount: 12, shadowOCREligiblePlanCount: 12 }`，`lineCropExperimentReport = { candidateCount: 12, ocrSucceededCount: 12, betterThanControlCount: 5, promotedLineShadowBlocks: [], stoppedAfterLineResearchBlocks: [1,6,10] }`。block 1 best line delta `0.095` 但 `wordPreservationRatio = 0.571`；block 6 delta `-0.053` 且引入 OCR 错误；block 10 delta `0.046` 且词保留 `0.583`，均未过既有 promotion gate。TextRegion crop adopted 仍为 `0`，主指标保持 `13 matched / 0 unmatched`、核心 OCR `0.7106`、装饰 `0.8000`、`cleanTextDiagnostic.passRate = 0.4545`；`Let's Battle!`、`What are you even talking about?`、`We need to get results...` 仍在明细中可信匹配。结论：当前 line-level / deskew shadow 有局部边际候选，但不足以采用；应停止继续在这 3 块上堆 crop/line/deskew 试参，下一步转向真实 TextBoxes detector、真实 BubbleMask / SegmentMask 或更强 OCR/翻译模型。
- 本次未提交工作区 v21：新增 `externalArtifactReadinessReport`，建立真实 Koharu artifact 适配前证据闸门，覆盖 `TextBoxes`、`BubbleMask`、`SegmentMask` 三类外部输入。读取路径固定为 bundle 内 `test/koharu_artifacts/1.manifest.json`、`1.textboxes.json`、`1.bubbles.json`、`1.segment_mask.json` 或 manifest 指定路径；解析结果只进入 shadow report 和 `1_ocr_probe_text.txt`，不改变 `configuration.currentBlockSource`、`finalTextUsedForTranslation`、主覆盖图、`blockPassed` 或 `textRegionCropReport.adoptedCount`。
- 本次未提交工作区 v21 实测指标：iPhone 17 Pro 模拟器重新跑 `test/1.png` 并导出，当前仓库没有 `test/koharu_artifacts/`，因此 `externalArtifactReadinessReport = { manifestFound: false, textBoxesFound: false, bubbleMaskFound: false, segmentMaskFound: false, readinessVerdict: manifestMissing, nextAction: stopUntilArtifactsProvided, missingArtifacts: [manifest, TextBoxes, BubbleMask, SegmentMask], blockAlignmentCount: 13 }`。`1_ocr_probe_text.txt` 顶部和每块均写入 `externalArtifacts` 摘要。主指标保持 `13 matched / 0 unmatched`、核心 OCR `0.7106`、装饰 `0.8000`、`frameworkComparison.consistencyPassed = true`、`fusionComparison.consistencyPassed = true`、TextRegion crop `13/10/0/13`、line research 仍停在 `[1,6,10]`。结论：当前正确下一步是提供真实 detector / mask artifact 或转换方案；不能声称 detector 已接入，也不继续堆 crop / line / deskew 变体。
