...
当前正式版本：`3.4`（图片失败或取消后修改目标语言会更新 Store-owned Retry 凭据；已完成图片继续即时重译，运行中任务仍冻结语言）。v3.3 的 Koharu mask 拓扑 gate、v3.2 载荷校验、v3.1 的图片 OCR 待复查筛选、v3.0 的 OCR 摘要与 Store-owned 重新识别，以及既有图片生命周期能力仍保留。仓库尚无真实 Koharu 四件套、Speech corpus 或真实竖排图片 corpus，因此不声称 OCR、翻译或识别质量提升。日常开发合入 `smalldata_test`，不合并到 `main`。
...
- `图片`：图片检查区保持主视觉，目标语言菜单与旁贴/覆盖 segmented control 位于工具区；照片、文件、取消、重试、导出和 OCR 块状态集中展示。任务从 loading 起即显示固定的目标语言；失败或取消后修改目标语言只更新下次 Retry 凭据，不立即启动任务，也不会误标现有内容；已完成图片选择不同结果语言后会重新翻译。
