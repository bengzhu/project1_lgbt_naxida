...
- 注入 Koharu artifact 且运行 `ci-fast` / `full` 时，云端探针 smoke 必须证明 App 侧 `externalArtifactReadinessReport.readinessVerdict = readyForShadowOCR`、`externalTextBoxesShadowOCRAllowed = true`、`externalTextBoxShadowOCRReport.executed = true`、`candidateCount > 0`，并且 `koharuNativeArtifactContractDryRunReport.contractDryRunVerdict = activeArtifactsReadyForShadowOCR`；不能只看下载或 validator 步骤日志。
