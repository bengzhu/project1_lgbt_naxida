...
当前正式版本：`3.37`（图片 OCR 修正 sheet 现在让可保存性、未保存修改、是否重译和传给既有 Store 的文本共享同一首尾空白规范化值；`trim` 后仍等于当前原文时仍是“确认无误”，可以直接取消或下滑关闭，不再出现与操作不一致的“放弃修正”确认。真正改变文字时，既有保护、只重译目标 block 和关闭后焦点交接不变。v3.36 的开发控制台 Koharu readiness 摘要仍只读已有 probe report，不触发新探针、不读写 artifact，也不改变普通图片 OCR、翻译、覆盖图或质量基线）。仓库仍没有真实 Koharu 四件套、Speech corpus 或真实竖排图片 corpus，因此不声称 OCR、翻译或识别质量提升。云端 receipt 传播规则（v3.26）仍适用；日常开发合入 `smalldata_test`，不合并到 `main`。
