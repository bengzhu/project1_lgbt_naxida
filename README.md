...
当前正式版本：`3.20`（VoiceOver 连续复查会在开始、完成、撤销、重启后聚焦当前有效结果、局部放大或完成态；图片 revision 变化会拒收旧焦点）。v3.19-v2.2 的行级快速动作、进度、复查队列、直接点选、局部放大、预览、交互、Retry、Koharu gate 与图片生命周期能力仍保留。复查与无障碍焦点状态只属于当前图片 revision，不写 Store 或持久化；仓库尚无真实 Koharu 四件套、Speech corpus 或真实竖排图片 corpus，因此不声称 OCR、翻译或识别质量提升。日常开发合入 `smalldata_test`，不合并到 `main`。
