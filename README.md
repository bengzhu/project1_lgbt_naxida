...
- v1.57 起 bundle-lite / promotion gate 会消费 TextBox -> SegmentMask linkage，输出 linkage breakdown、review / blocked blocks 和 TXT 摘要；v1.58 起 convergence 继续消费这些 linkage work item / gate，使最终收敛层能阻塞弱 linkage。v1.63 起 external TextBox shadow OCR 会在真实 artifact ready 后对竖排或接近 90/180/270 度的 TextBox 执行有上限的 orientation-aware rotation OCR，并记录 attempted rotations、selected rotation、Vision language profile 和 unsupported reason；line polygon warp 与任意角度 deskew 仍保持阻塞。该路径仍是 report-only gate，不新增 LLM / PNG，也不改变主 OCR、翻译、覆盖图或 active artifact。
