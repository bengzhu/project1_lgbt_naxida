...
当前正式版本：`3.16`（图片页有风险块时提供一键待复查队列入口，会切到“待复查”并保留仍有效的当前选择，否则定位首个风险块，再复用现有局部放大与前后导航）。v3.15-v2.2 的直接点选、定位、局部放大、预览、交互、Retry、Koharu gate 与图片生命周期能力仍保留。仓库尚无真实 Koharu 四件套、Speech corpus 或真实竖排图片 corpus，因此不声称 OCR、翻译或识别质量提升。日常开发合入 `smalldata_test`，不合并到 `main`。
