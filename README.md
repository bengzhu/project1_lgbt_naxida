...
当前候选版本：`3.23`（恢复人工修正过的图片 OCR block 前会先确认；取消不改变当前 block、图片历史、导出或复查状态，确认后才恢复该图片的 Vision OCR 原文与初始译文）。v3.22-v2.2 的恢复、单块重译、VoiceOver 焦点、快速复查、进度、预览、Retry、Koharu gate 与图片生命周期能力仍保留。确认流程不改变 Vision OCR、漫画探针或质量基线；仓库尚无真实 Koharu 四件套、Speech corpus 或真实竖排图片 corpus，因此不声称 OCR、翻译或识别质量提升。日常开发合入 `smalldata_test`，不合并到 `main`。
...
- `图片`：图片检查区保持主视觉，有低置信或方向待定块时显示本次复查完成/总数/剩余进度，入口按进度明确显示开始或继续；每个风险结果行可直接完成并定位下一块，也可撤销，主行点击仍只负责图片定位与局部放大。VoiceOver 焦点会按动作来源留在下一结果行或下一块局部放大，队列结束时聚焦完成态。局部放大保留完成/撤销及前后切换，全部完成后可重新开始。完整预览中的 OCR 覆盖块可直接点选或取消；点击 OCR 结果行会把图片工作区带回视口并高亮对应块。目标语言菜单与旁贴/覆盖 segmented control 位于工具区，照片、文件、取消、重试、导出和 OCR 块状态集中展示。任务从 loading 起冻结内容语言；失败或取消后修改输入/目标语言只更新下次 Retry 选择，选回当前内容语言会撤销对应待重试更改，菜单和状态不会继续显示无效差异；免费模式的图片输入语言菜单显示锁定图标，尝试修改时明确提示 Pro 且不污染文本页语言；结果标题始终显示当前内容实际语言，已完成图片选择不同语言后会重新识别或翻译。
