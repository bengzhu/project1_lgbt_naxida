...
当前正式版本：`1.91`（文本首页科技工作台已收口）。v1.89 已收口：人工矩阵、DEBUG paste 可测性与 wide-iPad UI evidence。日常开发合入 `smalldata_test`，不合并到 `main`。
...
- v1.92 候选更新 v1.64 的 orientation 边界：合法四点 `linePolygons` 会用有上限的透视校正生成逐行 shadow OCR crop，成功执行后不再标记 `linePolygonWarpUnsupported`；warp 失败和任意角度 deskew 继续作为 convergence blockers。该能力仍只消费真实四件套，保持 shadow-only，不改变主 OCR、翻译或覆盖图。
