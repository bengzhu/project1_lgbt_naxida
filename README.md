...
当前正式版本：`3.23`（恢复人工修正过的图片 OCR block 前会先确认；取消不改变当前 block、图片历史、导出或复查状态，确认后才恢复该图片的 Vision OCR 原文与初始译文）。v3.22-v2.2 的恢复、单块重译、VoiceOver 焦点、快速复查、进度、预览、Retry、Koharu gate 与图片生命周期能力仍保留。确认流程不改变 Vision OCR、漫画探针或质量基线；仓库尚无真实 Koharu 四件套、Speech corpus 或真实竖排图片 corpus，因此不声称 OCR、翻译或识别质量提升。日常开发合入 `smalldata_test`，不合并到 `main`。
