...
- `AITRANS CI Results` 对 `codeb/**` 和 `smalldata_test` push 默认走 `probe_mode=skip` 快验，只跑静态检查、Xcode build、manifest 和未加密结果包，不下载 GGUF、不启动模拟器漫画探针。
...
- 手动 `workflow_dispatch` 可选填 `koharu_artifact_release_tag`、`koharu_artifact_asset`、`koharu_artifact_sha256`，从 Release 下载真实 Koharu 四件套 archive 并在 Xcode build 前注入 `test/koharu_artifacts/`；`koharu_artifact_required=true` 时下载、SHA、解压或 validator 失败会直接失败。
- 注入 Koharu artifact 且运行 `ci-fast` / `full` 时，云端探针 smoke 必须证明 App 侧 `externalArtifactReadinessReport.readinessVerdict = readyForShadowOCR`、`externalTextBoxesShadowOCRAllowed = true`、`externalTextBoxShadowOCRReport.executed = true`，并且 `koharuNativeArtifactContractDryRunReport.contractDryRunVerdict = activeArtifactsReadyForShadowOCR`；不能只看下载或 validator 步骤日志。
...
- v1.57 起 bundle-lite / promotion gate 会消费 TextBox -> SegmentMask linkage，输出 linkage breakdown、review / blocked blocks 和 TXT 摘要；v1.58 起 convergence 继续消费这些 linkage work item / gate，使最终收敛层能阻塞弱 linkage。v1.62 起 external TextBox shadow OCR 会记录 `sourceDirection` / `linePolygons` / `rotationDegrees` 的 orientation ledger，竖排、line polygon 或旋转候选在 orientation-aware shadow path 未执行前不能进入 would-promote 预览。它们仍是 report-only gate，不新增 OCR / LLM / PNG，也不改变主 OCR、翻译、覆盖图或 active artifact。
