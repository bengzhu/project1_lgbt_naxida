...
当前正式版本：`3.13`（点击 OCR 结果行后，除完整预览高亮外，还会用已下采样预览显示带上下文的 16:9 局部放大窗；可通过 44pt 关闭命令取消定位）。v3.12-v2.2 的选择、预览、交互、Retry、Koharu gate 与图片生命周期能力仍保留。仓库尚无真实 Koharu 四件套、Speech corpus 或真实竖排图片 corpus，因此不声称 OCR、翻译或识别质量提升。日常开发合入 `smalldata_test`，不合并到 `main`。
