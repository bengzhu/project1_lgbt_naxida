...
- `图片操作状态边界`：OCR 结束后的逐块翻译仍保留截图、文字块、选中定位和局部预览，方便确认进度；不过 loading／recognizing／translating、失败或不完整状态都不会开放修正、恢复、忽略恢复、覆盖方式切换、开始／重启复查或完成／撤销复查。只有完整 `.translated` 才能写入这些变更；导出重绘期间还会继续锁住改变结果的操作，避免用户在中间态看到 block 就提前提交复查结论。
