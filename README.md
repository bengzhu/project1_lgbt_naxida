...
当前正式版本：`3.10`（图片预览使用后台 ImageIO 下采样，最大边限制为 2048px 并应用 EXIF 方向，避免在界面任务中解压全分辨率原图）。v3.9-v2.2 的交互、Retry、Koharu gate 与图片生命周期能力仍保留。仓库尚无真实 Koharu 四件套、Speech corpus 或真实竖排图片 corpus，因此不声称 OCR、翻译或识别质量提升。日常开发合入 `smalldata_test`，不合并到 `main`。
