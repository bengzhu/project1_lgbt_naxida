...
- `codeb/v1.87-enterprise-ui` 的 App 相关 push CI 会复用当前 Debug simulator build，在一台紧凑 iPhone 上生成 `ui-evidence/` 多状态截图和 `ui-evidence-manifest.json`；截图步骤失败会使该候选分支 CI 失败，不能用编译结果替代视觉验收。当前不采集 iPad / Mac 证据。
