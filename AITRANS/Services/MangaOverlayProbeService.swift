...
struct MangaOverlayExternalTextBoxCropResult: Equatable, Sendable {
    var text: String?
    var cropBBox: [Double]
    var paddingX: Double
    var paddingY: Double
    var rotationApplied: Double
    var linePolygonWarpAttempted: Bool
    var linePolygonWarpExecuted: Bool
    var linePolygonWarpedLineCount: Int
    var linePolygonWarpFailureReasons: [String]
    var ocrPath: String
    var bboxFallbackReason: String?
    var bboxFallbackError: String?
}
...
struct MangaOverlayLinePolygonWarpError: LocalizedError, Sendable {
    ...
    var errorDescription: String? {
        if let fallbackError {
            return "warpReason=\(reason);fallbackError=\(fallbackError)"
        }
        return reason
    }
}
...
struct MangaOverlayProbeService: Sendable {
    ...
    func recognizeExternalTextBoxCrop(
        ...
        linePolygons: [[[Double]]]? = nil
    ) async throws -> MangaOverlayExternalTextBoxCropResult {
        try await Task.detached(priority: .userInitiated) {
            let bounds = CGRect(x: 0, y: 0, width: CGFloat(image.width), height: CGFloat(image.height))
            ...
            guard seedRect.width >= 2, seedRect.height >= 2 else {
                return MangaOverlayExternalTextBoxCropResult(
                    text: nil,
                    cropBBox: Self.bboxArray(from: seedRect),
                    paddingX: 0,
                    paddingY: 0,
                    rotationApplied: Double(normalizedRotation),
                    linePolygonWarpAttempted: linePolygons?.isEmpty == false,
                    linePolygonWarpExecuted: false,
                    linePolygonWarpedLineCount: 0,
                    linePolygonWarpFailureReasons: linePolygons?.isEmpty == false ? ["linePolygonWarpInvalidTextBoxBBox"] : [],
                    ocrPath: "invalidTextBoxBBox",
                    bboxFallbackReason: nil,
                    bboxFallbackError: nil
                )
            }
            if let linePolygons, !linePolygons.isEmpty {
                do {
                    let warped = try Self.recognizeLinePolygonWarpedText(
                        in: image,
                        linePolygons: linePolygons,
                        options: options,
                        rotationAngle: normalizedRotation,
                        recognitionLanguages: recognitionLanguages
                    )
                    if warped.text?.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty != false {
                        do {
                            ...
                            return MangaOverlayExternalTextBoxCropResult(
                                text: fallback.text,
                                cropBBox: fallback.cropBBox,
                                paddingX: fallback.paddingX,
                    ...
                    let warpReason = "linePolygonWarpExecutionFailed:\(Self.externalTextBoxOCRFailureDescription(error))"
                    do {
                        let fallback = try Self.recognizeExternalTextBoxBBoxCrop(
                            in: image,
                            seedRect: seedRect,
            ...
            return MangaOverlayExternalTextBoxCropResult(
                ...
                paddingY: fallback.paddingY,
                rotationApplied: Double(normalizedRotation),
                linePolygonWarpAttempted: false,
                linePolygonWarpExecuted: false,
                linePolygonWarpedLineCount: 0,
    ...
    private static func externalTextBoxOCRFailureDescription(_ error: Error) -> String {
        "\(String(reflecting: type(of: error))):\(error.localizedDescription)"
    }

    private static func recognizeExternalTextBoxBBoxCrop(
        in image: CGImage,
        seedRect: CGRect,
        bounds: CGRect,
    ...
    ) throws -> (text: String?, cropBBox: [Double], paddingX: Double, paddingY: Double) {
        let padding = adaptiveCropPadding(for: seedRect)
        let cropRect = clamp(seedRect.insetBy(dx: -padding.x, dy: -padding.y), to: bounds).integral
        let text = try recognizePreprocessedText(
            in: image,
            cropRect: cropRect,
            options: options,
        ...
        return (text, bboxArray(from: cropRect), Double(padding.x), Double(padding.y))
    }

    private static func recognizeLinePolygonWarpedText(
        in image: CGImage,
        linePolygons: [[[Double]]],
        options: MangaOverlayPreprocessingOptions,
        rotationAngle: Int,
        ...
        guard linePolygons.count <= maximumLineCount else {
            throw MangaOverlayLinePolygonWarpError(reason: "linePolygonWarpLineLimitExceeded")
        }
    ...
    private static func orderedLinePolygonCorners(
        ...
        guard polygon.count == 4 else {
            throw MangaOverlayLinePolygonWarpError(reason: "linePolygonWarpRequiresFourPoints:\(index)")
        }
        let points = try polygon.enumerated().map { pointIndex, values -> CGPoint in
            guard values.count == 2,
                  values[0].isFinite,
                  values[1].isFinite else {
                throw MangaOverlayLinePolygonWarpError(reason: "linePolygonWarpPointInvalid:\(index):\(pointIndex)")
            }
            let point = CGPoint(x: values[0], y: values[1])
            guard point.x >= 0,
                  point.y >= 0,
                  point.x <= CGFloat(imageWidth),
                  point.y <= CGFloat(imageHeight) else {
                throw MangaOverlayLinePolygonWarpError(reason: "linePolygonWarpPointOutOfBounds:\(index):\(pointIndex)")
            }
            return point
        }
        ...
        let winding = points.sorted { lhs, rhs in
            atan2(lhs.y - centroid.y, lhs.x - centroid.x)
                < atan2(rhs.y - centroid.y, rhs.x - centroid.x)
        }
        guard let startIndex = winding.indices.min(by: { lhs, rhs in
            let lhsScore = winding[lhs].x + winding[lhs].y
            let rhsScore = winding[rhs].x + winding[rhs].y
            if abs(lhsScore - rhsScore) > 0.001 {
                return lhsScore < rhsScore
            }
            if abs(winding[lhs].y - winding[rhs].y) > 0.001 {
                return winding[lhs].y < winding[rhs].y
            }
            return winding[lhs].x < winding[rhs].x
        }) else {
            throw MangaOverlayLinePolygonWarpError(reason: "linePolygonWarpOrderingFailed:\(index)")
        }
        let ordered = Array(winding[startIndex...]) + Array(winding[..<startIndex])
        let closedTail = Array(ordered.dropFirst()) + [ordered[0]]
        let signedArea = zip(ordered, closedTail).reduce(CGFloat.zero) { partial, pair in
            partial + pair.0.x * pair.1.y - pair.1.x * pair.0.y
        } / 2
        let edgeLengths = zip(ordered, closedTail).map { hypot($0.1.x - $0.0.x, $0.1.y - $0.0.y) }
        let crossProducts = ordered.indices.map { pointIndex -> CGFloat in
            let current = ordered[pointIndex]
            let next = ordered[(pointIndex + 1) % ordered.count]
            let following = ordered[(pointIndex + 2) % ordered.count]
            return (next.x - current.x) * (following.y - next.y)
                - (next.y - current.y) * (following.x - next.x)
        }
        let convex = crossProducts.allSatisfy { $0 > 0.5 } || crossProducts.allSatisfy { $0 < -0.5 }
        let selfIntersects = lineSegmentsProperlyIntersect(ordered[0], ordered[1], ordered[2], ordered[3])
            || lineSegmentsProperlyIntersect(ordered[1], ordered[2], ordered[3], ordered[0])
        guard abs(signedArea) >= 4,
              edgeLengths.allSatisfy({ $0 >= 2 }),
              convex,
              !selfIntersects else {
            throw MangaOverlayLinePolygonWarpError(reason: "linePolygonWarpDegenerate:\(index)")
        }
        return ordered
    }

    private static func lineSegmentsProperlyIntersect(
        _ a: CGPoint,
        _ b: CGPoint,
        _ c: CGPoint,
        _ d: CGPoint
    ...
    static func writeOCRProbeText(
        ...
        let externalMaskTopologyByBlock = Dictionary(
            uniqueKeysWithValues: (externalArtifactReadinessReport?.maskTopologyReport?.assignmentLedgers ?? []).map {
                ($0.blockIndex, $0)
            }
        )
        ...
        let content = blocks.map { block in
            ...
            let externalBubbleMajorityMaskValue = externalAlignment?.bubbleMajorityMaskValue.map(String.init) ?? "nil"
            let externalBubblePixelCoverage = externalAlignment?.bubblePixelCoverageRatio?.formatted(.number.precision(.fractionLength(4))) ?? "nil"
            let externalSegmentPixelCoverage = externalAlignment?.segmentPixelCoverageRatio?.formatted(.number.precision(.fractionLength(4))) ?? "nil"
            let externalTextBoxSegmentContainment = externalAlignment?.textBoxSegmentContainmentRatio?.formatted(.number.precision(.fractionLength(4))) ?? "nil"
            let externalMaskTopology = externalMaskTopologyByBlock[block.index]
            let externalMaskTopologyComponents = externalMaskTopology?.componentIndexes.map(String.init).joined(separator: ",") ?? "nil"
            ...
            externalArtifacts: readiness=\(externalReadiness) shadowOCRAllowed=\(externalShadowAllowed) activeDirectory=\(externalActiveDirectory) contractExampleOnly=\(externalContractExampleOnly) maskPayloadGateReady=\(externalArtifactReadinessReport.map { String($0.maskPayloadGateReady == true) } ?? "nil") bubbleMaskPayloadVerdict=\(externalArtifactReadinessReport?.bubbleMaskPayloadVerdict ?? "nil") segmentMaskPayloadVerdict=\(externalArtifactReadinessReport?.segmentMaskPayloadVerdict ?? "nil") textBoxID=\(externalTextBoxMatch) textBoxIoU=\(externalTextBoxIoU) bubbleInstanceID=\(externalBubbleMatch) bubbleIoU=\(externalBubbleIoU) bubbleMajorityMaskValue=\(externalBubbleMajorityMaskValue) bubblePixelCoverage=\(externalBubblePixelCoverage) segmentMask=\(externalSegmentCoverage) segmentPixelCoverage=\(externalSegmentPixelCoverage) textBoxSegmentContainment=\(externalTextBoxSegmentContainment) alignment=\(externalAlignmentVerdict) nextAction=\(externalNextAction)
            externalMaskTopology: gateReady=\(externalArtifactReadinessReport.map { String($0.maskTopologyGateReady == true) } ?? "nil") verdict=\(externalArtifactReadinessReport?.maskTopologyReport?.topologyVerdict ?? "nil") assignmentSource=\(externalArtifactReadinessReport?.maskTopologyReport?.assignmentSource ?? "nil") textBoxID=\(externalMaskTopology?.textBoxID ?? "none") expectedBubbleMaskValue=\(externalMaskTopology.map { String($0.expectedBubbleMaskValue) } ?? "nil") glyphPixels=\(externalMaskTopology.map { String($0.glyphPixelCount) } ?? "nil") expectedBubblePixels=\(externalMaskTopology.map { String($0.segmentPixelsInExpectedBubble) } ?? "nil") foreignBubblePixels=\(externalMaskTopology.map { String($0.foreignBubbleSegmentPixels) } ?? "nil") noBubblePixels=\(externalMaskTopology.map { String($0.noBubbleSegmentPixels) } ?? "nil") components=\(externalMaskTopologyComponents) partitionConserved=\(externalMaskTopology.map { String($0.partitionConserved) } ?? "nil")
            externalTextBoxShadowOCR: executed=\(externalShadowSummary.map { String($0.ocrExecuted) } ?? "false") selectedTextBoxID=\(externalShadowSummary?.selectedTextBoxID ?? "none") candidateBBox=[\(externalShadowBBox)] ocrSucceeded=\(externalShadowSummary.map { String($0.ocrSucceeded) } ?? "false") ocrText=\(externalShadowText) qualityDelta=\(externalShadowDelta) wordPreservation=\(externalShadowPreservation) promotionVerdict=\(externalShadowSummary?.promotionVerdict ?? "skipped") blockers=\(externalShadowBlockers) spatialGeometryVerdict=\(externalShadowSummary?.spatialGeometryVerdict ?? "nil") bubbleAlignmentVerdict=\(externalShadowSummary?.bubbleAlignmentVerdict ?? "nil") assignmentGeometryTrusted=\(externalShadowSummary?.assignmentGeometryTrusted.map(String.init) ?? "nil") sourceDirection=\(externalShadowDirection) orientation=\(externalShadowOrientation) linePolygonsPresent=\(externalShadowLinePolygons) rotationDegrees=\(externalShadowRotation) orientationShadowNeeded=\(externalShadowOrientationNeeded) orientationShadowExecuted=\(externalShadowOrientationExecuted) orientationAttemptedRotations=\(externalShadowAttemptedRotations) orientationSelectedRotation=\(externalShadowSelectedRotation) orientationLanguages=\(externalShadowLanguages) orientationUnsupported=\(externalShadowUnsupported) orientationVerdict=\(externalShadowOrientationVerdict)
        ...
        let convergenceExternalMaskPixelPayloadWorkItem = koharuArtifactConvergenceReport?.workItemLedger.first {
            $0.workItemID == "WI-external-mask-pixel-payload"
        }
        let convergenceExternalMaskPixelPayloadGate = koharuArtifactConvergenceReport?.gateLedger.first {
            $0.gateID == "G-external-mask-pixel-payload"
        }
        let convergenceExternalMaskTopologyWorkItem = koharuArtifactConvergenceReport?.workItemLedger.first {
            $0.workItemID == "WI-external-mask-topology-linkage"
        }
        let convergenceExternalMaskTopologyGate = koharuArtifactConvergenceReport?.gateLedger.first {
            $0.gateID == "G-external-mask-topology-linkage"
        }
        let convergenceExternalShadowCoverageGate = koharuArtifactConvergenceReport?.gateLedger.first {
            $0.gateID == "G-external-textbox-shadow-ocr-coverage"
        }
        ...
        let convergenceExternalOrientationGate = koharuArtifactConvergenceReport?.gateLedger.first {
            $0.gateID == "G-external-textbox-orientation-shadow-path"
        }
        ...
        let nativeArtifactContractDryRunFileSummary = (koharuNativeArtifactContractDryRunReport?.requiredFiles ?? [])
            .map { "nativeArtifactContractDryRunRequiredFile: path=\($0.path) kind=\($0.artifactKind) activeFileFound=\($0.activeFileFound) identityStatus=\($0.identityStatus) size=\($0.fileSizeBytes.map(String.init) ?? "nil") sha256=\($0.sha256 ?? "nil") dryRunPreviews=\($0.dryRunPreviewCount) status=\($0.status) missingFields=\($0.missingRequiredFields.joined(separator: " | ")) forbiddenSourceCount=\($0.forbiddenSourceCount) next=\($0.nextAction)" }
            .joined(separator: "\n")
        ...
        koharuNativeArtifactContractDryRunReport: enabled=\(koharuNativeArtifactContractDryRunReport.map { String($0.enabled) } ?? "nil") requiredFiles=\(koharuNativeArtifactContractDryRunReport.map { String($0.requiredFileCount) } ?? "nil") previews=\(koharuNativeArtifactContractDryRunReport.map { String($0.dryRunPreviewCount) } ?? "nil") gates=\(koharuNativeArtifactContractDryRunReport.map { String($0.contractGateCount) } ?? "nil") verdict=\(koharuNativeArtifactContractDryRunReport?.contractDryRunVerdict ?? "nil") appSideIdentity=\(koharuNativeArtifactContractDryRunReport?.appSideArtifactIdentityVerdict ?? "nil") appSideFilesPresent=\(koharuNativeArtifactContractDryRunReport.map { String($0.appSideArtifactIdentityFilesPresent) } ?? "nil") appSideHashesPresent=\(koharuNativeArtifactContractDryRunReport.map { String($0.appSideArtifactIdentityHashesPresent) } ?? "nil") sourceImageSHA256Declared=\(koharuArtifactIdentityReconciliationReport?.sourceImageSHA256Declared ?? "nil") sourceImageSHA256Expected=\(koharuArtifactIdentityReconciliationReport?.sourceImageSHA256Expected ?? "nil") sourceImageSHA256Matches=\(koharuArtifactIdentityReconciliationReport.map { String($0.sourceImageSHA256Matches) } ?? "nil") dryRunOnly=\(koharuNativeArtifactContractDryRunReport.map { String($0.dryRunOnly) } ?? "nil") activeExportAllowed=\(koharuNativeArtifactContractDryRunReport.map { String($0.activeExportAllowed) } ?? "nil") groundTruthUsedForDecision=\(koharuNativeArtifactContractDryRunReport.map { String($0.groundTruthUsedForDecision) } ?? "nil") wouldChangeMainFlow=\(koharuNativeArtifactContractDryRunReport.map { String($0.wouldChangeMainFlow) } ?? "nil")
        ...
        externalMaskPixelPayload: bubbleVerdict=\(externalArtifactReadinessReport?.bubbleMaskPayloadVerdict ?? "nil") segmentVerdict=\(externalArtifactReadinessReport?.segmentMaskPayloadVerdict ?? "nil") gateReady=\(externalArtifactReadinessReport.map { String($0.maskPayloadGateReady == true) } ?? "nil")
        externalMaskTopology: gateReady=\(externalArtifactReadinessReport.map { String($0.maskTopologyGateReady == true) } ?? "nil") evaluated=\(externalArtifactReadinessReport.map { String($0.maskTopologyReport?.evaluated == true) } ?? "nil") verdict=\(externalArtifactReadinessReport?.maskTopologyReport?.topologyVerdict ?? "nil") assignmentSource=\(externalArtifactReadinessReport?.maskTopologyReport?.assignmentSource ?? "nil") assignedBlocks=\(externalArtifactReadinessReport?.maskTopologyReport?.assignedBlockIndexes.map(String.init).joined(separator: ",") ?? "nil") missingBlocks=\(externalArtifactReadinessReport?.maskTopologyReport?.missingBlockIndexes.map(String.init).joined(separator: ",") ?? "nil") totalGlyphPixels=\(externalArtifactReadinessReport?.maskTopologyReport.map { String($0.totalGlyphPixelCount) } ?? "nil") expectedBubblePixels=\(externalArtifactReadinessReport?.maskTopologyReport.map { String($0.segmentPixelsInExpectedBubble) } ?? "nil") foreignBubblePixels=\(externalArtifactReadinessReport?.maskTopologyReport.map { String($0.foreignBubbleSegmentPixels) } ?? "nil") noBubblePixels=\(externalArtifactReadinessReport?.maskTopologyReport.map { String($0.noBubbleSegmentPixels) } ?? "nil") orphanPixels=\(externalArtifactReadinessReport?.maskTopologyReport.map { String($0.orphanSegmentPixels) } ?? "nil") multiplyAssignedPixels=\(externalArtifactReadinessReport?.maskTopologyReport.map { String($0.multiplyAssignedSegmentPixels) } ?? "nil") duplicateTextBoxes=\(externalArtifactReadinessReport?.maskTopologyReport?.duplicateAssignedTextBoxIDs.joined(separator: ",") ?? "nil") crossBubbleComponents=\(externalArtifactReadinessReport?.maskTopologyReport?.crossBubbleComponentIndexes.map(String.init).joined(separator: ",") ?? "nil") orphanComponents=\(externalArtifactReadinessReport?.maskTopologyReport?.orphanComponentIndexes.map(String.init).joined(separator: ",") ?? "nil") partitionConserved=\(externalArtifactReadinessReport.map { String($0.maskTopologyReport?.partitionConserved == true) } ?? "nil") blockers=\(externalArtifactReadinessReport?.maskTopologyReport?.blockers.joined(separator: ",") ?? "nil")
        externalTextBoxShadowOCR: enabled=\(externalTextBoxShadowOCRReport.map { String($0.enabled) } ?? "nil") executed=\(externalTextBoxShadowOCRReport.map { String($0.executed) } ?? "nil") gateVerdict=\(externalTextBoxShadowOCRReport?.gateVerdict ?? "nil") candidates=\(externalTextBoxShadowOCRReport.map { String($0.candidateCount) } ?? "nil") ocrExecuted=\(externalTextBoxShadowOCRReport.map { String($0.ocrExecutedCount) } ?? "nil") ocrSucceeded=\(externalTextBoxShadowOCRReport.map { String($0.ocrSucceededCount) } ?? "nil") evaluated=\(externalTextBoxShadowOCRReport?.evaluatedBlockCount.map(String.init) ?? "nil") matched=\(externalTextBoxShadowOCRReport?.matchedBlockIndexes?.map(String.init).joined(separator: ",") ?? "nil") succeeded=\(externalTextBoxShadowOCRReport?.succeededBlockIndexes?.map(String.init).joined(separator: ",") ?? "nil") failed=\(externalTextBoxShadowOCRReport?.failedBlockIndexes?.map(String.init).joined(separator: ",") ?? "nil") skipped=\(externalTextBoxShadowOCRReport?.skippedBlocks.map(String.init).joined(separator: ",") ?? "nil") uniqueTextBoxes=\(externalTextBoxShadowOCRReport?.uniqueMatchedTextBoxCount.map(String.init) ?? "nil") duplicateAssignedTextBoxes=\(externalTextBoxShadowOCRReport?.duplicateAssignedTextBoxIDs?.joined(separator: ",") ?? "nil") matchedCoverage=\(externalTextBoxShadowOCRReport?.matchedCoverageRatio?.formatted(.number.precision(.fractionLength(4))) ?? "nil") successfulCoverage=\(externalTextBoxShadowOCRReport?.successfulCoverageRatio?.formatted(.number.precision(.fractionLength(4))) ?? "nil") matchedOCRSuccess=\(externalTextBoxShadowOCRReport?.matchedOCRSuccessRatio?.formatted(.number.precision(.fractionLength(4))) ?? "nil") coverageVerdict=\(externalTextBoxShadowOCRReport?.coverageVerdict ?? "nil") minimumTrustedIoU=\(externalTextBoxShadowOCRReport?.minimumTrustedIoU?.formatted(.number.precision(.fractionLength(3))) ?? "nil") geometryTrustedBlockIndexes=\(externalTextBoxShadowOCRReport?.geometryTrustedBlockIndexes?.map(String.init).joined(separator: ",") ?? "nil") geometryWeakBlockIndexes=\(externalTextBoxShadowOCRReport?.geometryWeakBlockIndexes?.map(String.init).joined(separator: ",") ?? "nil") geometryUnknownBubbleBlockIndexes=\(externalTextBoxShadowOCRReport?.geometryUnknownBubbleBlockIndexes?.map(String.init).joined(separator: ",") ?? "nil") geometryCoverageRatio=\(externalTextBoxShadowOCRReport?.geometryCoverageRatio?.formatted(.number.precision(.fractionLength(4))) ?? "nil") geometryCoverageVerdict=\(externalTextBoxShadowOCRReport?.geometryCoverageVerdict ?? "nil") betterThanControl=\(externalTextBoxShadowOCRReport.map { String($0.betterThanControlCount) } ?? "nil") orientationVerdict=\(externalTextBoxShadowOCRReport?.orientationReadinessVerdict ?? "nil") orientation=\(externalTextBoxShadowOCRReport?.orientationCategoryBreakdown.map { "\($0.key)=\($0.value)" }.sorted().joined(separator: ",") ?? "nil") sourceDirection=\(externalTextBoxShadowOCRReport?.sourceDirectionBreakdown.map { "\($0.key)=\($0.value)" }.sorted().joined(separator: ",") ?? "nil") linePolygonBlocks=\(externalTextBoxShadowOCRReport?.linePolygonCandidateBlocks.map(String.init).joined(separator: ",") ?? "nil") rotationBlocks=\(externalTextBoxShadowOCRReport?.rotationCandidateBlocks.map(String.init).joined(separator: ",") ?? "nil") verticalBlocks=\(externalTextBoxShadowOCRReport?.verticalCandidateBlocks.map(String.init).joined(separator: ",") ?? "nil") orientationNeeded=\(externalTextBoxShadowOCRReport?.orientationShadowPathNeededBlocks.map(String.init).joined(separator: ",") ?? "nil") orientationExecuted=\(externalTextBoxShadowOCRReport?.orientationShadowPathExecutedBlocks.map(String.init).joined(separator: ",") ?? "nil") orientationPartial=\(externalTextBoxShadowOCRReport?.orientationShadowPathPartialBlocks.map(String.init).joined(separator: ",") ?? "nil") orientationNotExecuted=\(externalTextBoxShadowOCRReport?.orientationShadowPathNotExecutedBlocks.map(String.init).joined(separator: ",") ?? "nil") orientationUnsupported=\(externalTextBoxShadowOCRReport?.orientationUnsupportedBlocks.map(String.init).joined(separator: ",") ?? "nil") orientationUnsupportedReasons=\(externalTextBoxShadowOCRReport?.orientationUnsupportedReasonBreakdown.map { "\($0.key)=\($0.value)" }.sorted().joined(separator: ",") ?? "nil")
        ...
        convergenceExternalMaskPixelPayload=workItemStatus:\(convergenceExternalMaskPixelPayloadWorkItem?.status ?? "nil"):gateStatus:\(convergenceExternalMaskPixelPayloadGate?.status ?? "nil"):blocks=\(convergenceExternalMaskPixelPayloadWorkItem?.targetBlocks.map(String.init).joined(separator: ",") ?? "nil"):gateBlocks=\(convergenceExternalMaskPixelPayloadGate?.affectedBlocks.map(String.init).joined(separator: ",") ?? "nil"):next=\(convergenceExternalMaskPixelPayloadWorkItem?.nextAction ?? "nil")
        convergenceExternalMaskTopologyLinkage=workItemStatus:\(convergenceExternalMaskTopologyWorkItem?.status ?? "nil"):gateStatus:\(convergenceExternalMaskTopologyGate?.status ?? "nil"):blocks=\(convergenceExternalMaskTopologyWorkItem?.targetBlocks.map(String.init).joined(separator: ",") ?? "nil"):gateBlocks=\(convergenceExternalMaskTopologyGate?.affectedBlocks.map(String.init).joined(separator: ",") ?? "nil"):next=\(convergenceExternalMaskTopologyWorkItem?.nextAction ?? "nil")
        convergenceExternalShadowOCRCoverage=workItemStatus:\(convergenceExternalShadowCoverageWorkItem?.status ?? "nil"):gateStatus:\(convergenceExternalShadowCoverageGate?.status ?? "nil"):blocks=\(convergenceExternalShadowCoverageWorkItem?.targetBlocks.map(String.init).joined(separator: ",") ?? "nil"):gateBlocks=\(convergenceExternalShadowCoverageGate?.affectedBlocks.map(String.init).joined(separator: ",") ?? "nil"):next=\(convergenceExternalShadowCoverageWorkItem?.nextAction ?? "nil")
        convergenceExternalTextBoxOrientation=workItemStatus:\(convergenceExternalOrientationWorkItem?.status ?? "nil"):gateStatus:\(convergenceExternalOrientationGate?.status ?? "nil"):blocks=\(convergenceExternalOrientationWorkItem?.targetBlocks.map(String.init).joined(separator: ",") ?? "nil"):gateBlocks=\(convergenceExternalOrientationGate?.affectedBlocks.map(String.init).joined(separator: ",") ?? "nil"):next=\(convergenceExternalOrientationWorkItem?.nextAction ?? "nil")
