...
struct MangaOverlayProbeService: Sendable {
    ...
    static func writeOCRProbeText(
        ...
        koharuNativeArtifactContractDryRunReport: enabled=\(koharuNativeArtifactContractDryRunReport.map { String($0.enabled) } ?? "nil") requiredFiles=\(koharuNativeArtifactContractDryRunReport.map { String($0.requiredFileCount) } ?? "nil") previews=\(koharuNativeArtifactContractDryRunReport.map { String($0.dryRunPreviewCount) } ?? "nil") gates=\(koharuNativeArtifactContractDryRunReport.map { String($0.contractGateCount) } ?? "nil") verdict=\(koharuNativeArtifactContractDryRunReport?.contractDryRunVerdict ?? "nil") appSideIdentity=\(koharuNativeArtifactContractDryRunReport?.appSideArtifactIdentityVerdict ?? "nil") appSideFilesPresent=\(koharuNativeArtifactContractDryRunReport.map { String($0.appSideArtifactIdentityFilesPresent) } ?? "nil") appSideHashesPresent=\(koharuNativeArtifactContractDryRunReport.map { String($0.appSideArtifactIdentityHashesPresent) } ?? "nil") dryRunOnly=\(koharuNativeArtifactContractDryRunReport.map { String($0.dryRunOnly) } ?? "nil") activeExportAllowed=\(koharuNativeArtifactContractDryRunReport.map { String($0.activeExportAllowed) } ?? "nil") groundTruthUsedForDecision=\(koharuNativeArtifactContractDryRunReport.map { String($0.groundTruthUsedForDecision) } ?? "nil") wouldChangeMainFlow=\(koharuNativeArtifactContractDryRunReport.map { String($0.wouldChangeMainFlow) } ?? "nil")
