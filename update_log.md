## v3.330：OCR observation best reducer comparator 方向修复（2026-08-23，候选已验证）

静态审计发现 scoped block Vision、compact diagnostic、`synthesizeJapaneseVerticalLineCandidates` 与 `needsJapaneseOrientationFallback` 把降序 observation comparator 传给要求升序谓词的 Swift `Sequence.max(by:)`，混合强弱 observation 时会实际选出最弱项。v3.330 新增 candidate-first generic `bestObservation(in:prefersJapanese:)` 与日语 wrapper：scoped/diagnostic 返回既有 comparator 的最强项，强 primary 不再因弱项误触发 opposite request，空/全弱集合继续 fallback；合成竖列的 rotation/role 改为继承最强 fragment，文字顺序、平均 confidence、rect/owner 不变。既有 comparator/score、scoped usable gate、`.48`/`.5`/单字门、12/8/4 orientation fallback 上限、crop/warp、dedupe/layout、翻译 QA、取消、generation、持久化和非日语规则不变；历史 v3.173/v3.327 合同同步验证 reducer，不再固化旧 bug。新增 `scripts/test-v3330-japanese-best-observation-reducer-contract.py`，工程版本为 `3.330`，Japanese benchmark route 已接入。新合同 `10/10`、329 个无进程合同（1,741 tests）、Python AST `376/376`、JSON `144/144`、workflow YAML `3/3`、shell `32/32`、plist/project `5/5` 与 `git diff --check` 通过，27 个实际导入 `subprocess` 的合同按约束跳过。实现 SHA `7636d441e3246f2fa6bc3afe6de302d6bcda47ff` 的 exact-SHA full [32642708201](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32642708201) 成功，Japanese benchmark job `97202155198`、主 job `97202194469`、静态、Speech、UI/Home/Paste、云端 Xcode `26.6 (17F113)`、manifest、JUnit `10/10` 与 full-validation receipt 全部通过；commit status 为 `AITRANS CI/full-validation=success`，Koharu/GGUF 按非必需策略跳过。未运行本地 Xcode/Swift/Core ML/Rust/GGUF/App runtime；PR/合入 evidence 待补，不引入或等待 Koharu/GGUF，也不外推为通用 OCR/CER、翻译盲评、授权语料、目标设备或 holdout 质量证据。

## v3.329：普通日语 duplicate fusion meaningful 偏好（2026-08-23，已完成）

v3.328 已在普通 recovery 的单框 top-5 pool 内先过滤无意义文本；跨 page/rotation/crop/line observation 的最终日语去重仍可能让长 Latin/标点高分项先占位，遮蔽置信度接近的有效日文。v3.329 在 geometry/text 已判定 duplicate 后、普通 score 替换前加入有界 semantic preference：候选必须通过共享真实日文字母与 letter/script density `>=.5` 门、confidence 有限且 `>=.40`，并位于 incumbent `-.14` 的既有 Vision confidence window 内；否则保留 incumbent。detector/compact 专用替换、owner/boundary 继承、无合格重复项的 page 标点 fallback、非日语、请求预算、crop/warp、layout、翻译 QA、取消、generation 和持久化不变。新增 `scripts/test-v3329-japanese-duplicate-fusion-quality-contract.py`，工程版本为 `3.329`，Japanese benchmark route 已接入。新合同 `10/10`、328 个无外部进程/编译入口合同（1,731 tests）通过，27 个 `subprocess` 合同按约束跳过；Python AST `375/375`、JSON `144/144`、workflow YAML `3/3`、shell `32/32`、plist/project `5/5` 与 `git diff --check` 通过。实现 SHA `c8f4146e2f3cdf9cc6e732eb3ea684e356bba8bc` 的 exact-SHA full [32639974398](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32639974398) 成功，Japanese benchmark job `97195459040`、主 job `97195508891`、静态、Speech、UI/Home/Paste、云端 Xcode、JUnit `10/10` 与 full-validation receipt 全部通过；Koharu/GGUF 按非必需策略跳过。PR [#393](https://github.com/bengzhu/project1_lgbt_naxida/pull/393) CI [32640600464](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32640600464) 的 benchmark job `97196994895` 与主 job `97197051815` 成功，并以 merge SHA `30cd90bb471aa84f4c1e19cc8097b0f900b4dd29` 合入 `smalldata_test`；合入后 push CI [32640680395](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32640680395) 的 benchmark job `97197185384`、主 job `97197239755`、云端 Xcode、JUnit `10/10` 与 full-validation receipt 全部成功。未运行本地 Xcode/Swift/Core ML/Rust/GGUF/App runtime；`main` 未修改。本轮证据不外推为通用 OCR/CER、翻译盲评、真实 Koharu parity、真实 GGUF、授权语料、目标设备或 v3.289 holdout 质量证明。
...
本地安全回归为 322 个无外部进程/编译入口合同全部通过，27 个含编译/runtime 入口的历史合同按约束跳过；新合同 `9/9`、Python AST `369/369`、JSON `144/144`、workflow YAML `3/3`、shell `32/32`、plist/project `5/5` 与 `git diff --check` 全部通过。实现 SHA `0e9248c9f4904a5e89345ac8404c86fc64356f8a` 的 PR [#387](https://github.com/bengzhu/project1_lgbt_naxida/pull/387) CI [32630765838](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32630765838) 成功；exact-SHA full [32630788496](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32630788496) 的 Japanese benchmark、静态检查、Speech、UI interaction、Home/Paste、云端 Xcode build、manifest、JUnit 与 `AITRANS CI/full-validation=success` receipt 全部通过，Koharu/GGUF 按非必需策略跳过。PR 以 merge SHA `242bb32e7870f8c28298d16043223949b3b2837a` 合入 `smalldata_test`；合入后 push CI [32631313211](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32631313211) 的 v3.323 benchmark、静态检查与 full-validation receipt 成功，Xcode/UI/Speech 等按 merge scope 跳过。未运行本地 Xcode/Swift/Core ML/Rust/GGUF/App runtime；`main` 未修改。本轮只证明该局部自有 block fallback 返回门与工程回归通过，不把合同外推为通用 OCR/CER、翻译盲评、真实 Koharu parity、真实 GGUF、授权语料、目标设备或 v3.289 holdout 质量证据。
...
Vision 与 bundled Manga OCR 继续共用 `JapaneseOCRTextNormalizer`；纯日语仍使用历史全角标点 fallback。detector、crop/warp、OCR 请求预算、candidate/geometry/layout、翻译 tag/QA、取消、generation、持久化、非图片路径和 Koharu/GGUF/授权语料/目标设备证据边界不变。新增 `scripts/test-v3311-japanese-fullwidth-token-normalization-contract.py`，工程版本推进至 `3.311` 并接入 Japanese benchmark contract route。

本地安全回归：337 个 v3 合同中 310 个无进程入口合同共 1,576 个测试通过，27 个实际启动外部进程的历史合同按约束跳过；356 个 Python AST、144 个 tracked JSON、3 个 workflow YAML、32 个 tracked shell、4 个 plist/project version 与 `git diff --check` 通过。未运行本地 Xcode/Swift/Core ML/Rust/GGUF/App runtime。
...
新增 `scripts/test-v3305-japanese-mixed-script-normalization-contract.py`，工程版本推进至 `3.305`，接入 Japanese benchmark contract route。该 helper 不调用 OCR、翻译或 Store，不增加请求，也不改变 detector、crop/warp、geometry/layout、候选选择、预算、QA、取消、持久化或非日语路径。本地安全合同与 PR full 已通过；本轮不声称真实 OCR/CER、翻译盲评、GGUF、授权语料、目标设备或 v3.289 holdout 证据。

精确实现 SHA `19854acfa9f37213fa3592aa76632669cb473d70` 的 PR [#369](https://github.com/bengzhu/project1_lgbt_naxida/pull/369) full [32440198386](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32440198386) 成功：Japanese benchmark `96649209341`、主 bundle/Xcode/JUnit/manifest/receipt `96649296050` 全部通过，Koharu `96649209924` 按默认可选策略跳过。PR 以 merge SHA `d43d463bf61f5c92200170fb8ec9359b443d2a44` 合入 `smalldata_test`；合入后 push [32440363618](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32440363618) 的 benchmark `96649695552` 成功、Koharu `96649696285` 跳过，但主 bundle `96649749169` 因历史 standalone Manga OCR harness 漏带 `JapaneseOCRTextNormalizer.swift` 编译失败，不能记录为 `AITRANS CI/full-validation=success`。该集成缺口由 v3.306 修复，`main` 未修改。
...
新增 `scripts/test-v3301-japanese-sfx-kind-inference-contract.py`，工程版本推进至 `3.301`。本地安全回归：v3.301 `7/7`、v3.300 `6/6`、v3.299 `6/6`、v3.295 `9/9`；327 个 Python 合同均 AST 解析成功，300 个无进程入口合同共 1,500 个测试通过，27 个实际含进程入口的历史合同按约束跳过；144 个 JSON、3 个 workflow YAML、32 个 shell、4 个 plist、工程版本两处 `3.301` 与 `git diff --check` 通过。云端 v3.3214 Manga OCR harness 已显式编译 `TranslationContextQuality.swift` 并携带可选 `textKind` 接线；精确 SHA `9be163b62cf486b5a7bd54be20ae0adbdf4880dc` 的 full [32339850147](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32339850147) 成功，benchmark [96336579818](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32339850147/job/96336579818)、主 bundle/UI/Xcode/JUnit/manifest/receipt [96336667362](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32339850147/job/96336667362) 全部通过并发布 `AITRANS CI/full-validation=success`；Koharu parity 按 `koharu_parity_required=false` 正确跳过。未运行本地 Xcode/Swift/Core ML/Rust/GGUF/App runtime；本轮只证明保守类型提示边界、主路径接线和云端工程回归，不声称真实 OCR/CER、翻译质量、盲评、Koharu parity、授权语料、目标设备证据或 v3.289 holdout。
...
PR [#362](https://github.com/bengzhu/project1_lgbt_naxida/pull/362) 随后以 merge SHA `395a488c46c7c1bced3eb9338d2511aded1038f1` 合入 `smalldata_test`；合入后 push CI [32299829133](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32299829133) 的 Japanese benchmark `96219590599` 与主 bundle `96219705330` 成功，主 artifact 对 merge SHA 的 Xcode 26.6、UI interaction、Speech/Home/Paste、JUnit `10/10` 和 receipt 全部成立，Koharu parity 按父链 skip，UI evidence/manga probe 按配置 skip。readiness report 仍为 `blocked`，manifest SHA 和三个安全 flags 不变；v3.292 证据层已进入目标分支，但真实授权 corpus、预测 artifact、目标设备 evidence、OCR/CER、翻译质量与 holdout 仍缺失。
...
修复后的本地允许范围验证：v3.290 合同 `7/7`、v3.289 structure 合同 `7/7`、v3.288 跨批次 QA 合同 `9/9`；全量 `316` 个 Python 合同中 `289` 个无进程入口合同通过、`27` 个含 `subprocess` 的合同跳过、`0` 失败；`334` 个 tracked Python AST、`3` 个 workflow YAML、`30` 个 tracked shell、`4` 个 plist、工程版本两处 `3.290` 与 `git diff --check` 通过。未运行本地 Xcode/Swift/Core ML/Rust/GGUF/App runtime；修复仍须由下一次 exact-SHA cloud full 的 Xcode/JUnit/receipt 验收。
...
第九个 exact-SHA full [32284079603](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32284079603) 对最终 SHA `c14096954ef2f9aa75d7a6ba58482a9baf2cc973` 成功：Koharu parity `96169412154`、benchmark `96169412411`、主 macOS job `96170995494` 的 UI interaction、Xcode、14 张 UI evidence、JUnit、manifest、artifact 与 `AITRANS CI/full-validation=success` receipt 全部通过。合入后的 push CI [32284048360](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32284048360) 也成功；v3.290 已在 `smalldata_test`，`main` 未修改。该验证只收口合同/编译/固定云端 parity 边界，真实 GGUF、授权语料、目标设备证据和 v3.289 holdout 仍待外部提供。
...
v3.276 已把 Koharu `inference_text_blocks` 的临时 `block_index` 贯穿 Vision line、bundled Manga OCR、去重和 layout，但 layout 只把不同 known owner 当作硬分区；同一 owner 的 line 仍须通过普通 same-column、gap 和 protected-boundary 门控。因此，同一 detector TextRegion 中相距较远的多个 line prediction 仍可能被拆成多个最终 block，与 Koharu 先按 `block_index` 无条件归组、再 `join("")` 的语义不一致。

本版只在日语 `prefersMangaReadingOrder` 的 vertical clustering 中启用 owner-first：先为每个 known `verticalTextRegionOwner` 建立一个 cluster，同 owner observations 不再经过第二次 geometry gate；`Cluster.block` 继续按稳定 top-to-bottom 行序无分隔拼接，并以所有 line confidence 的算术平均作为 block confidence。ownerless observations 仍走历史 geometry fallback；它可与唯一命中的 known owner 单行 geometry 保持双向兼容，但同时匹配多个 known owner 时会留在 ownerless cluster，不能成为跨 TextRegion 的桥，也不会被远距离同 owner lines 的 union rect 吸附。不同 known owner 仍绝不合并，非 manga／横排／ownerless-only 路径不启用 owner-first。

临时 owner 仍只存在于 OCR recognition/layout pass，不进入 `ImageTranslationBlock`、Store、UI、导出或持久化。8 个 Manga line 请求、12/24 Vision line candidate、16M perspective pixel、confidence `>=.55`／日语脚本密度 `>=.5`、detector bbox ownership、quad/bbox fallback、crop／warp／方向、取消传播、翻译、渲染和非图片路径均不变。该版本迁移的是 Koharu 的块级分组语义，不新增或替换模型权重，也不把固定 `test/jap.jpg` 外推为通用日语 OCR／翻译质量提升。
...
v3.271 已把右上 `やがって` 的 crop 收紧到文字与省略号范围，但下方仍有人物／背景干扰，confidence 为 `0.23084585`。本版进一步依据 `test/jap.jpg` 的四个主字符连通组件，将 `right-column-a` 从 `(864,136)-(908,342)` 收紧到 `(864,136)-(908,298)`，去掉省略号与下方干扰。固定 pinned Koharu MIT48 reference 在云端仍准确输出 `やがって`，confidence 提升到 `0.98682830`；compact `ニコッ` 为 `0.59093815`，7 个 crop 中 6 个通过日语脚本密度 gate。

既有 `confidence >= .55`、日语脚本密度、compact 精确文本、detector ownership、预算与方向边界保持不变。这是固定样图的云端 crop 诊断改进，不是通用日语 OCR／翻译／识别质量声明；GPL MIT48 权重与 Koharu GPL runtime 仍只在云端 parity 使用，不打包进 AITRANS，也不替换现有 bundled Apache Manga OCR。

新增 `scripts/test-v3272-koharu-mit48-right-column-text-only-contract.py`，工程版本为 `3.272`。本地严格遵守“不要本地编译”，只执行安全 Python 合同、YAML／shell 语法与 `git diff --check`，未运行本地 Xcode、Swift、Rust、Core ML 或 runtime 编译。

验证边界：

- exact-SHA full [31582991645](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31582991645) 对实现 SHA `6c28599fbc205939fc1e50c0dc25d34cda28b184` 成功；Cloud Koharu parity job 为 `94070186027`，云端 Xcode/JUnit/UI/Home/Paste/Speech 与 receipt 全部通过。
- PR #353 的 CI [31584467786](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31584467786) 通过；merge SHA 为 `4c9e50d7e41dc99a1289fa82e0601f5e7f924fb2`，合入 `smalldata_test` 后 push CI [31584537250](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31584537250) 通过。
...
Koharu 当前 `manga_ocr.rs` 直接通过 `crop_text_block_bbox` 消费扩展 detector TextRegion；AITRANS 现在同样把 bbox 作为 Manga OCR 主 crop，严格 line quad 只在 bbox 识别缺失、非日文、日文脚本密度 `<0.5`、置信度非有限或 `<0.55` 时补跑。两路结果继续按可靠性等级、有限置信度和日文字符数择优，避免有效但错投相邻日文列的 quad 以高置信偷换 detector owner，同时保留弱 bbox 的 line-quad 恢复。batch／单 crop 故障隔离、取消传播、detector ownership、12／48 请求预算、Vision fallback、布局、翻译、渲染和非日语路径不变。

新增 `scripts/test-v3239-image-japanese-manga-ocr-bbox-primary-contract.py` 与真实 Core ML 相邻列错配 runtime 并接入 CI，工程版本为 `3.239`。本地 target `今度こそこの爆乳を持ち帰る！` 置信度 `0.99948496`，distractor `前は生意気に俺の誘い断りやがって...` 置信度 `0.9421599`，故意把 distractor quad 附到 target bbox 后仍返回 target；反向 weak-bbox→line-quad runtime 以 `0.9995424` 恢复 target。exact-SHA full [31407753378](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31407753378) 对 SHA `6dc7388a4c1eece3cad4f6e7bc864e0edb245b64` 使用 Xcode `26.6 (17F113)` 完成两条对抗 runtime、单页／长页 runtime、static/UI/Speech/home/paste 与 JUnit `10/10`（0 failures），发布 `AITRANS CI/full-validation=success`，probe `skip`；云端 target 为 `0.99948496`、distractor 为 `0.94102454`，错配后仍返回 target，bbox→quad fallback 为 `0.9994928`，单页 5 个 vertical blocks，长页 16 blocks／16 vertical、底部 y=`0.940558`。本地长页额外保留横排 `ニコッ`，为 17 blocks／16 vertical。另审计 Koharu 当前 `mayocream/manga-ocr` SHA `4380edba990b959c508752350955350c1c80c31c`，其 `264/264` 个共有 tensor 与随包 kha-white revision 完全相等，仅多 `decoder.bert.embeddings.position_ids` buffer，因此不做无识别收益的模型替换。固定 `test/jap.jpg` 只验证 ownership/crop 回归，不外推通用日语 OCR、翻译或识别质量；Koharu artifact readiness 仍为 `manifestMissing / stopUntilArtifactsProvided`。
...
新增 `scripts/test-v3232-image-japanese-koharu-line-quad-manga-ocr-contract.py` 并接入 CI，工程版本为 `3.232`。本地 v3 合同共 `232` 份；单页真实 Core ML runtime 继续精确返回 `5` 个 vertical blocks，四页长图继续返回 `17` blocks／`16` vertical，底部 y 约 `0.940558`。exact-SHA full [31379487189](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31379487189) 对 SHA `f030b9582d1eadb45a3c73acdf8c24219b35ba20` 使用 Xcode `26.6 (17F113)` 完成 static/UI/Speech/home/paste、Xcode 与 JUnit `10/10`（0 failures），发布 `AITRANS CI/full-validation=success`，probe 按配置为 `skip`；固定 `test/jap.jpg` 只证明 line crop geometry 与既有 runtime 边界，不声称通用日语 OCR、翻译或识别质量。Koharu artifact readiness 仍为 `manifestMissing / stopUntilArtifactsProvided`。

v3.157 merge `1266de53935525c1014ec0b4cbecb9b7f20b6e86`（PR #221）与 v3.158 merge `c940815a43e300685667d8b01888e53af910ec9c`（PR #222）继续在当前祖先链；v3.232 已通过 PR #287 合入 `smalldata_test`，merge SHA 为 `100a7807a5c86c1aceb8cbe30e74f6dd22074863`，本地／origin 仅保留 `main` 与 `smalldata_test`。
...
- 本机只有 `/Library/Developer/CommandLineTools`，因此不把本地缺少完整 Xcode 当作编译证据；云端 full [31356368877](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31356368877) 对 SHA `13864d12154b77f86eb177127d77fc1a25eed321` 使用 Xcode `26.6 (17F113)` 完成，static/UI/Speech/home/paste 合同与 JUnit `10/10`（0 failures）通过并发布 `AITRANS CI/full-validation=success`，probe `skip`。固定样图只验证 provenance 与布局消费，不外推通用日语 OCR、翻译或识别质量；Koharu mask artifact readiness 仍为 `manifestMissing / stopUntilArtifactsProvided`。
- v3.157 merge `1266de53935525c1014ec0b4cbecb9b7f20b6e86`（PR #221）与 v3.158 merge `c940815a43e300685667d8b01888e53af910ec9c`（PR #222）均为当前祖先链，候选分支已从本地与 `origin` 清理，无待 cherry-pick。
...
- 候选 metadata [31310728862](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31310728862) 对 docs SHA `bf0ed503ad534870f33ef17f30d6eeca06c71c97` 以 `candidate_metadata_followup_reuses_parent_full_validation` 复用实现 full；PR #277 fast [31310775114](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31310775114) 与 merge fast [31310809139](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31310809139) 均成功并复用候选收据，merge SHA `6ef85fce62d24a1f668a60484369dd24774caf59`。后三者 Xcode skipped，不是新的编译证据。
...
## v3.207：Koharu 日语竖排 line coverage quality gate

日期：2026-08-09

状态：Agent X 收紧 v3.205 line-first OCR 的“覆盖成功”定义。普通图片日语竖排 block 只有在每个可枚举 source line 都被独立 `verticalLine` reread 以 `confidence >= 0.48`、日语脚本密度 `>= 0.5` 通过质量门控时才跳过 block crop；多 glyph source line 不接受单 glyph 结果作为完整覆盖，弱、非日语或空结果继续安全回退 block crop，避免错误的 line success 造成竖排漏字。工程版本为 `MARKETING_VERSION=3.207`，候选分支 `codeb/v3.207-image-japanese-koharu-line-quality-gate` 已推送；候选 exact-SHA full [31294146132](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31294146132) 对 SHA `f7a18e4c008903fd50f15183fa3b9a8629216bef` 完成 Xcode/JUnit `10/10`（0 failures）。

核心变更：

- 新增 `isReliableJapaneseLineCoverageResult`，在既有 tight geometry、一对一 source line 匹配之前过滤低置信度和缺少日语脚本证据的 line 结果。
...
状态：Agent X 继续参考 Koharu `extract_text_block_regions` 的 line-first 消费边界。普通图片日语竖排 block 在存在有效 line polygon proxy 时先执行 perspective／轴对齐 line reread，只有 line OCR 没有可用 observation 才回退 block crop；v3.203 tight ownership、方向 provenance、fallback 与预算边界保持不变。工程正式版本为 `MARKETING_VERSION=3.204`，候选 commit `5e288e4fb477ecbc6489e71b19415ebade6b8137` 已通过 PR [#268](https://github.com/bengzhu/project1_lgbt_naxida/pull/268) 合入 `smalldata_test`，merge SHA `b65653bc577ba65c51ed6cc1c2bd373b00e76aec`；`main` 未触碰。v3.157/v3.158 对应历史候选已合入当前基线，本次继续作为回归合同，不存在未合入的活动分支。
...
- 实现 exact-SHA full [31290525270](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31290525270)：commit `ae922bda0bf566cc14d422a6d9c9a4c042b34218`，`validationProfile=full`，Xcode/static/UI/Speech/home/paste 成功，JUnit `10/10` 且 0 failures；Koharu readiness `manifestMissing`，nextAction `stopUntilArtifactsProvided`。
- 候选 metadata [31290904290](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31290904290)：candidate SHA `5e288e4fb477ecbc6489e71b19415ebade6b8137` 复用实现 full，Xcode/UI/Speech 按 metadata 路由跳过，JUnit 收据生成成功。
- PR #268 fast [31290942391](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31290942391)：`validationProfile=fast` 复用候选 full，Xcode/UI/Speech 跳过，不是新的编译证据。
- merge fast [31290981606](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31290981606)：merge SHA `b65653bc577ba65c51ed6cc1c2bd373b00e76aec`，`validationProfile=fast` 复用候选 full，`receiptPropagationAllowed=true`；Xcode/UI/Speech 跳过，不是新的编译证据。
...
- 文档 metadata follow-up [31224104859](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31224104859)：commit `a3d8f3bb16af010058bb2093cc9ee08eb164a858`，`validationProfile=fast`、`validationReason=smalldata_metadata_followup_reuses_parent_full_validation`，`smalldataIncrementalMetadataOnly=true`，复用父 merge `211dab273bf9b0830b411f0586938ba77c93b46d / success`，`receiptPropagationAllowed=true`，仅六份文档变化，Xcode/UI/Speech 与漫画探针跳过，JUnit `10/10`；不是新的编译证据。

## v3.185：日语竖排漏列 tile reconnaissance

日期：2026-08-08

状态：Agent X 继续迁移 Koharu 的 detector→TextBox→crop/OCR 分层，补齐 Vision 页级与 90°／270° 方向复查都没有产出整列候选时的恢复边界。AITRANS 现在在普通图片日语竖排 block 形成后，对全页最多 6 个带 18% 重叠的全高 tile 做 bounded reconnaissance；已被现有竖排 block 充分覆盖的 tile 跳过，未覆盖 tile 复用既有日语灰度／放大与 90° crop OCR，弱／空结果按页级最多 4 次改走 270°，再交给原图映射、日语去重与既有 block/line 布局。工程正式版本为 `MARKETING_VERSION=3.185`。候选 commit `0c0585a850f4a0a7a4fc4a5735c791439713c4c2` 已通过 PR [#249](https://github.com/bengzhu/project1_lgbt_naxida/pull/249) 合入，merge SHA `ccd46c169ae7447d4f0485e4d982277f8ba33e46`；`main` 未触碰。

核心变更：

- `recognizeJapaneseVerticalTileFallback` 以图片宽度四分之一为 tile 宽、保留 18% overlap，最多建立 6 个起点并强制覆盖右边缘；`verticalTileIsCovered` 只有在既有竖排 block 对 tile 的水平覆盖至少 `0.45`、垂直覆盖至少 `0.30` 时才抑制读取，避免把未探测的整列误当成已覆盖。
- 每个未覆盖 tile 通过 `cropImage`、`prepareJapaneseCropForVision` 与 `recognizeJapaneseCropPass(angle: 90, minimumTextHeight: 0.003, cropScale:)` 读取；`needsJapaneseOrientationFallback` 触发时按独立页级预算最多 4 次 `angle: 270`，结果最终用 `deduplicateJapaneseObservations` 融合。该路径不改变现有 16 个 block、24 个 line、16M warp 像素预算。
- 新增 `scripts/test-v3185-image-japanese-vertical-tile-fallback-contract.py` 并接入显式 UI/full fail-fast；v3.184 及更早合同继续回归。该迁移只作用于普通图片日语 Vision fallback，不加载 Manga OCR/PaddleOCR 权重，不读取探针、ground truth 或真实 Koharu 四件套，不改变普通语言、整页 OCR 主路径、翻译、renderer/export、Store、Koharu active gate、metrics 或 `output`。
...
- 候选 exact-SHA full [31224644168](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31224644168)：`validationProfile=full`、`validationReason=candidate_development_push`，commit `0c0585a850f4a0a7a4fc4a5735c791439713c4c2`，Xcode build、静态、UI、Speech、home、paste 均成功，JUnit `10/10` 且 0 failures；`probe_mode=skip`，Koharu active artifact verdict `manifestMissing`，nextAction `stopUntilArtifactsProvided`。
...
状态：Agent X 继续把 Koharu `comic_text_bubble_detector::merge_slice_regions` 的同标签 IoU 合并边界迁入普通图片日语 OCR。继 v3.193 的 containment-like minimum-area overlap `>= 0.85` 后，`isDuplicateObservation` 对双方都有 Vision 字符范围 `lineRegionRect` 的日语候选增加 IoU `>= 0.50` 几何去重；宽 request-level box、缺少任一紧区域与非日语路径仍使用原有非空文本相等／包含与文本相似度门控，避免相邻竖排列因宽 crop 或普通路径误合并。工程正式版本为 `MARKETING_VERSION=3.194`。候选 commit `e4388409785bc60278b819e36e8b4afdc7ec7307` 已通过 PR [#258](https://github.com/bengzhu/project1_lgbt_naxida/pull/258) 合入，merge SHA `5ce315332aa6ab073e3e9733f174cbf2ccac0121`；`main` 未触碰。

核心变更：

- 新增 `intersectionOverUnion` 安全 helper，以交集／并集计算紧字符区域 IoU，并在零面积时返回 0；日语 tight geometry 只有 containment-like overlap `>= 0.85` 或 IoU `>= 0.50` 才能绕过文本相似度直接去重。
- 新增 `scripts/test-v3194-image-japanese-tight-region-iou-dedupe-contract.py` 并接入显式 UI/full fail-fast；同步让 v3.193 历史版本合同接受后续版本，避免版本推进破坏历史回归。
- 该步只作用于普通图片日语 OCR fusion，不加载新模型或真实 Koharu 工件，不读取探针、ground truth，不改变普通语言、布局、翻译、renderer/export、Store、Koharu active gate、metrics 或 `output`。

边界：候选、PR、merge 均为 `probe_mode=skip`；真实 `test/koharu_artifacts/` 四件套、Speech corpus 与真实竖排图片质量 corpus 仍缺失，active readiness 为 `manifestMissing / stopUntilArtifactsProvided`。`test/jap.jpg` 只作合同 fixture，没有新的 OCR／翻译／Koharu 指标，不得据此声称日语 OCR、翻译、识别或 Koharu 质量提升。

云端证据：

- 候选 exact-SHA full [31232966715](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31232966715)：`validationProfile=full`、`validationReason=manual_full`，commit `e4388409785bc60278b819e36e8b4afdc7ec7307`，Xcode build、静态、UI、Speech、home、paste 均成功，JUnit `10/10` 且 0 failures；`probe_mode=skip`，Koharu active artifact verdict `manifestMissing`，nextAction `stopUntilArtifactsProvided`。
- PR #258 fast [31233259741](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31233259741)：`validationProfile=fast`、`validationReason=pull_request_followup_no_synchronize`，`reusedFullValidationSha=e4388409785bc60278b819e36e8b4afdc7ec7307`、state `success`；Xcode/UI/Speech skipped，不是新的编译证据。
- merge fast [31233297104](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31233297104)：merge SHA `5ce315332aa6ab073e3e9733f174cbf2ccac0121`，`validationProfile=fast`、`validationReason=merge_reuses_successful_candidate_full_validation`，复用候选 full，`receiptPropagationAllowed=true`；Xcode/UI/Speech skipped，不是新的编译证据。

## v3.193：日语 OCR 紧字符区域去重

日期：2026-08-08

状态：Agent X 继续把 Koharu `comic_text_bubble_detector::merge_slice_regions` 的 containment-first 边界迁入普通图片日语 OCR。`isDuplicateObservation` 现在仅在双方都有 Vision 字符范围 `lineRegionRect` 且 minimum-area overlap `>= 0.85` 时，允许几何本身合并几乎包含的重复候选；缺少任一紧区域、只有宽 request-level box 或非日语路径仍使用原有非空文本相等／包含与文本相似度门控，避免相邻竖排列因宽 crop 重叠而被误合并。工程正式版本为 `MARKETING_VERSION=3.193`。候选 commit `a645943b85345790309adde433344a53209bbccc` 已通过 PR [#257](https://github.com/bengzhu/project1_lgbt_naxida/pull/257) 合入，merge SHA `c00ffc306a193351179e86b7fd7ad0783bd1ad4b`；`main` 未触碰。

核心变更：

- 在既有日本语 tight `lineRegionRect` geometry 选择后增加 `hasTightJapaneseGeometry && overlap >= 0.85` 快速去重，直接对应 Koharu `merge_slice_regions` 的 containment-like 合并语义；request-level `rect` 仍不能仅凭几何合并。
- 新增 `scripts/test-v3193-image-japanese-tight-region-dedupe-contract.py` 并接入显式 UI/full fail-fast；同步让 v3.192 历史版本合同接受后续版本，避免版本推进破坏历史回归。
- 该步只作用于普通图片日语 OCR fusion，不加载新模型或真实 Koharu 工件，不读取探针、ground truth，不改变普通语言、布局、翻译、renderer/export、Store、Koharu active gate、metrics 或 `output`。
...
- 候选 exact-SHA full [31232333781](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31232333781)：`validationProfile=full`、`validationReason=manual_full`，commit `a645943b85345790309adde433344a53209bbccc`，Xcode build、静态、UI、Speech、home、paste 均成功，JUnit `10/10` 且 0 failures；`probe_mode=skip`，Koharu active artifact verdict `manifestMissing`，nextAction `stopUntilArtifactsProvided`。
- PR #257 fast [31232570686](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31232570686)：`validationProfile=fast`、`validationReason=pull_request_followup_no_synchronize`，`reusedFullValidationSha=a645943b85345790309adde433344a53209bbccc`、state `success`；Xcode/UI/Speech skipped，不是新的编译证据。
- merge fast [31232612519](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31232612519)：merge SHA `c00ffc306a193351179e86b7fd7ad0783bd1ad4b`，`validationProfile=fast`、`validationReason=merge_reuses_successful_candidate_full_validation`，复用候选 full，`receiptPropagationAllowed=true`；Xcode/UI/Speech skipped，不是新的编译证据。
...
状态：Agent X 继续把 `reference/koharu-main` 的检测切片边界迁入普通图片日语 OCR。Koharu `comic_text_bubble_detector::ImageSlicer` 使用目标切片高宽比 `3.0`、纵向重叠 `0.2`，并在最后一片不足目标高的 `0.7` 时并入上一片；v3.191 将同一规则应用到每条竖排窄条，使长漫画页的 OCR crop 尺寸与 Koharu detector 的局部输入几何一致。工程正式版本为 `MARKETING_VERSION=3.191`。最终提交 `32c350c6ce4c40d90dd1d9505a830c083a62ce49` 已通过 PR [#255](https://github.com/bengzhu/project1_lgbt_naxida/pull/255) 合入，merge SHA `16cce70a48afda648f045edf9d97f9a1191890cd`；`main` 未触碰。

核心变更：

- 新增 `japaneseVerticalSliceWindows(imageHeight:stripWidth:)`，以 `3.0 / 0.20 / 0.70` 复现 Koharu `ImageSlicer` 的目标高度、effective step、尾片判断与触底窗口；短图安全回退为单窗口。
- `recognizeJapaneseVerticalTileFallback` 消费该 helper 的局部 `(start,height)`，总有效窗口最多 18 个；既有 block coverage gate、90° 主读取、最多 4 次 270° 方向 fallback、日语脚本／几何过滤、灰度／放大、source direction、原图映射与去重保持不变。
- 新增 `scripts/test-v3191-image-japanese-koharu-slicer-contract.py` 并接入显式 UI/full fail-fast；该合同不依赖未随 CI checkout 的研究目录，只验证已迁移的产品常量和边界。该变更不加载 Manga OCR/PaddleOCR 权重，不读取探针、ground truth 或真实 Koharu 工件，不改变普通语言、翻译、renderer/export、Store、Koharu active gate、metrics 或仓库 `output/`。

边界：最终 full 使用 workflow_dispatch exact-SHA `probe_mode=skip`；真实 Koharu 四件套、Speech corpus 与真实竖排图片质量 corpus 仍缺失，active readiness 为 `manifestMissing / stopUntilArtifactsProvided`。`test/jap.jpg` 只作合同 fixture，没有新 OCR／翻译／Koharu 指标，不得据此声称日语 OCR、翻译、识别或 Koharu 质量提升。

云端证据：

- 最终 exact-SHA full [31230729061](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31230729061)：`validationProfile=full`、`validationReason=manual_full`，commit `32c350c6ce4c40d90dd1d9505a830c083a62ce49`，Xcode build、静态、UI、Speech、home、paste 均成功，JUnit `10/10` 且 0 failures；Koharu verdict `manifestMissing`，nextAction `stopUntilArtifactsProvided`。
- PR #255 fast [31231091935](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31231091935)：`validationProfile=fast`、`validationReason=pull_request_followup_no_synchronize`，`reusedFullValidationSha=32c350c6ce4c40d90dd1d9505a830c083a62ce49`、state `success`；Xcode/UI/Speech skipped，不是新的编译证据。
- merge fast [31231128313](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31231128313)：`validationProfile=fast`、`validationReason=merge_reuses_successful_candidate_full_validation`，复用最终 full，`receiptPropagationAllowed=true`；Xcode/UI/Speech skipped，不是新的编译证据。

## v3.190：日语竖排局部侦察窗口

日期：2026-08-08

状态：Agent X 继续把普通图片日语 Vision fallback 向 Koharu 的 `TextBox → crop/OCR` 边界收敛。v3.185 的全高窄条能恢复整列漏识别，但在长漫画页上会让文字只占 crop 的很小比例；v3.190 将每条横向侦察带拆成局部纵向窗口，使 `test/jap.jpg` 这类长图中的候选以更大相对尺寸进入同一 Vision crop reread。工程正式版本为 `MARKETING_VERSION=3.190`。候选 commit `44e0030aa1914468fceedfc5404eaf7510e9673b` 已通过 PR [#254](https://github.com/bengzhu/project1_lgbt_naxida/pull/254) 合入，merge SHA `ec8ad6333005527042fd7f13f8f203bfa36fafe5`；`main` 未触碰。

核心变更：

- `recognizeJapaneseVerticalTileFallback` 以页面高度 `58%` 与 `tileWidth * 3` 的较大值作为局部窗口高度，纵向保留 `18%` overlap，并校正最后一个窗口以覆盖底边；横向四分之一页宽、18% overlap 与右边缘覆盖保持不变。
- 只有通过既有 block coverage gate 且成功裁剪的窗口才计入总预算，最多执行 12 个有效窗口；90° 主读取、最多 4 次弱结果 270° fallback、灰度／放大、日语脚本／几何过滤、原图坐标映射、direction hint 与去重均继续复用既有路径。
- 新增 `scripts/test-v3190-image-japanese-localized-tile-window-contract.py` 并接入显式 UI/full fail-fast；v3.156–v3.189 日语合同全部回归。该变更不加载 Manga OCR/PaddleOCR 权重，不读取探针、ground truth 或真实 Koharu 工件，不改变普通语言、翻译、renderer/export、Store、Koharu active gate、metrics 或仓库 `output/`。

边界：候选、PR、merge 均为 `probe_mode=skip`；真实 Koharu 四件套、Speech corpus 与真实竖排图片质量 corpus 仍缺失，active readiness 为 `manifestMissing / stopUntilArtifactsProvided`。`test/jap.jpg` 只作合同 fixture，没有新 OCR／翻译／Koharu 指标，不得据此声称日语 OCR、翻译、识别或 Koharu 质量提升。

云端证据：

- 候选 exact-SHA full [31229567448](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31229567448)：`validationProfile=full`、`validationReason=candidate_development_push`，commit `44e0030aa1914468fceedfc5404eaf7510e9673b`，Xcode build、静态、UI、Speech、home、paste 均成功，JUnit `10/10` 且 0 failures；Koharu verdict `manifestMissing`，nextAction `stopUntilArtifactsProvided`。
- PR #254 fast [31229946643](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31229946643)：`validationProfile=fast`、`validationReason=pull_request_followup_no_synchronize`，`reusedFullValidationSha=44e0030aa1914468fceedfc5404eaf7510e9673b`、state `success`；Xcode/UI/Speech skipped，不是新的编译证据。
- merge fast [31229977158](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31229977158)：`validationProfile=fast`、`validationReason=merge_reuses_successful_candidate_full_validation`，复用候选 full，`receiptPropagationAllowed=true`；Xcode/UI/Speech skipped，不是新的编译证据。

## v3.189：日语单字 crop direction hint

日期：2026-08-08

状态：Agent X 继续对齐 Koharu 的 `TextBox.source_direction` 语义。v3.187 已让日语竖排 crop reread 携带 `.vertical` hint，但布局仍要求文本至少两个 CJK scalar 才接受该 hint；Vision 将窄列拆成单 glyph 时，这个可信 provenance 会被丢弃，随后可能被 `wideBox` 几何回退误判为横排。v3.189 将门控改为“只要含 CJK 即可”，仍保留 `allowsVerticalText` 与 `prefersMangaReadingOrder` 两层日语边界，并继续让页级 observation 不携带 hint。工程正式版本为 `MARKETING_VERSION=3.189`。候选 commit `8c89b0c0d1c0dff91efd3c115583ab5f04d14017` 已通过 PR [#253](https://github.com/bengzhu/project1_lgbt_naxida/pull/253) 合入，merge SHA `6a510efece0b7ef6deeb9f1c3dd19c11f625ba1a`；`main` 未触碰。

核心变更：

- `ImageOCRLayoutEngine.resolveDirection` 新增 `hasCJKText = cjkCount > 0`，日语 manga-order 且 `sourceDirectionHint == .vertical` 时以 `hasCJKText` 提前返回 `koharuVerticalCropHint`；单字／单标点 crop 不再要求 `containsTextRun`，仍位于 `wideBox` 回退之前。
- hint 仍只由日语竖排 block／line／tile crop reread 与合成结果提供；整页 `mapRotatedObservation`、普通语言、横排与非日语路径保持原几何启发式。新增合同覆盖单 glyph、作用域、页级 mapper 与版本/CI route；不加载 Manga OCR/PaddleOCR 权重、不读取探针或 ground truth，不改变 OCR、翻译、renderer/export、Store、Koharu active gate、metrics 或 `output`。

边界：候选、PR、merge 均为 `probe_mode=skip`；真实 `test/koharu_artifacts/` 四件套、Speech corpus 与真实竖排图片质量 corpus 仍缺失，active readiness 为 `manifestMissing / stopUntilArtifactsProvided`。`test/jap.jpg` 只作合同 fixture，没有新 OCR／翻译／Koharu 指标，不更新 `metrics/version_history.csv` 或仓库 `output/`，不得据此声称日语 OCR、翻译、识别或 Koharu 质量提升。

云端证据：

- 候选 exact-SHA full [31228731388](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31228731388)：`validationProfile=full`、`validationReason=candidate_development_push`，Xcode build、静态、UI、Speech、home、paste 均成功，JUnit `10/10` 且 0 failures；Koharu active artifact verdict `manifestMissing`，nextAction `stopUntilArtifactsProvided`。
- PR #253 fast [31229051329](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31229051329)：`validationProfile=fast`、`validationReason=pull_request_followup_no_synchronize`，`reusedFullValidationSha=8c89b0c0d1c0dff91efd3c115583ab5f04d14017`、state `success`；Xcode/UI/Speech 跳过，不是新的编译证据。
- merge fast [31229096792](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31229096792)：merge SHA `6a510efece0b7ef6deeb9f1c3dd19c11f625ba1a`，`validationProfile=fast`、`validationReason=merge_reuses_successful_candidate_full_validation`，复用候选 full，`receiptPropagationAllowed=true`；Xcode/UI/Speech 跳过，不是新的编译证据。

## v3.188：日语竖排 Cluster 确定性读取顺序

日期：2026-08-08

状态：Agent X 继续对齐 Koharu 的 Recursive XY-Cut 读取语义。Koharu 在右到左文字列内明确保持自上而下；AITRANS 之前让 vertical `Cluster` 直接按 observation 到达顺序拼接，日语 crop/tile reread 的返回顺序变化可能造成同列文字乱序。v3.188 在已形成的同列 cluster 内显式按 `rect.y` 上到下排序，同 y 时按 `midX` 右到左，再以宽高、文字和 confidence 做确定性 tie-breaker 后拼接；横排换行与同列合并门控保持不变。工程正式版本为 `MARKETING_VERSION=3.188`。候选最终 commit `1580a5c145efede4fefa23fd743c8170e077b1f9` 已通过 PR [#252](https://github.com/bengzhu/project1_lgbt_naxida/pull/252) 合入，merge SHA `bbba2eee0a31a3bfd8b82508c77caac3677e1810`；`main` 未触碰。

核心变更：

- `Cluster.block` 仅在方向为 `.vertical` 时建立 `orderedObservations`，使用有限、文件内自包含的比较器按 y↑、同 y 的 x↓、几何／文字／confidence 稳定排序，再拼接文本；不依赖 `cluster` merge traversal 或 crop/tile 到达顺序。
- `.horizontal` 与 `.unknown` 继续使用原有 `observations.enumerated().reduce`，同列合并仍要求 `sameColumn`、有限纵向 gap 和既有 overlap／中心距离门控；排序不扩大候选，不改变 OCR、翻译、renderer/export、Store、Koharu active gate、metrics 或 `output`。
- 新增 `scripts/test-v3188-image-japanese-vertical-cluster-reading-order-contract.py` 并接入显式 UI/full fail-fast；v3.156–v3.187 日语合同继续回归。第一次候选提交 `e29a6bc2d03bbe6c036d77da597eeb24d2d25dba` 的 full `31227549010` 因 comparator 作用域错误失败，仅作修复前记录，不作为验收证据；最终提交已修正并通过云端 full。

边界：最终候选、PR、merge 均为 `probe_mode=skip`；真实 `test/koharu_artifacts/` 四件套、Speech corpus 与真实竖排图片质量 corpus 仍缺失，active readiness 为 `manifestMissing / stopUntilArtifactsProvided`。`test/jap.jpg` 只作合同 fixture，没有新的 OCR／翻译／Koharu 指标，不更新 `metrics/version_history.csv` 或仓库 `output/`，不得据此声称日语 OCR、翻译、识别或 Koharu 质量提升。

云端证据：

- 候选最终 exact-SHA full [31227754288](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31227754288)：`validationProfile=full`、`validationReason=candidate_development_push`，commit `1580a5c145efede4fefa23fd743c8170e077b1f9`，Xcode build、静态、UI、Speech、home、paste 均成功，JUnit `10/10` 且 0 failures；`probe_mode=skip`，Koharu active artifact verdict `manifestMissing`，nextAction `stopUntilArtifactsProvided`。
- PR #252 fast [31228156354](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31228156354)：`validationProfile=fast`、`validationReason=pull_request_followup_no_synchronize`，`reusedFullValidationSha=1580a5c145efede4fefa23fd743c8170e077b1f9`、state `success`；Xcode/UI/Speech skipped，不是新的编译证据。
- merge fast [31228206021](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/31228206021)：merge SHA `bbba2eee0a31a3bfd8b82508c77caac3677e1810`，`validationProfile=fast`、`validationReason=merge_reuses_successful_candidate_full_validation`，`reusedFullValidationSha=1580a5c145efede4fefa23fd743c8170e077b1f9`、state `success`、`receiptPropagationAllowed=true`；Xcode/UI/Speech skipped，不是新的编译证据。
...
## v3.307：completed-only 跨 batch context 边界（2026-08-21）

v3.288–v3.304 已把术语记忆、稳定 batch identity、全局 ordinal 和上一批只读摘要接到普通日语图片翻译；本轮继续收紧 transient context 的输入资格。`TranslationReadOnlyBatchSummary.isEligibleForPrompt` 现在要求摘要只读、无 pending input、明确来自 completed blocks，至少有一个 item，且每个 item 的 ordinal、source excerpt 与 target excerpt 有效。`TranslationPromptContext.normalized()` 丢弃不合格摘要，block-level QA configuration 复用同一 normalized context，previous-context leakage 也只检查合格摘要。

这只是 context provenance/QA 边界：无效摘要不进入模型 prompt、不参与泄漏判定，也不写入 block、snapshot、Store、transcript 或导出；OCR request、geometry/layout、8 blocks/1,800 chars、tag、fallback、取消、partial persistence、UI、renderer 和非日语路径不变。本轮不声称真实 OCR/CER、翻译盲评、GGUF、授权语料、目标设备或 v3.289 holdout 证据。新增 `scripts/test-v3307-japanese-translation-completed-context-contract.py`，工程版本推进至 `3.307`，CI Japanese benchmark contract route 已接入。候选 SHA `30ee7337132fbb27bb2dae0069840f94c463eecb` 的 exact-SHA full [32446579737](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32446579737) 成功：benchmark `96667203528`、主 bundle/Xcode/UI/Speech `96667258662` 与 `AITRANS CI/full-validation=success` receipt 全部通过；首次 full 因 `japaneseTranslationQAConfiguration` 漏 `return` 的编译错误失败，已在同一候选分支补齐并重跑。PR [#371](https://github.com/bengzhu/project1_lgbt_naxida/pull/371) 已合入 `smalldata_test`，merge SHA `01dc186ce16cc0941c5568729d68c4e7532d4060`；合入后 push CI [32447271858](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32447271858) 的 benchmark `96669079883`、主 bundle `96669134491` 与 full-validation receipt 成功，Koharu parity 按未提供真实 artifact 的策略 skip。`main` 未修改。
## v3.308：日语图片 scoped OCR 的有限 Manga/Vision 候选门（2026-08-21，已完成）

v3.307 的 completed-only context 边界已完成后，继续审计普通图片 OCR 主路径发现：同一日语 block 的 scoped rerecognition 在 Manga OCR 达到门槛后立即返回，不会比较已有 bounded Vision crop；Vision 只能在 Manga 失败或不达标时生效，可能错过同一 crop 上更完整的可接受读取。

本轮把同一 scoped block 的 bundled Manga OCR 保留为确定性基线，并收集既有 bounded Vision 方向候选。垂直 block 仍最多 `[270, 90]`、横排 `[0]`、未知方向 `[270, 90, 0]`；Vision 结果只有通过非空、有限 confidence `>=.55`、日语脚本密度 `>=.5`，并以文字数量/密度或 bounded confidence margin 证明更强时才替换，平局保持 Manga。Vision 单方向失败、模型缺失或弱候选不会清除已接受基线，取消继续抛出 `CancellationError`。

这只是普通 OCR scoped candidate gate：不重跑 detector/layout，不改变 block geometry、translation QA、generation、scoped cancel、reviewed progress、持久化或非日语路径；不引入 Koharu/GPL artifact，不使用 ground truth，不新增固定 `test/jap.jpg` 特判。新增 `scripts/test-v3308-image-japanese-scoped-candidate-contract.py`，工程版本推进至 `3.308`，CI Japanese benchmark contract route 已接入。本地 `9/9` 新合同、`302` 个无进程入口合同、`354` 个 Python AST、`3` 个 tracked workflow YAML、`32` 个 shell、`4` 个 tracked plist、`144` 个非 third-party JSON 与 `git diff --check` 通过；32 个含进程/运行时标记的历史合同按约束跳过，未运行本地 Xcode/Swift/Core ML/Rust/GGUF/App runtime。候选 SHA `04d4666f688145a92c941cab66f7d93a4b54d93d` 的 exact-SHA full [32450328440](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32450328440) 成功：benchmark `96677522229`、主 bundle/Xcode/UI/Home/Paste/Speech/JUnit/manifest/receipt `96677576664` 全部通过，Koharu `96677522984` 按未提供 artifact 策略 skip。PR [#372](https://github.com/bengzhu/project1_lgbt_naxida/pull/372) 已以 merge SHA `f55e73ace7f70fc0203c03a2cc1d9543c0e875e6` 合入 `smalldata_test`；合入后 push CI [32451237937](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32451237937) 的 benchmark `96680023149`、主 bundle/receipt `96680086359` 成功，Xcode 按 changed-file scope skip，Koharu `96680023991` skip。`main` 未修改。
## v3.309：同步 scoped OCR 接受候选的 provenance（2026-08-21，已完成）

v3.308 已在同一 bounded crop 上比较 bundled Manga OCR 与有限 Vision 方向候选，但 scoped reread 提交时只复制了新文字、置信度和自动 SFX 类型提示，旧 `ocrProvenance` 仍会被保留，导致复查 disclosure 可能出现“新文字、旧引擎/旧 crop/旧旋转来源”。本轮在既有 `rerecognizeImageTranslationBlock` scoped commit 中同步 `recognized.ocrProvenance`；既有 transient Vision baseline/restore 路径保持不变。
