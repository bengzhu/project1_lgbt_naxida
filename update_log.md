...
新增 `scripts/test-v3239-image-japanese-manga-ocr-bbox-primary-contract.py` 与真实 Core ML 相邻列错配 runtime 并接入 CI，工程版本为 `3.239`。本地 target `今度こそこの爆乳を持ち帰る！` 置信度 `0.99948496`，distractor `前は生意気に俺の誘い断りやがって...` 置信度 `0.9421599`，故意把 distractor quad 附到 target bbox 后仍返回 target；反向 weak-bbox→line-quad runtime 以 `0.9995424` 恢复 target。单页保持 5 个 vertical blocks，四页长图保持 17 blocks／16 vertical、底部 y=`0.940558`；候选 exact-SHA full 尚待提交后验证。另审计 Koharu 当前 `mayocream/manga-ocr` SHA `4380edba990b959c508752350955350c1c80c31c`，其 `264/264` 个共有 tensor 与随包 kha-white revision 完全相等，仅多 `decoder.bert.embeddings.position_ids` buffer，因此不做无识别收益的模型替换。固定 `test/jap.jpg` 只验证 ownership/crop 回归，不外推通用日语 OCR、翻译或识别质量；Koharu artifact readiness 仍为 `manifestMissing / stopUntilArtifactsProvided`。
