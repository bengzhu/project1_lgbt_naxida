...
## v3.307：completed-only 跨 batch context 边界（2026-08-21，进行中）
...
这只是 context provenance/QA 边界：无效摘要不进入模型 prompt、不参与泄漏判定，也不写入 block、snapshot、Store、transcript 或导出；OCR request、geometry/layout、8 blocks/1,800 chars、tag、fallback、取消、partial persistence、UI、renderer 和非日语路径不变。本轮不声称真实 OCR/CER、翻译盲评、GGUF、授权语料、目标设备或 v3.289 holdout 证据。新增 `scripts/test-v3307-japanese-translation-completed-context-contract.py`，工程版本推进至 `3.307`，CI Japanese benchmark contract route 已接入；full/PR/merge/post-merge receipt 待完成。
