...
## v3.308：日语图片 scoped OCR 的有限 Manga/Vision 候选门（2026-08-21，验证中）
...
这只是普通 OCR scoped candidate gate：不重跑 detector/layout，不改变 block geometry、translation QA、generation、scoped cancel、reviewed progress、持久化或非日语路径；不引入 Koharu/GPL artifact，不使用 ground truth，不新增固定 `test/jap.jpg` 特判。新增 `scripts/test-v3308-image-japanese-scoped-candidate-contract.py`，工程版本推进至 `3.308`，CI Japanese benchmark contract route 已接入。本地静态回归与 exact-SHA cloud full 尚待本轮继续完成；在证据完成前不声称真实 OCR/CER、翻译质量、GGUF、授权语料、目标设备或 v3.289 holdout 改善。
