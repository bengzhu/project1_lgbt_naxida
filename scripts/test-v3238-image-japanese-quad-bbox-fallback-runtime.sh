...
if blank_confidence is None or float(blank_confidence.group(1)) >= 0.55:
    raise SystemExit(f"blank quad must produce a weak primary result: {text}")
if fallback_confidence is None or float(fallback_confidence.group(1)) < 0.55:
    raise SystemExit(f"detector bbox retry must produce a reliable result: {text}")
if blank is None or fallback is None or blank.group(1) == fallback.group(1):
    raise SystemExit(f"detector bbox retry did not replace the weak quad text: {text}")
if not re.search(r"[\u3041-\u30ff\u3400-\u9fff]", fallback.group(1)):
    raise SystemExit(f"bbox retry did not recover Japanese Manga OCR text: {text}")
PY
