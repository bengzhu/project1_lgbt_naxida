#!/usr/bin/env python3
"""Static and pure-policy contract for v3.339 detector merge closure."""
...
def merge_once(rects: list[Rect]) -> list[Rect]:
    """Model the pre-v3.339 scan, including swap-remove ordering."""
...
def merge_to_closure(rects: list[Rect]) -> list[Rect]:
    """Model the v3.339 scan, restarting after every envelope expansion."""
    remaining = list(rects)
    merged: list[Rect] = []
    while remaining:
        candidate = remaining.pop()
