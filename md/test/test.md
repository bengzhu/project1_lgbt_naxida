...
- 若注入的真实 TextBox 带 `sourceDirection`、`linePolygons` 或 `rotationDegrees`，还必须核对 `externalTextBoxShadowOCRReport.orientationReadinessVerdict`、`orientationShadowPathNeededBlocks`、`orientationShadowPathNotExecutedBlocks`、候选 `riskFlags/blockers` 和 `1_ocr_probe_text.txt` 的 orientation 摘要；当前未实现 orientation-aware shadow OCR 时，相关块不能进入 `wouldPromoteByExistingGateBlocks`。
