...
- 第三个 exact-SHA CI [32263359545](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32263359545) 的 benchmark/static/Xcode 已成功，JUnit `9/10`；唯一失败是历史 v3.264 quad-warp runtime 编译 `MangaOCRService.swift` 时漏带 v3.281 provenance/layout 类型。该 runtime 已补入 `ImageOCRProvenance.swift` 与 `ImageOCRLayoutEngine.swift`，v3.264 合同锁定依赖顺序；warp 产品实现与 checksum 不变，仍待下一 SHA 的完整 UI/JUnit receipt。
