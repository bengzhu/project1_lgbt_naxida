...
- v3.34 正式要求选中 OCR block 的局部放大窗在关闭命令下提供命名明确的 44pt “修正识别文字”铅笔入口，并与结果行严格共享 `!isRunning && !isRenderingExport` 可用条件。入口只能将当前完整活动 block 交给既有 `beginCorrection`；helper 必须先拒绝忙碌或已失效 block，再登记现有结果行 `onDismiss` fallback 并呈现既有修正 sheet。禁止局部预览直接调用 Store correction、Vision OCR、翻译、renderer/export 或创建新的焦点／持久化状态；取消、放弃和无修改关闭仍沿 v3.33 在 sheet 完整关闭后回到结果行。`scripts/test-v334-image-focus-preview-correction-contract.py` 在 v3.33 后接入同一图片/UI fail-fast step，源码合同不能替代真机／模拟器的实际 VoiceOver 或紧凑布局回放。
