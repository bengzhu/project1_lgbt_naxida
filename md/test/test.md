...
Agent C 首轮与二轮视觉退回后，v1.88 contract 还必须锁定两项回归：compact-width 根 `VStack` 必须把 `floatingTabBarClearance` 放在 `ScrollView` 之后，从外部缩短滚动视口，不得退回内容尾部 padding 或 bottom `safeAreaInset`；真实 `PasteButton` 必须保留为交互层，并以透明前景加不接收触摸的实底中文 `Label("粘贴", systemImage: "doc.on.clipboard")` 覆盖系统 locale 标签。新 HEAD 的空输入、XXL 与 Accessibility 截图必须分别证明不再显示英文 “Paste”，且 Tab Bar 不再遮挡输入文字或翻译主按钮。
