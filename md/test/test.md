...
- v3.4 候选要求图片目标语言与输入语言使用相同的 task-scoped Retry 边界：已完成且 Store-owned source 存在时更新凭据并立即重译；失败或取消且 `canRetryImageTranslation` 时只更新下次 Retry 凭据；运行中拒绝改写；无 source 时不得重标旧内容。语言可用性只由 `selectTargetLanguage` / `canUseLanguage` 决定，不能用额外 `isProUnlocked` 阻断英语、中文等免费目标。`retryImageTranslation()` 必须优先使用内容凭据，cancel 保留、clear 重置。`scripts/test-v34-image-retry-language-contract.py` 与 v1.87 / v2.2-v3.1 图片合同在同一 fail-fast step 运行。本版不改 Vision OCR、layout、翻译实现、renderer、漫画探针、ground truth、metrics 或 output。
