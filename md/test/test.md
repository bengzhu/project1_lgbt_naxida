...
- v3.387 保留 v3.386 的所有候选，并追加完全贴合已验证 `把以下翻译成中文：` 模板的无 context 裸 prompt；漫画批译仍要求 `[N]` 标签解析，结果继续经过严格逐块 QA。
- 新增纯静态合同 `scripts/test-v3387-japanese-bare-prompt-contract.py`，工程版本 `3.387`，CI 已接入；下一次云端运行还会保存基于真实终态会话重开的整图 OCR 原文框截图、坐标清单和逐块 JSON，方便逐块对照 `test/2.png`，不合成质量证据。

### v3.386 普通图片 Local GGUF 日语中文任务句式 fallback（已验证失败，继续前进）

- v3.385 的精确 SHA `2e28617ed47fb1fc2a48d75939f0a17779b9f7f4` 云端运行 [33391703333](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33391703333) 已完成真实 `test/2.png` OCR（17 个非空日语块），但 270M 模型对英文极简候选仍回显原文/说明，翻译完整性为 `0/17`；证据仅作失败诊断。
- v3.386 在保留上下文感知与英文极简尝试后，追加与已验证英文→中文模板同形的无 context 中文任务句式（日语→简体中文/英文），但精确合入 SHA `a20b993cb437ac7efd861fe69dd7fbb817096f8d` 的云端运行 [33393722346](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33393722346) 仍为 `0/17`，故继续前进。
- 新增纯静态合同 `scripts/test-v3386-japanese-chinese-prompt-contract.py`，工程版本当时为 `3.386`，CI 已接入；不合成截图或质量证据。

### v3.385 普通图片 Local GGUF 日语最小 fallback（已验证失败，继续前进）

- v3.384 的真实 `test/2.png` 运行已完成 OCR，得到 17 个非空日语块，但 270M 模型对带 compact context 的双语 prompt 仍产生提示词回显，翻译完整性为 `0/17`；该 run 只作失败诊断，不作为翻译完成证据。
- v3.385 保留上下文感知尝试，并为日语 standard 与漫画 `[N]` 批译增加无 context 的英文极简候选；精确 SHA `2e28617ed47fb1fc2a48d75939f0a17779b9f7f4` 的云端运行 [33391703333](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33391703333) 仍为 `0/17`，故继续前进。

### v3.384 普通图片 Local GGUF 日语 prompt 形状修复（已验证失败，继续前进）

- v3.383 的 compact context 已避免上下文超限；v3.384 将日语 standard 与漫画批译改成短双语任务指令，明确 `Text to translate`/`待翻译文本` 输入标记，并省略冗余通用提示。
- 精确 SHA `02e25095d4665beffb1fc4bc841ec233e40bf83e` 的云端 `test/2.png` 运行 [33389855026](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33389855026) 已完成 OCR（17 个非空日语块）并保存界面截图，但翻译完整性仍失败：`translatedBlockCount=0/17`；批量两次分别返回翻译拒答与 context/输入说明回显，单块 fallback 也回显 prompt。因此该截图只作失败诊断，不作为日语翻译完成证据。
- 新增纯静态合同 `scripts/test-v3384-japanese-prompt-shape-contract.py`，工程版本当时为 `3.384`，CI 已接入；不合成截图或把静态合同外推为通用 OCR/CER/翻译质量证据。

### v3.383 普通图片 Local GGUF standard fallback prompt 修复（已验证失败，继续前进）

- v3.382 的批量 prompt 已不再超出 context，但 270M 模型回显整批输入；v3.383 让所有日语 standard 单块回退也使用有限只读 compact context，完整 context 仍只供 QA，漫画批量 256-token 上限、OCR/layout、逐块 QA/回退、取消、持久化与非图片路径不变。
- 精确 SHA `08a605303ae098be0c077ad74c809cb6f17970a2` 的云端 `test/2.png` 运行 [33378259762](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33378259762) 已完成 OCR（17 个非空日语块）并保存界面截图，但翻译完整性失败：`translatedBlockCount=0/17`；probe 中批量两次分别回显原文与提示词，单块 fallback 在第 4 块触发 `outputTooLong`。因此该截图只作失败诊断，不作为日语翻译完成证据。
- 新增纯静态合同 `scripts/test-v3383-japanese-standard-compact-context-contract.py`，工程版本当时为 `3.383`，CI 已接入；不合成样图或把静态合同外推为通用质量证据。
...
- 严格漫画 `[N]` 批译、placeholder/source leakage、目标语言/数字/术语/长度 QA、OCR/layout、预算、逐块回退、取消、持久化与非日语路径不变。新增纯静态合同 `scripts/test-v3379-translation-standalone-english-label-contract.py`，工程版本 `3.379`，CI 已接入。
- 本地安全回归 `378/378` 通过、27 个进程/编译/runtime 入口按约束跳过（405 总计），Python AST `405/405`、JSON `144/144`、YAML `3/3`、shell `32/32`、plist `4/4` 与 `git diff --check` 通过；实现 SHA `7c6d5cf952e74f641f6ee9c439545fb158982601` 的 exact-SHA full [33367420463](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33367420463)、PR [#443](https://github.com/bengzhu/project1_lgbt_naxida/pull/443) checks [33368408182](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33368408182)、merge SHA `6454b3179a2706287037db57dc092ac20092627e` 与合入后 push CI [33368493788](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33368493788) 均成功；候选 Xcode 26.6/JUnit `10/10`（0 failures）与 Japanese benchmark 通过，发布 `AITRANS CI/full-validation=success` receipt。
- 按约束未运行本地 Xcode、Swift/Core ML、App runtime、Rust/Cargo 或 GGUF；`test/3.png` 未提供，不合成样图或质量证据，Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。

### v3.376 translation metadata content boundary（已完成）

新增纯静态合同 `scripts/test-v3376-translation-metadata-content-boundary-contract.py`：验证 `translation engine`、`输出风格` 在合法译文开头带自然 payload 时保留，只有 marker 独立成行或后面紧跟标点才按 prompt metadata 清理；明确指令 marker、表格、标签、逐块 QA、OCR、geometry/layout、预算、取消、持久化和非日语路径不变。工程版本 `3.376`，CI 已接入；本地安全合同 `375/375` 通过、27 个进程/编译/runtime 入口跳过（`402` 总计），Python AST `402/402`、JSON `144/144`、YAML `3/3`、shell `32/32`、plist `4/4` 与 `git diff --check` 通过；实现 SHA `418f86e9e502bb57e831c77eb34380ff0f97fd60` 的 exact-SHA full [33359531013](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33359531013)、PR [#440](https://github.com/bengzhu/project1_lgbt_naxida/pull/440) checks [33360556432](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33360556432)、merge SHA `6d5422e0bc223b4a22a3000374be4b0810f647f8` 与合入后 push CI [33360620974](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33360620974) 均成功。`test/3.png` 未提供，不合成样图或质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
...
新增纯静态合同 `scripts/test-v3373-translation-term-width-qa-contract.py`：验证已确认术语的源文与译文匹配先做 Unicode 规范化、大小写与全角/半角宽度折叠，再做保留标点的包含判断；产品与 cloud shadow evaluator 同步，revoked 术语继续 fail-closed。OCR、geometry/layout、预算、标签、逐块回退、取消、持久化和非日语路径不变。工程版本 `3.373`，CI 已接入；本地安全回归 `372/372` 通过、27 个进程/编译/runtime 入口按约束跳过（`399` 总计），Python AST `399/399`、JSON `144/144`、YAML `3/3`、shell `32/32`、plist `4/4` 与 diff 检查通过；exact full [33353855302](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33353855302)、PR [#437](https://github.com/bengzhu/project1_lgbt_naxida/pull/437) checks [33354891526](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33354891526)、merge SHA `b69d2995137ac52d7ade3b003114fc080cadaf5e` 与合入后 push [33354973808](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33354973808) 均成功。`test/3.png` 未提供，不合成样图或质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
...
- 新增共享 `validJapaneseDirectionConfidence`，让 `needsJapaneseWeakBlockRecovery`、`isJapaneseVerticalBlockAtRisk` 与 vertical crop 的 direction tie-break 共同拒绝 `NaN/±∞` 和有限越界值；合法 `[0,1]`、`.45` 风险门、最多 16 个 vertical block crop、8 次方向 fallback、4 个 weak-block scoped reread、pixel/line/tile/frontier、owner、OCR/layout、翻译 QA、取消、持久化与非日语路径不变。新增 `scripts/test-v3352-japanese-direction-confidence-risk-domain-contract.py`，工程版本 `3.352`；实现 SHA `321e52831b4ca9d202ae897276316c980f541fe7` 的 exact-SHA full [33294222837](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33294222837)、PR [#416](https://github.com/bengzhu/project1_lgbt_naxida/pull/416) checks [33294632461](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33294632461)、merge SHA `180f47f50cfa483a5796801d6247ccebef4ff493` 与合入后 push CI [33294679936](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33294679936) 均成功；新合同 `10/10`，本地 `351` 个安全合同通过，`27` 个进程／编译／runtime 合同跳过，`378` 个合同总计无失败；Python AST `378/378`、JSON `237/237`（排除 2 个已知 JSONC）、YAML `3/3`、shell `32/32`、plist `4/4` 与 `git diff --check` 全部通过。未运行本地 Xcode、Swift、Core ML、Rust/Cargo、GGUF 或 App runtime。Koharu/GGUF、授权语料和目标设备证据仍独立于普通 OCR 主路径，不把本轮结果外推为通用 OCR/CER 或翻译质量提升。

v3.352 文档收口 push CI [33294843498](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33294843498) 已成功；该 CI 只验证文档收口后的当前 `smalldata_test` 状态，不新增编译或模型证据。

### v3.351 Japanese pixel recovery block eligibility contract（已完成）

- `detectJapanesePixelFirstVerticalRegions` 不再让全部 `verticalBlocks` 仅凭几何阻止 pixel-first candidate；只有 vertical、`directionConfidence` 有限且位于 `[0,1]`、`>=.45`，并通过既有非空、OCR confidence `>=.48`、真实日语文字、日语文字密度与脚本文本密度 `>=.5` 的 block 才声明 geometry 已覆盖。弱/空/非日语/低密度/方向不确定 block 继续保留既有 pixel-first 机会；可靠 block 仍按既有 `japanesePixelDetectorRegionIsCovered` 抑制重复 geometry。最多 12 个 pixel-first crop、4 次方向 fallback、line frontier、tile/block recovery、OCR/layout、翻译 QA、取消、持久化与非日语路径不变。新增 `scripts/test-v3351-japanese-pixel-recovery-block-eligibility-contract.py`，工程版本 `3.351`；实现 SHA `09cd27d2931ed5aa5f79a0325beac06d9e106975` 的 exact-SHA full [33293068254](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33293068254)、PR [#415](https://github.com/bengzhu/project1_lgbt_naxida/pull/415) checks [33293551364](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33293551364)、merge SHA `db302ec1bf281533d01e31e3bc1b919cc95b6a57` 与合入后 push CI [33293585668](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33293585668) 均成功；新合同 `10/10`，本地 `350` 个安全合同通过，`27` 个进程／编译／runtime 合同跳过，`377` 个合同总计无失败；Python AST `377/377`、JSON `237/237`（排除 2 个已知 JSONC）、YAML `3/3`、shell `32/32`、plist `4/4` 与 `git diff --check` 全部通过。未运行本地 Xcode、Swift、Core ML、Rust/Cargo、GGUF 或 App runtime。Koharu/GGUF、授权语料和目标设备证据仍独立于普通 OCR 主路径，不把本轮结果外推为通用 OCR/CER 或翻译质量提升。
...
- 新合同：`scripts/test-v3346-japanese-pixel-first-spatial-balance-contract.py`；工程版本推进至 `3.346`，Japanese benchmark route 已接入。本地 345 个安全合同通过，27 个进程／编译／runtime 合同按约束跳过；Python AST `372/372`、JSON `237/237`（排除 2 个已知 JSONC）、YAML `3/3`、shell `32/32`、plist `4/4` 与 `git diff --check` 通过。实现 SHA `48f3c0a87f1ec9a76cd7a8f0b70177180137b9dd` 的 exact-SHA full [33286852343](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33286852343)、PR [#410](https://github.com/bengzhu/project1_lgbt_naxida/pull/410) checks [33287447031](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33287447031)、merge SHA `1273d17a0529919fd33930f767ed4b12ea40c213`、合入后 push CI [33287488329](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33287488329) 与文档收口 push CI [33287597848](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33287597848) 均成功。本地不运行 Xcode、Swift、Core ML、Rust/Cargo、GGUF 或 App runtime。
...
- 新合同：`scripts/test-v3345-japanese-vertical-crop-spatial-balance-contract.py`；工程版本推进至 `3.345`，Japanese benchmark route 已接入。本地 344 个安全合同通过，27 个进程／编译／runtime 合同按约束跳过；Python AST `371/371`、JSON `237/237`（排除 2 个已知 JSONC）、YAML `3/3`、shell `32/32`、plist `4/4` 与 `git diff --check` 通过。实现 SHA `04cd7ee3b02f0b8af5bf8fabb80dbf4c6c2bc24e` 的 exact-SHA full [33285792124](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33285792124)、PR [#409](https://github.com/bengzhu/project1_lgbt_naxida/pull/409) checks [33286241499](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33286241499)、merge SHA `e35bf081a54c3fcefd2a6183ac80982568b5cfd0`、合入后 push CI [33286277864](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33286277864)、文档收口 push CI [33286335887](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33286335887) 均成功。本地未运行 Xcode、Swift、Core ML、Rust/Cargo、GGUF 或 App runtime。
...
- 新合同：`scripts/test-v3343-japanese-vision-line-owner-balance-contract.py`；工程版本推进至 `3.343`，Japanese benchmark route 已接入。本地 342 个安全合同通过，27 个进程／编译／runtime 合同按约束跳过；Python AST `369/369`、JSON `237/237`（排除 2 个已知 JSONC）、YAML `3/3`、shell `32/32`、plist `4/4` 与 `git diff --check` 通过。实现 SHA `cffa4c8db09a2d08a171f8783824a2a47f7566dd` 的 exact-SHA full [33283434267](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33283434267)、PR [#407](https://github.com/bengzhu/project1_lgbt_naxida/pull/407) checks [33283968741](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33283968741)、merge SHA `3525129214c8460dc677d1c3ffd58a74503acbd3`、合入后 push CI [33284018210](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33284018210) 与文档收口 push CI [33284101512](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33284101512) 均成功。本地未运行 Xcode、Swift、Core ML、Rust/Cargo、GGUF 或 App runtime。
...
- 新合同：`scripts/test-v3342-japanese-line-owner-balance-contract.py`；工程版本 `3.342`，Japanese benchmark route 已接入。本地 341 个安全合同通过，27 个进程／编译／runtime 合同按约束跳过；Python AST `368/368`、workflow YAML `3/3`、shell `32/32`、plist `4/4`、JSON `237/237`（排除 2 个已知 JSONC）与 `git diff --check` 通过。本地不运行 Xcode、Swift、Core ML、Rust/Cargo、GGUF 或 App runtime。实现 SHA `a6fdf8f16a85270b933ffeefc676bd36847a999b` 的 exact-SHA full [33282218767](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33282218767)、PR [#406](https://github.com/bengzhu/project1_lgbt_naxida/pull/406) checks [33282657014](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33282657014)、merge SHA `cdc97bb5a67b4f8e4499ab212bd2ed00f1204019`、合入后 push CI [33282708733](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33282708733) 与文档收口 push CI [33282792889](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33282792889) 均成功。
...
- 新合同：`scripts/test-v3339-image-japanese-detector-slice-merge-closure-contract.py`；工程版本推进至 `3.339`，Japanese benchmark route 接入。本地 338 个安全 Python 合同通过，27 个含实际进程/编译/runtime 入口合同按约束跳过；Python AST `365/365`、workflow YAML `3/3`、shell `31/31`、plist `4/4`、JSON `237/237`（排除 2 个已知 JSONC）与 `git diff --check` 全部通过。本地不运行 Xcode、Swift、Core ML、Rust/Cargo、GGUF 或 App runtime。实现 SHA `a37e458c09bd87934cc7bfc1d58900d65b5d4deb` 的 exact-SHA full [33152417310](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33152417310)、PR [#403](https://github.com/bengzhu/project1_lgbt_naxida/pull/403) checks [33153446720](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33153446720)、merge SHA `7679d30bc28c2a07fb01d71ef3f2feff24b8e661`、合入后 push CI [33153513443](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33153513443) 与最终文档 receipt push CI [33153720226](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33153720226) 均成功。Koharu parity 按普通 OCR 独立策略跳过；本轮只证明跨切片重复 region 的闭包防护，不外推为通用 OCR/CER、翻译盲评、真实 Koharu/GGUF parity、授权语料、目标设备或 holdout 质量证据。
...
- 新合同：`scripts/test-v3332-ocr-confidence-domain-contract.py`；工程版本为 `3.332`，Japanese benchmark route 已接入。新合同 `10/10`、331 个无进程合同（1,761 tests）、Python AST `378/378`、JSON `144/144`、workflow YAML `3/3`、shell `32/32`、plist/project `5/5` 与 `git diff --check` 通过，27 个实际导入 `subprocess` 的合同按约束跳过。实现 SHA `6b10be2e9305c45163e2e75776963def401d5136` 的 exact-SHA full [32648953240](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32648953240) 成功，benchmark job `97217459621`、主 job `97217499411`、Xcode `26.6 (17F113)`、simulator、manga probe、manifest 与 JUnit `10/10` 通过；PR [#396](https://github.com/bengzhu/project1_lgbt_naxida/pull/396) CI [32650444711](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32650444711) 成功，以 merge SHA `ab9c9115e721b96b67b8d25394e701c3bb9eda48` 合入 `smalldata_test`，合入后 push CI [32650526586](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32650526586) 成功复用候选 full receipt。未运行本地 Xcode/Swift/Core ML/Rust/GGUF/App runtime；Koharu/GGUF/授权语料/目标设备继续只作可选研究或质量证据。
...
- 新合同：`scripts/test-v3330-japanese-best-observation-reducer-contract.py`；工程版本为 `3.330`，Japanese benchmark route 已接入。新合同 `10/10`、329 个无进程合同（1,741 tests）、Python AST `376/376`、JSON `144/144`、workflow YAML `3/3`、shell `32/32`、plist/project `5/5` 与 `git diff --check` 通过；27 个实际导入 `subprocess` 的合同跳过。历史 v3.173/v3.327 合同已改为验证 reducer，不再固化旧 `.max(by:)` bug。实现 SHA `7636d441e3246f2fa6bc3afe6de302d6bcda47ff` 的 exact-SHA full [32642708201](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32642708201) 成功，benchmark job `97202155198` 与主 job `97202194469` 覆盖 benchmark schema/fixture、静态、Speech、UI/Home/Paste、云端 Xcode `26.6 (17F113)`、manifest、JUnit `10/10` 和 full receipt；commit status 为 `AITRANS CI/full-validation=success`，Koharu/GGUF 按非必需策略跳过。PR [#394](https://github.com/bengzhu/project1_lgbt_naxida/pull/394) CI [32643730705](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32643730705) 的 benchmark job `97204653726` 与主 job `97204697871` 成功，并以 merge SHA `3c9482b8dfdf806a67de26fce4dd075518bfc95f` 合入 `smalldata_test`；合入后 push CI [32643803792](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32643803792) 的 benchmark job `97204835594`、主 job `97204882888`、云端 Xcode、JUnit `10/10` 与 full receipt 全部成功。未运行本地 Xcode/Swift/Core ML/Rust/GGUF/App runtime；`main` 未修改，Koharu/GGUF/授权语料/目标设备继续只作可选研究或质量证据。
...
- 新合同：`scripts/test-v3329-japanese-duplicate-fusion-quality-contract.py`；工程版本为 `3.329`，Japanese benchmark route 已接入。新合同 `10/10`、328 个无外部进程/编译入口合同（1,731 tests）、Python AST `375/375`、JSON `144/144`、workflow YAML `3/3`、shell `32/32`、plist/project `5/5` 与 `git diff --check` 通过；27 个 `subprocess` 合同按约束跳过。实现 SHA `c8f4146e2f3cdf9cc6e732eb3ea684e356bba8bc` 的 exact-SHA full [32639974398](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32639974398) 成功，benchmark job `97195459040`、主 job `97195508891`、静态/Speech/UI/Home/Paste、云端 Xcode、JUnit `10/10` 与 receipt 全部通过，Koharu/GGUF 按非必需策略跳过。PR [#393](https://github.com/bengzhu/project1_lgbt_naxida/pull/393) CI [32640600464](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32640600464) 成功，并以 merge SHA `30cd90bb471aa84f4c1e19cc8097b0f900b4dd29` 合入 `smalldata_test`；合入后 push CI [32640680395](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32640680395) 的 benchmark job `97197185384`、主 job `97197239755`、云端 Xcode、JUnit `10/10` 与 full receipt 全部成功。未运行本地 Xcode/Swift/Core ML/Rust/GGUF/App runtime；`main` 未修改。请求预算、crop/warp、geometry/layout、翻译 QA、取消、generation、持久化和非日语路径不变；Koharu/GGUF/授权语料/目标设备继续只作可选研究或质量证据。
...
- 本地新合同 `10/10`、324 个无进程合同（1,691 tests）、Python AST `371/371`、JSON `144/144`、workflow YAML `3/3`、shell `32/32`、plist/project `5/5` 与 `git diff --check` 通过；27 个编译/runtime 合同跳过。实现 SHA `554b65c4ee2f6ab135a5348beb3eb31f2cc721ff` 的 PR [#389](https://github.com/bengzhu/project1_lgbt_naxida/pull/389) CI [32633537678](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32633537678) 成功；exact-SHA full [32633561784](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32633561784) 的 benchmark job `97179853166` 与主 job `97179887362` 成功，覆盖静态、Speech、UI/Home/Paste、云端 Xcode、manifest、JUnit `10/10` 与 full receipt，Koharu/GGUF 按非必需策略跳过。PR 以 merge SHA `305bc199554a33c9a709146da4ad998cbc67b1df` 合入 `smalldata_test`；合入后 push CI [32634107689](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/32634107689) 的 v3.325 benchmark、静态与复用 full receipt 成功。未运行本地 Xcode/Swift/Core ML/Rust/GGUF/App runtime；`main` 未修改。
...
### v3.379 translation standalone English label boundary（待云端验收）

新增纯静态合同 `scripts/test-v3379-translation-standalone-english-label-contract.py`：验证普通非 tagged 翻译输出中的 `Here is the translation`、`Here are the translations` 等有限英文标签独立成行且仅带边缘空白/标点时只移除该 metadata 行，保留后续译文；带自然语言正文的同名句、同一行标签恢复、严格漫画 `[N]` 批译清洗与共享 placeholder/source leakage/目标语言/数字/术语/长度 QA、OCR/layout、预算、逐块回退、取消、持久化和非日语路径边界保持不变。工程版本 `3.379`，CI 已接入；本轮本地安全回归与 exact-SHA full 待完成，`test/3.png` 未提供，不合成样图或质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。

### v3.378 translation leading label boundary（已完成）

新增纯静态合同 `scripts/test-v3378-translation-leading-label-boundary-contract.py`：验证普通非 tagged 翻译输出在行首出现有限标签并紧跟明确标点和正文时只剥离标签、保留正文；无分隔标签、嵌入正文、既有 `翻译是：` 歧义内容、严格漫画 `[N]` 批译清洗与共享 placeholder/source leakage/目标语言/数字/术语/长度 QA 边界保持不变。工程版本 `3.378`，CI 已接入；本轮本地安全回归 377/377 通过、27 个进程/编译/runtime 入口按约束跳过（404 总计），Python AST 404/404、tracked JSON 144/144、workflow YAML 3/3、shell 32/32、plist 4/4 与 git diff --check 通过；实现 SHA `dc6fd5d1c032983fb692bd288295d27023d4a378` 的 exact-SHA full [33365044114](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33365044114)、PR [#442](https://github.com/bengzhu/project1_lgbt_naxida/pull/442) checks [33366094900](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33366094900)、merge SHA `763b27c1f9d02f1e1358803e92cd297556764f8f` 与合入后 push CI [33366187576](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33366187576) 均成功，发布 `AITRANS CI/full-validation=success` receipt。按约束不运行本地 Xcode、Swift、Core ML、App runtime、Rust/Cargo 或 GGUF；`test/3.png` 未提供，不合成样图或质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
