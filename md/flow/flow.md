...
当前 v3.374 translation batch preamble recovery（进行中）：`known standalone preamble -> strip only before first [N] -> existing strict tag parser/order/QA`；模型先回显已知翻译标签时不再整批退回逐块路径，未知前导文本仍被首标签门拒绝。OCR、geometry/layout、预算、标签语义、取消、持久化和非日语路径不变。新增 `scripts/test-v3374-translation-batch-preamble-contract.py`，工程版本 `3.374`，workflow 已接入；cloud full/merge receipt 待执行。`test/3.png` 未提供，不伪造质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
