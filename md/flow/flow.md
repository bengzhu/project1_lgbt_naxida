...
当前 v3.362 Japanese translation context-echo boundary（候选验证中）：`previous completed batch summary -> exact normalized output echo + different source gate -> block-only QA retry`；只有完整复现上一批译文且当前原文不同才判跨 batch 泄漏，合法嵌入短语与重复同一原文保留，OCR/layout、预算、标签、取消、持久化和非日语路径不变。新增 `scripts/test-v3362-japanese-translation-context-echo-contract.py`，工程版本 `3.362`；未提供 `test/3.png`，不伪造翻译质量证据。
