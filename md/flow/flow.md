...
大版本合入后的首次使用体验闸门见 [`experience-iteration.md`](experience-iteration.md)，当前短状态见根目录 [`experience_state.md`](../../experience_state.md)；该流程只管理体验验证、日志、截图和 latest build，不改变产品主路径。
...
当前 v3.390 OCR detection workspace（待云端验收）：`OCR 检测` 是独立的 `AppTab` 和 Store 状态树，`图片/相册/拍照/粘贴 -> VisionOCRService -> [ImageTranslationBlock] -> 原图 bbox overlay + 结果复查/导出`，不进入图片翻译或 LLM。语言与版式分别选择；自动/日语在竖排版式下保留现有 90°/270° 方向复读。页面首版包括点击高亮、低置信筛选、单块复读、手动编辑、复制全部、TXT/JSON、取消/重试和准备图片→检测文字区域→OCR→整理阅读顺序阶段；diagnostics 默认折叠并显示分辨率、引擎/模型、阶段耗时、块/字符速度、模型原始分数和通过/需复核状态。test2 CI capture 继续使用真实终态 session，并额外复制 `test2-ocr-full-overlay-v3388.png` 别名；截图仅为 UI/geometry 证据，不外推通用质量结论。
...
当前 v3.388 Local GGUF Japanese raw-completion fallback boundary（待云端验收）：普通图片 `test/2.png` 仍从 ordinary image OCR→日语翻译入口进入；v3.387 真实 trace 已得到 17 个非空日语 OCR 块，但 270M 模型对聊天模板候选反复回显，最终为 `0/17`。v3.388 在标准日语→简体中文/英文路径最前面增加不经过 chat template 的窄范围原始补全文本，成功后仍进入同一清洗、源文泄漏/目标语言/数字/术语/长度与逐块 QA，失败继续回到既有聊天候选。漫画批译 `[N]` 标签、OCR、批量预算、取消、持久化和非图片路径不变；新增 `scripts/test-v3388-japanese-raw-completion-contract.py`，工程版本 `3.388`，云端重跑待完成。

当前 v3.387 Local GGUF Japanese bare-prompt fallback boundary（待云端验收）：普通图片 `test/2.png` 仍从 ordinary image OCR→日语翻译入口进入；v3.386 真实运行已得到 17 个非空日语 OCR 块，但 270M 模型虽识别中文任务句式，仍回显说明文字和日文原文。v3.387 在保留此前候选后，追加完全贴合已验证 `把以下翻译成中文：` 模板的无 context 裸提示；漫画批译同步保留 `[N]` 标签顺序约束。批量生成上限仍为 256 token，为 `LlamaRuntime` 的 1,024-token context 留出空间；所有结果继续进入严格逐块 QA。新增 `scripts/test-v3387-japanese-bare-prompt-contract.py`，工程版本 `3.387`，workflow 已接入；云端入口会保存真实逐块译文、LLM trace、结果列表截图，以及基于真实终态会话重开的整图 OCR 原文框截图与坐标清单，云端重跑待完成。完整 context 仍只供 QA，sampler state、v3.380 的 exact shared-Han QA、逐块质量门、取消/持久化边界不变；不合成质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。

当前 v3.379 translation standalone-English-label boundary（已完成）：`plain translation -> exact standalone English label line -> remove label line only -> preserve content lines -> shared cleanup/QA`；带正文的 `Here is the translation ...` 等自然句、严格漫画 `[N]` 标签、placeholder/source leakage/目标语言/数字/术语/长度 QA、OCR/layout、预算、逐块回退、取消、持久化和非日语路径不变。新增 `scripts/test-v3379-translation-standalone-english-label-contract.py`，工程版本 `3.379`，workflow 已接入；本地安全回归 `378/378` 通过、27 个进程/编译/runtime 入口跳过（405 总计），Python AST `405/405`、JSON `144/144`、YAML `3/3`、shell `32/32`、plist `4/4` 与 diff 检查通过；实现 SHA `7c6d5cf952e74f641f6ee9c439545fb158982601` 的 exact full [33367420463](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33367420463)、PR [#443](https://github.com/bengzhu/project1_lgbt_naxida/pull/443) checks [33368408182](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33368408182)、merge SHA `6454b3179a2706287037db57dc092ac20092627e` 与合入后 push [33368493788](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33368493788) 均成功，发布 `AITRANS CI/full-validation=success` receipt。候选 Xcode 26.6/JUnit `10/10` 与 Japanese benchmark 通过，UI evidence/probe 按参数跳过。`test/3.png` 未提供，不合成质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。

当前 v3.378 translation leading-label boundary（已完成）：`plain translation -> finite leading label + explicit delimiter + payload -> strip label only -> shared cleanup/QA`；无分隔标签、嵌入正文、既有 `翻译是：` 歧义内容、严格漫画 `[N]` 标签、OCR、geometry/layout、预算、逐块回退、取消、持久化和非日语路径不变。新增 `scripts/test-v3378-translation-leading-label-boundary-contract.py`，工程版本 `3.378`，workflow 已接入；本地安全回归 377/377、Python AST 404/404、tracked JSON 144/144、workflow YAML 3/3、shell 32/32、plist 4/4 与 diff 检查通过；实现 SHA `dc6fd5d1c032983fb692bd288295d27023d4a378` 的 exact full [33365044114](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33365044114)、PR [#442](https://github.com/bengzhu/project1_lgbt_naxida/pull/442) checks [33366094900](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33366094900)、merge SHA `763b27c1f9d02f1e1358803e92cd297556764f8f` 与合入后 push [33366187576](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33366187576) 均成功，发布 `AITRANS CI/full-validation=success` receipt。`test/3.png` 未提供，不合成样图或质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。

当前 v3.377 translation batch inline preamble（已完成）：`known preamble + same-line first [N] tag -> strip prefix only -> preserve tag/payload -> existing tag/QA/fallback`；未知前缀保持首标签 fail-closed，OCR、geometry/layout、预算、逐块回退、取消、持久化和非日语路径不变。新增 `scripts/test-v3377-translation-batch-inline-preamble-contract.py`，工程版本 `3.377`，workflow 已接入；本地安全回归 `376/376`、Python AST `403/403`、tracked JSON `144/144`、YAML `3/3`、shell `31/31`、plist `4/4` 与 diff 检查通过；最终实现 SHA `9ae1205f3123203fc91c7075b635bcb2c75379f1` 的 exact full [33362823753](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33362823753)、PR [#441](https://github.com/bengzhu/project1_lgbt_naxida/pull/441) checks [33363194227](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33363194227)、merge SHA `8b63ff223b260303e9e3efb24fc779a6fb00d5c6` 与合入后 push [33363279607](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33363279607) 均成功，发布 `AITRANS CI/full-validation=success` receipt。`test/3.png` 未提供，不伪造质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。

当前 v3.376 translation metadata content boundary（已完成）：`leading content-sensitive marker + natural payload -> preserve translated content; standalone/punctuation-delimited marker -> remove prompt metadata -> existing validation/QA`；`translation engine`、`输出风格` 只有独立或标点分隔的行首形态才清理，明确指令 marker、表格、标签、OCR/layout、预算、逐块回退、取消、持久化和非日语路径不变。新增 `scripts/test-v3376-translation-metadata-content-boundary-contract.py`，工程版本 `3.376`，workflow 已接入；本地安全合同 `375/375` 通过、27 个进程/编译/runtime 入口跳过（`402` 总计），Python AST `402/402`、JSON `144/144`、YAML `3/3`、shell `32/32`、plist `4/4` 与 `git diff --check` 通过；exact-SHA full [33359531013](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33359531013)、PR [#440](https://github.com/bengzhu/project1_lgbt_naxida/pull/440) checks [33360556432](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33360556432)、merge SHA `6d5422e0bc223b4a22a3000374be4b0810f647f8` 与合入后 push [33360620974](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33360620974) 均成功，发布 `AITRANS CI/full-validation=success` receipt。`test/3.png` 未提供，不伪造质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。

上一项 v3.375 translation refusal-marker boundary（已完成）：`embedded refusal phrase -> preserve translated content; leading refusal/apology + marker -> fail closed -> existing shared QA`；共享产品 policy 与 cloud shadow evaluator 只在响应开头或有限道歉前缀后识别拒答，正文引用“无法翻译/翻译失败”等短语不再被误判。OCR、geometry/layout、预算、标签、逐块回退、取消、持久化和非日语路径不变。新增 `scripts/test-v3375-translation-refusal-marker-boundary-contract.py`，工程版本 `3.375`，workflow 已接入；本地安全合同 `374/374` 通过、27 个进程/编译/runtime 入口跳过（`401` 总计），Python AST `401/401`、JSON `144/144`、YAML `3/3`、shell `32/32`、plist `4/4` 与 `git diff --check` 通过；exact-SHA full [33357611551](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33357611551)、PR [#439](https://github.com/bengzhu/project1_lgbt_naxida/pull/439) checks [33358377260](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33358377260)、merge SHA `fb32332d19860224fb13310c07f75db316391053` 与合入后 push [33358439503](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33358439503) 均成功，发布 `AITRANS CI/full-validation=success` receipt。`test/3.png` 未提供，不伪造质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。

当前 v3.374 translation batch preamble recovery（已完成）：`known standalone preamble -> strip only before first [N] -> existing strict tag parser/order/QA`；模型先回显已知翻译标签时不再整批退回逐块路径，未知前导文本仍被首标签门拒绝。OCR、geometry/layout、预算、标签语义、取消、持久化和非日语路径不变。新增 `scripts/test-v3374-translation-batch-preamble-contract.py`，工程版本 `3.374`，workflow 已接入；本地安全合同 `373/373` 通过、27 个进程/编译/runtime 入口跳过（`400` 总计），Python AST `400/400`、JSON `144/144`、YAML `3/3`、shell `32/32`、plist `4/4` 与 diff 检查通过。实现 SHA `16425ddbaab9597958fbe350fd0e361a0553c948` 的 exact full [33355934806](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33355934806)、PR [#438](https://github.com/bengzhu/project1_lgbt_naxida/pull/438) checks、merge SHA `5e54836e22fca7574ba8685cf77f86f743a09394` 与合入后 push [33356807008](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33356807008) 均成功，发布 `AITRANS CI/full-validation=success` receipt。`test/3.png` 未提供，不伪造质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。

当前 v3.373 translation term width QA（已完成）：`confirmed term source/output -> Unicode canonical + case/width folding -> exact containment -> existing block QA`；全角/半角与规范化等价的术语不再被误判为缺失，标点仍保持语义边界，revoked 术语继续 fail-closed。OCR、geometry/layout、预算、标签、逐块回退、取消、持久化和非日语路径不变。新增 `scripts/test-v3373-translation-term-width-qa-contract.py`，工程版本 `3.373`，workflow 已接入；本地安全回归 `372/372`、Python AST `399/399`、tracked JSON `144/144`、workflow YAML `3/3`、shell `32/32`、plist `4/4` 与 diff 检查通过。实现 SHA `be7f33ec6892a0b5958d586f920bbf27be3d9e4d` 的 exact full [33353855302](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33353855302)、PR [#437](https://github.com/bengzhu/project1_lgbt_naxida/pull/437) checks [33354891526](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33354891526)、merge SHA `b69d2995137ac52d7ade3b003114fc080cadaf5e` 与合入后 push [33354973808](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33354973808) 均成功，发布 `AITRANS CI/full-validation=success` receipt。`test/3.png` 未提供，不伪造质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。

当前 v3.372 Vision vertical line single-rotation（已完成）：`vertical quad -> bounded direct projection (unrotated) -> shared Japanese crop preparation -> one caller-selected rotation -> Vision line OCR -> existing observation fusion/translation QA`。Core Image 兼容投影保持同一未旋转输出边界；detector-owned Manga OCR quad path 仍自行单次 `rotate270`。8 次 line OCR、2 个 geometry-only 名额、16M perspective budget、owner/quality/layout、取消、持久化和非日语路径不变。新增 `scripts/test-v3372-image-japanese-line-single-rotation-contract.py`，工程版本 `3.372`，workflow 已接入；本地安全回归 `371/371` 通过、27 个进程/编译/runtime 入口跳过（`398` 总计），Python AST `398/398`、JSON `144/144`、YAML `3/3`、shell `32/32`、plist `4/4` 与 diff 检查通过；exact-SHA full [33351843254](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33351843254)、PR checks [33352464164](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33352464164)、merge SHA 4e02d6337f8516c8df1cd20a62ac7cdca049704c、合入后 push [33352543308](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33352543308) 与 AITRANS CI/full-validation=success 均通过。`test/3.png` 未提供，不伪造质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。

当前 v3.371 Translation leading-prompt recovery（已完成）：`raw model output -> hard turn-token cut -> remove leading prompt lines / retain inline Output only payload -> cut prompt suffix after real content -> shared validation/QA`。连续的开头提示行不再连同后续译文一起截掉；已有译文后的提示回显仍为后缀边界，嵌入正文的同名短语仍保留。新增 `scripts/test-v3371-translation-leading-prompt-recovery-contract.py`，工程版本 `3.371`，workflow 已接入；本地安全合同 `370/370` 通过、27 个进程/编译/runtime 入口跳过，Python AST `416/416`、JSON `144/144`、YAML `3/3`、shell `32/32`、plist `4/4` 与 diff 检查通过。实现 SHA `1b28159857ff7a75740ec8a5aecff56b77f4b7e6` 的 exact full [33346717836](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33346717836)、PR checks [33346679209](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33346679209)、merge SHA `f1f606d36abc8fb12845c08626ff8e9da49c4d1b`、合入后 push [33347337847](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33347337847) 与 `AITRANS CI/full-validation=success` 均通过；候选 Xcode/JUnit `10/10`/manifest 成功，probe/Koharu 按可选边界跳过。`test/3.png` 未提供，不伪造质量证据；授权语料和目标设备证据不阻塞普通路径。

当前 v3.370 Translation control-marker boundary（已完成）：`raw model output -> terminal control-token cut or line-start prompt-marker cut -> preserve embedded translated content -> shared validation/QA`；自然语言控制词只在输出开头或独立行首截断，真正的 `<start_of_turn>`/`<end_of_turn>` 仍为硬边界。新增 `scripts/test-v3370-translation-control-marker-boundary-contract.py`，工程版本 `3.370`，workflow 已接入；本地安全合同 `369/369` 通过，27 个进程/编译/runtime 入口按约束跳过，Python AST `396/396`、JSON `144/144`、YAML `3/3`、shell `32/32`、plist `4/4` 与 diff 检查通过。实现 SHA `fadd4a96e03bcca1933aab50b0f13a5a78d151dc`、exact full [33321228109](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33321228109)、PR checks [33321140688](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33321140688)、merge SHA `728cbcb8684475e08da61d3d3670862a2170958f`、合入后 push [33322001689](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33322001689) 与 `AITRANS CI/full-validation=success` receipt 均通过；`test/3.png` 未提供，不伪造质量证据，Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。

当前 v3.369 Translation placeholder precision（已完成）：`raw translation -> narrow line-start request + explicit translation-input placeholder gate -> preserve short dialogue -> shared validation/QA`；`请提供文本`、`请输入内容` 等合法短译文不再因通用对象词被误判，明确 `需要翻译的文本`/`text to translate` 请求和既有 refusal/meta marker 仍过滤。OCR/layout、预算、标签、取消、持久化、UI/renderer 与非日语路径不变。新增 `scripts/test-v3369-translation-placeholder-precision-contract.py`，工程版本 `3.369`，workflow 已接入；本地安全回归 `368/368` 通过，27 个进程/编译/runtime 入口跳过；精确 full [33319870129](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33319870129)、PR [#433](https://github.com/bengzhu/project1_lgbt_naxida/pull/433) checks [33319863115](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33319863115)、merge SHA `1f7654c95ca87955ec65f4b0a26e409b6056e454` 与合入后 push [33320584895](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33320584895) 均成功，`AITRANS CI/full-validation=success` receipt 通过；`test/3.png` 未提供，不伪造质量证据，Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。

当前 v3.368 Translation metadata prefix boundary（已完成）：`raw model output -> line-start prompt metadata filter -> preserve embedded translated content -> shared validation/QA`；`translation engine`、`输出风格` 等 marker 只有位于行首（允许边缘标点）时才作为 metadata 删除，合法译文句中出现同样短语时整行保留；明确 metadata bullet、表格和翻译标签继续过滤，placeholder/source leakage/numeric/target-language QA 不变。OCR/layout、预算、标签、取消、持久化和非日语路径不变。新增 `scripts/test-v3368-translation-metadata-prefix-contract.py`，工程版本 `3.368`，workflow 已接入；本地安全合同 `367/367` 通过，27 个进程/编译/runtime 入口跳过，Python AST `414/414`、JSON `144/144`、YAML `3/3`、shell `32/32`、plist `4/4` 与 diff 检查通过。实现 SHA `9f8c52877e93e3d97f9dc450d006f51ed9dfd951`、exact full [33318086676](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33318086676)、PR checks [33318078495](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33318078495)、merge SHA `9d3a3d604a4500191073e558f69fd3bd0f161bc6`、合入后 push [33318776814](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33318776814) 与 `AITRANS CI/full-validation=success` 均通过；`test/3.png` 未提供，不伪造质量证据，Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。

当前 v3.367 Translation numeric token Unicode QA（已完成）：`OCR text -> shared block QA -> narrow fullwidth numeric canonicalization -> numberMismatch`；QA 只将全角数字及数值分隔符映射到 ASCII 后比较，保留数字顺序、分隔符和前导零差异，避免合法 `１２３`/`123` 表示被误拒，同时继续拒绝缺失、合并、拆分或改写数字。OCR/layout、预算、标签、取消、持久化和非日语路径不变。新增 `scripts/test-v3367-translation-numeric-token-unicode-contract.py`，工程版本 `3.367`，CI 已接入；本地安全合同 `366/366` 通过，27 个进程/编译/runtime 入口跳过，Python AST `413/413`、JSON `144/144`、YAML `3/3`、shell `32/32`、plist `4/4` 与 diff 检查通过。实现 SHA `456e84c8f51f5c83c45acaa3e84597b8532d6ea2`、exact full [33316619753](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33316619753)、PR checks [33316611350](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33316611350)、merge SHA `47d49cee95590e45d4e50255c1298d82199f5fb3`、合入后 push [33317161956](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33317161956) 与 `AITRANS CI/full-validation=success` 均通过；`test/3.png` 未提供，不伪造质量证据。

当前 v3.366 Japanese fallback context scope（已完成）：`mixed tagged batch -> exact per-block kind -> single-block context scope -> kind-specific output limit -> existing block QA`；批量失败后的 plain-text 回退不再携带其它编号块的风格 metadata，保留 confirmed 术语和已完成上一批只读摘要；无 `textKind` 的旧块按对应 batch offset 或 dialogue fallback，不继承批次第一块。OCR/layout、标签、8 块/1,800 字符预算、取消、持久化和非日语路径不变。新增 `scripts/test-v3366-japanese-fallback-context-scope-contract.py`，工程版本 `3.366`，CI 已接入；本地安全合同 `365` 个通过、`27` 个进程/编译/runtime 入口跳过（`392` 总计），Python AST `412/412`、JSON `144/144`、workflow YAML `3/3`、shell `32/32`、plist `4/4`、project versions `3.366/3.366` 与 `git diff --check` 通过。实现 SHA `39cadeb70c0e0d4c800e9b86957913ecf32fb0cb`、exact full [33315051697](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33315051697)、PR [#430](https://github.com/bengzhu/project1_lgbt_naxida/pull/430) checks [33315618865](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33315618865)、merge SHA `a6286f876e80fd6d6d33f062d26078817b708fe4`、合入后 push [33315668737](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33315668737) 均成功，云端 receipt 为 `AITRANS CI/full-validation=success`；Koharu/GGUF、授权语料和目标设备不阻塞，`test/3.png` 未提供，不伪造质量证据。

当前 v3.365 Japanese kind-style scope（已完成）：`mixed tagged manga batch -> exact global ordinals -> per-kind scoped style guidance -> existing per-block QA`；对白、旁白、拟声词、标题和其它文字类型的提示分别绑定对应的全局 `[N]` 块，单块请求保留局部类型提示。OCR/layout、`[N]` 标签、8 块/1,800 字符预算、取消、持久化和非日语路径不变。新增 `scripts/test-v3365-japanese-kind-style-scope-contract.py`，工程版本 `3.365`；本地安全合同 `364` 个通过、`27` 个进程/编译/runtime 入口跳过（`391` 总计），Python AST `411/411`、JSON `144/144`、workflow YAML `3/3`、shell `32/32`、plist/project `5/5` 与 `git diff --check` 通过。实现 SHA `1a37c127`、exact full [33313408977](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33313408977)、PR checks [33313973492](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33313973492)、merge SHA `782cc7be`、合入后 push [33314013235](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33314013235) 均成功，云端 Xcode/JUnit/manifest 与 `AITRANS CI/full-validation=success` receipt 通过；Koharu 按配置跳过，`test/3.png` 未提供，不伪造质量证据。

当前 v3.364 Japanese SFX scope（已完成）：`mixed tagged manga batch -> global SFX ordinals -> block-scoped SFX hint -> existing per-block QA`；混合 batch 的声效/状态字提示只绑定对应的全局 `[N]` 块，其他编号块继续使用自己的 dialogue/narration/title 类型提示；单块 standard SFX 保留局部提示。OCR/layout、`[N]` 标签、8 块/1,800 字符预算、取消、持久化和非日语路径不变。新增 `scripts/test-v3364-japanese-sfx-scope-contract.py`，工程版本 `3.364`；本地安全合同 `363` 个通过、`27` 个进程/编译/runtime 入口跳过（`390` 总计），AST/JSON/YAML/shell/plist/project 与 `git diff --check` 通过。实现 SHA `0f576fb7`、exact full [33311997816](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33311997816)、PR checks [33312526828](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33312526828)、merge SHA `bba2cb84`、合入后 push [33312575192](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33312575192) 均成功，云端 Xcode/JUnit/manifest 与 `AITRANS CI/full-validation=success` receipt 通过；`test/2.png` 仅作混合漫画参考，`test/3.png` 未提供，不伪造质量证据。

当前 v3.363 Japanese standard translation prompt（已完成）：`Japanese plain-text fallback/reread -> explicit source/target contract -> faithful plain translation -> existing block-only QA`；日语→简体中文/英语的 standard 请求明确语言对、忠实/简洁要求、称谓/专名/拟声词保留和只输出译文，覆盖批量失败块回退与 OCR 复读后的单块复译。`mangaBlocks` 的 `[N]` 标签提示、OCR/layout、预算、取消、持久化和非日语路径不变。新增 `scripts/test-v3363-japanese-standard-translation-prompt-contract.py`，工程版本 `3.363`；本地安全合同 `362` 个通过、`27` 个进程/编译/runtime 入口跳过（`389` 总计），AST/JSON/YAML/shell/plist/project 与 `git diff --check` 通过。实现 SHA `ae20f960`、exact full [33310548726](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33310548726)、PR checks [33311049315](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33311049315)、merge SHA `e437c67c`、合入后 push [33311104151](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33311104151) 均成功，Xcode/JUnit/manifest 与 `AITRANS CI/full-validation=success` receipt 通过；`test/3.png` 未提供，不伪造质量证据。

当前 v3.362 Japanese translation context-echo boundary（已完成）：`previous completed batch summary -> exact normalized output echo + different source gate -> block-only QA retry`；只有完整复现上一批译文且当前原文不同才判跨 batch 泄漏，合法嵌入短语与重复同一原文保留，OCR/layout、预算、标签、取消、持久化和非日语路径不变。新增 `scripts/test-v3362-japanese-translation-context-echo-contract.py`，工程版本 `3.362`；本地安全合同 `361` 个通过、`27` 个进程/编译/runtime 入口跳过，AST/YAML/shell/plist/project 与 `git diff --check` 通过。实现 SHA `90fcf107`、exact full [33308856823](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33308856823)、PR checks [33309325698](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33309325698)、merge SHA `b699952b`、合入后 push [33309456645](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33309456645) 均成功，发布 `AITRANS CI/full-validation=success` receipt；未提供 `test/3.png`，不伪造翻译质量证据。

当前 v3.361 Japanese pixel compact dedupe（已完成）：`rotated Vision pixel regions -> existing geometry dedupe -> comparable duplicate compact preference -> reserved <=4 compact candidates -> bounded 12-crop recovery`；仅同一、面积可比 envelope 保留较晚 compact 资格，明显更宽的 regular region 仍按历史规则，12/4/方向上限、OCR/layout、翻译 QA、取消、持久化和非日语路径不变。新增 `scripts/test-v3361-japanese-pixel-compact-dedupe-contract.py`，工程版本 `3.361`，新合同 `8/8`；实现 SHA `f5819e14`、exact full [33306958641](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33306958641)、PR checks [33307449223](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33307449223)、merge SHA `d72283f2`、合入后 push [33307511951](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33307511951) 均成功，云端 Xcode/JUnit/manifest 与 `AITRANS CI/full-validation=success` receipt 通过；`test/3.png` 不伪造。

当前 v3.360 Japanese recovery-frontier block skip（已完成）：`lineRefined + pixelFirstRefined -> existing tile frontier -> read-only unique-owner annotation -> strict one-to-one source-line coverage -> skip duplicate block crop or keep fallback`；只有唯一 owner、可靠日语结果和完整源行覆盖可跳过宽 block crop，弱/部分/ownerless/歧义/foreign-owner 结果仍回退；既有 line/pixel/tile/block/方向预算、OCR/layout、翻译 QA、取消、持久化和非日语路径不变。新增 `scripts/test-v3360-japanese-recovery-frontier-block-skip-contract.py`，工程版本 `3.360`，新合同 `9/9`；本地 `359` 个安全合同通过、`27` 个进程/编译/runtime 入口合同跳过（`386` 总计），AST/YAML/shell/plist/project 与 `git diff --check` 通过。实现 SHA `f245acdb`、exact full [33305439236](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33305439236)、PR checks [33305910849](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33305910849)、merge SHA `6e6e58d1`、合入后 push [33305962983](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33305962983) 均成功，Xcode/JUnit/manifest 与 `AITRANS CI/full-validation=success` 通过；`test/3.png` 不伪造。

当前 v3.359 Japanese shared-Han QA exactness（已完成）：`Japanese image batch -> source-leakage QA -> exact shared-Han exception -> block-only retry`；只有规范化后完全相同的至少两字符纯汉字共享词免于泄漏失败，`日本→日本人`、单字、假名/拉丁/数字和其它语言对仍拒绝。OCR、geometry/layout、预算、tag、取消、持久化和非日语路径不变；新合同 `scripts/test-v3359-japanese-shared-han-qa-exactness-contract.py` 已接入，exact full [33303966969](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33303966969)、PR checks [33304583260](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33304583260)、merge push [33304633237](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33304633237) 均通过，`test/3.png` 不伪造。

当前 v3.358 Japanese shared-Han model cleanup（已完成）：`Japanese->Simplified Chinese model output -> narrow unchanged Han policy -> standard/tagged manga cleaner -> existing product QA`；仅放行规范化后相同的纯汉字共享词，假名/拉丁/数字/额外原文/其它语言对仍拒绝。OCR/geometry/layout、请求预算、tag/QA、取消、持久化和非日语路径不变。实现 SHA `a0b79e70`、merge SHA `020f4eb3`、exact full [33302341558](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33302341558)、合入后 push CI [33302865592](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33302865592) 均成功，JUnit `10/10`、Xcode build 与 `AITRANS CI/full-validation=success` 通过。`test/3.png` 尚未提供，不伪造输入或质量证据。
当前 v3.357 Japanese kana SFX translation recall（已完成）：`Japanese OCR block -> conservative kana SFX kind metadata -> existing tagged manga translation prompt -> short SFX/action wording`；只扩展高信号假名形状与拟声词提示，OCR/geometry/layout、请求预算、tag/QA、取消、持久化和非日语路径不变。exact full [33300948175](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33300948175)、merge push [33301557301](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33301557301)。`test/3.png` 尚未提供，不伪造输入或质量证据。
当前 v3.356 Japanese compact spatial balance（已完成）：`pixel-first compact candidates -> existing compact priority sort -> multi-band over-budget round-robin -> reserved <=4 compact slots -> bounded 12-crop recovery`；同一 band 不再独占 compact 名额，under-budget/single-band、12/4 上限、quality/geometry、OCR/layout、翻译 QA、取消和持久化边界不变。实现 SHA `a3b53349`、merge SHA `aa8fc40b`、exact full [33299312826](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33299312826)、合入后 push CI [33299935514](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33299935514)。
当前 v3.355 Japanese geometry-owner balance（已完成）：`geometry-only line pool -> existing text-backed coverage/strict single-owner gates -> owner-only bounded selector -> reserved <=2 geometry slots -> Manga line OCR`；多 owner 超预算时不让一个 owner 占满 geometry 名额，under-budget/single-owner/ownerless 行为、8/2 上限、Vision/Manga OCR、OCR/layout、翻译 QA、取消和持久化边界不变。实现 SHA `0568d4d1`、merge SHA `be23a758`、exact full [33298161646](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33298161646)、合入后 push CI [33298644345](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33298644345)。
当前 v3.354 Japanese direction-confidence owner domain（已完成）：`vertical candidate/line owner geometry -> finite closed [0,1] direction confidence -> existing >=.25 ownership gate -> owner/ownerless fusion`；非法 `NaN/±∞` 与有限越界值不再取得 owner，合法 `.25`/`.45` 语义、16/8/4/24/12/48 上限、line/pixel/tile/frontier、OCR/layout、翻译 QA、取消和持久化边界不变。实现 SHA `86500c81`、merge SHA `885dc9ea`、exact full [33297024649](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33297024649)、合入后 push CI [33297547961](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33297547961)。
当前 v3.353 Japanese vertical crop risk balance（已完成）：`vertical layout blocks -> risk-class partition -> same-class spatial round-robin -> bounded 16-block crop queue`；弱 block 先在风险类内获得既有限额，剩余名额再覆盖非风险 band，未超预算/单 band/无风险队列保持历史选择，16/8 上限、line/pixel/tile/frontier、owner、OCR/layout、翻译 QA、取消和持久化边界不变。
