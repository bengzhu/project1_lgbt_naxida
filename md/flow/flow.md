...
当前 v3.381 Local GGUF sampler state boundary（待云端验收）：普通图片 `test/2.png` 仍从 ordinary image OCR→日语翻译入口进入；`LlamaRuntime` 每次采样后显式 accept token，确保温度/top-p/重复控制推进。新增 `scripts/test-v3381-llama-sampler-state-contract.py`，工程版本 `3.381`，workflow 已接入；云端入口会保存真实逐块译文、LLM trace 和结果列表截图，云端重跑待完成。v3.380 的 exact shared-Han QA、逐块质量门、取消/持久化边界不变；不合成质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
