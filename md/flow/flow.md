...
当前 v3.377 translation batch inline preamble（研发中）：`known preamble + same-line first [N] tag -> strip prefix only -> preserve tag/payload -> existing tag/QA/fallback`；未知前缀保持首标签 fail-closed，OCR、geometry/layout、预算、逐块回退、取消、持久化和非日语路径不变。新增 `scripts/test-v3377-translation-batch-inline-preamble-contract.py`，工程版本 `3.377`，workflow 已接入；候选 full 与合入 receipt 待验证。`test/3.png` 未提供，不伪造质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
