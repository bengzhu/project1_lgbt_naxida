...
正式版本：`1.96`（图片翻译任务语言一致性已收口；Speech 真实 corpus 仍待提供；v1.94 task-scoped full-once / fast-follow-up CI 已收口；v1.97 Koharu 真实路径加固仍是候选，真实四件套运行态仍待 `ci-fast`）。
...
test/1.png
  ...
  -> external TextBox orientation-aware shadow OCR（真实 artifact ready 后对竖排 / 近 90 度倍数旋转 TextBox 执行有上限 rotation OCR；v1.92 对合法凸四点 line polygon 透视校正后逐行 OCR；v1.97 候选隔离单行失败并保留成功行，partial / fallback / 全部失败与任意角度 rotation 继续进入 convergence blockers）
