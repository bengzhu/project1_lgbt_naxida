...
当前正式版本：`3.20`（图片连续复查按动作来源迁移 VoiceOver 焦点，并在队列结束时聚焦完成态；图片 revision 变化拒收旧焦点。v3.19-v2.2 能力与 v3.3 Koharu mask 拓扑 gate 仍保留；真实竖排图片 corpus、Speech corpus 与 Koharu 真实四件套运行态仍待提供）。
