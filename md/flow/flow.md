...
当前 v3.363 Japanese standard translation prompt（候选）：`Japanese plain-text fallback/reread -> explicit source/target contract -> faithful plain translation -> existing block-only QA`；日语→简体中文/英语的 standard 请求明确语言对、忠实/简洁要求、称谓/专名/拟声词保留和只输出译文，覆盖批量失败块回退与 OCR 复读后的单块复译。`mangaBlocks` 的 `[N]` 标签提示、OCR/layout、预算、取消、持久化和非日语路径不变。新增 `scripts/test-v3363-japanese-standard-translation-prompt-contract.py`，工程版本 `3.363`，CI 已接入；待云端 full validation，`test/3.png` 未提供，不伪造质量证据。
