...
当前 v3.349 Japanese tile coverage eligibility（开发中）：`vertical blocks -> usable Japanese text + finite confidence/density gate -> existing vertical tile coverage -> bounded 18-window tile fallback`；弱/空/非日语 block 不再抑制 tile，可靠 block 与 line frontier 仍抑制重复窗口，最多 6 个 tile、方向 4、pixel-first/line/block recovery、OCR/layout、翻译 QA、取消和持久化边界不变。
