...
当前 v3.357 Japanese kana SFX translation recall（研发中）：`Japanese OCR block -> conservative kana SFX kind metadata -> existing tagged manga translation prompt -> short SFX/action wording`；只扩展高信号假名形状与拟声词提示，OCR/geometry/layout、请求预算、tag/QA、取消、持久化和非日语路径不变。`test/3.png` 尚未提供，不伪造输入或质量证据。
