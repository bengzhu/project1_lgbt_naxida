...
v3.200 日语竖排 block → 收集相关 `lineRegionRect` 并与原 block 做 envelope union → 以原 `block.rect` 最小边作为 Koharu font-size anchor 计算方向感知 padding → block crop OCR／fallback → line reread／去重／布局／批翻译／渲染；避免 union envelope 变大造成过度扩边，缺失 geometry 回退旧 crop，非日语与其他路径不变。exact-SHA full `31260969111`（SHA `c91858868cace78b846bd742f69a95f765b5737a`，Xcode/JUnit `10/10`）通过；候选静态路由 `31261252088` 不作为新的编译证据，PR/merge receipt 待补，探针 skip，readiness `manifestMissing / stopUntilArtifactsProvided`。
