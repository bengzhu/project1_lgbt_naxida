...
v3.204 日语竖排 block → 有效 line polygon proxy 先走 perspective／轴对齐 line OCR → 只有 line 无可用 observation 才回退 block crop → v3.203 tight ownership／方向 provenance／bounded fallback → 映射／去重／布局／批翻译／渲染；v3.157/v3.158 双向方向与 block crop 合同继续回归。实现 full `31290525270`（SHA `ae922bda0bf566cc14d422a6d9c9a4c042b34218`，Xcode/JUnit `10/10`）通过，候选 metadata、PR 与 merge 待完成，探针 skip，readiness `manifestMissing / stopUntilArtifactsProvided`。
