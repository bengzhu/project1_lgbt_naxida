...
当前 v3.356 Japanese compact spatial balance（研发中）：`pixel-first compact candidates -> existing compact priority sort -> multi-band over-budget round-robin -> reserved <=4 compact slots -> bounded 12-crop recovery`；同一 band 不再独占 compact 名额，under-budget/single-band、12/4 上限、quality/geometry、OCR/layout、翻译 QA、取消和持久化边界不变。
