...
当前 v3.350 Japanese tile direction eligibility（已完成）：`vertical blocks -> confident vertical direction (finite [0,1], >=.45) + usable Japanese text/OCR gate -> existing vertical tile coverage -> bounded 18-window tile fallback`；方向不确定、弱/空/非日语 block 不抑制 tile，可靠 block 与 line frontier 仍抑制重复窗口，最多 6 个 tile、方向 4、pixel-first/line/block recovery、OCR/layout、翻译 QA、取消和持久化边界不变。
