...
正式版本：`3.3`（Koharu artifact contract v2 在 v3.2 的 BubbleMask / SegmentMask 有界 RLE 上，新增复用 external shadow OCR 稳定一对一 TextBox assignment 的 SegmentMask→BubbleMask 像素/component 拓扑 gate；v1 摘要兼容但不能关闭 payload 或 topology gate；v3.1-v2.2 的图片复查与生命周期能力仍保留；真实竖排图片 corpus、Speech corpus 与 Koharu 真实四件套运行态仍待提供）。
