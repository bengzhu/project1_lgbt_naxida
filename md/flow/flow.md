...
当前 v3.307 日语图片翻译流程：`OCR/layout blocks -> mixed-script candidate fidelity selection -> mixed-script-safe Japanese OCR normalization -> bounded Japanese OCR recovery -> reconcile conservative SFX kind hint -> stable identity batch plan -> completed-only read-only context normalization -> strict tagged translation -> per-block QA/retry -> correction/retry context parity`。跨 batch 摘要只有在只读、非 pending、来自 completed blocks、含非空 source/target excerpt 且 ordinal 有效时才进入 prompt；无效摘要同时从 QA 泄漏判定中移除。人工修正、单块重试、scoped OCR 复读和忽略/恢复只更新 block 值，batch context 仍不进入 block、持久化或取消边界。
