...
当前 v3.385 Local GGUF Japanese minimal-fallback boundary（待云端验收）：普通图片 `test/2.png` 仍从 ordinary image OCR→日语翻译入口进入；v3.384 真实运行已得到 17 个非空日语 OCR 块，但 270M 模型回显带 compact context 的 prompt。v3.385 在保留带 context 的双语尝试后，追加无 context 的最小日语任务 prompt；漫画批译追加最小 `[N]` 标签候选。批量生成上限仍为 256 token，为 `LlamaRuntime` 的 1,024-token context 留出空间；所有结果继续进入严格逐块 QA。新增 `scripts/test-v3385-japanese-minimal-fallback-contract.py`，工程版本 `3.385`，workflow 已接入；云端入口会保存真实逐块译文、LLM trace 和结果列表截图，云端重跑待完成。完整 context 仍只供 QA，sampler state、v3.380 的 exact shared-Han QA、逐块质量门、取消/持久化边界不变；不合成质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
