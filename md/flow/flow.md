...
正式版本：`3.11`（图片预览新增 revision-scoped 加载、成功、失败和本地重试反馈；v3.10 的后台 ImageIO 有界下采样继续保留，Store 原始图片仍供 OCR 与导出使用。v3.9-v2.2 能力与 v3.3 Koharu mask 拓扑 gate 仍保留；真实竖排图片 corpus、Speech corpus 与 Koharu 真实四件套运行态仍待提供）。
