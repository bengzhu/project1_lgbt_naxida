...
当前 v3.370 Translation control-marker boundary（验证中）：`raw model output -> terminal control-token cut or line-start prompt-marker cut -> preserve embedded translated content -> shared validation/QA`；自然语言控制词只在输出开头或独立行首截断，真正的 `<start_of_turn>`/`<end_of_turn>` 仍为硬边界。新增 `scripts/test-v3370-translation-control-marker-boundary-contract.py`，工程版本 `3.370`，workflow 已接入；本地与云端 receipt 待验证，`test/3.png` 未提供，不伪造质量证据，Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
