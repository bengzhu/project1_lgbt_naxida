...
当前 v3.369 Translation placeholder precision（验证中）：`raw translation -> narrow line-start request + explicit translation-input placeholder gate -> preserve short dialogue -> shared validation/QA`；`请提供文本`、`请输入内容` 等合法短译文不再因通用对象词被误判，明确 `需要翻译的文本`/`text to translate` 请求和既有 refusal/meta marker 仍过滤。OCR/layout、预算、标签、取消、持久化、UI/renderer 与非日语路径不变。新增 `scripts/test-v3369-translation-placeholder-precision-contract.py`，工程版本 `3.369`，workflow 已接入；本地与云端 receipt 待验证，`test/3.png` 未提供，不伪造质量证据，Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
