...
当前 v3.358 Japanese shared-Han model cleanup（候选实现）：`Japanese->Simplified Chinese model output -> narrow unchanged Han policy -> standard/tagged manga cleaner -> existing product QA`；仅放行规范化后相同的纯汉字共享词，假名/拉丁/数字/额外原文/其它语言对仍拒绝。OCR/geometry/layout、请求预算、tag/QA、取消、持久化和非日语路径不变；exact-SHA cloud full 与 receipt 待提交后验证。`test/3.png` 尚未提供，不伪造输入或质量证据。
