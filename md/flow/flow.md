...
当前 v3.379 translation standalone-English-label boundary（待云端验收）：`plain translation -> exact standalone English label line -> remove label line only -> preserve content lines -> shared cleanup/QA`；带正文的 `Here is the translation ...` 等自然句、严格漫画 `[N]` 标签、placeholder/source leakage/目标语言/数字/术语/长度 QA、OCR/layout、预算、逐块回退、取消、持久化和非日语路径不变。新增 `scripts/test-v3379-translation-standalone-english-label-contract.py`，工程版本 `3.379`，workflow 已接入；本地安全回归与 exact-SHA full 待完成，`test/3.png` 未提供，不合成质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
