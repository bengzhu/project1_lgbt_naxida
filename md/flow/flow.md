...
当前 v3.380 Japanese shared-Han cloud QA boundary（待云端验收）：`source substring -> exact ja→zh-CN shared-Han equality exception -> otherwise sourceLeakage`；例外要求规范化后的源文与输出完全相等、源文长度大于 1 且源文仅含共享汉字，cloud shadow evaluator 与产品 `TranslationOutputPolicy` 保持同一矩阵。新增 `scripts/test-v3380-japanese-shared-han-cloud-qa-contract.py`，工程版本 `3.380`，workflow 已接入；`test/2.png` 已纳入当前工作树，并由独立云端入口走真实普通 OCR→翻译、持久化结果校验和结果列表截图，云端验收待完成；不合成质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
