...
当前 v3.364 Japanese SFX scope（候选）：`mixed tagged manga batch -> global SFX ordinals -> block-scoped SFX hint -> existing per-block QA`；混合 batch 的声效/状态字提示只绑定对应的全局 `[N]` 块，其他编号块继续使用自己的 dialogue/narration/title 类型提示；单块 standard SFX 保留局部提示。OCR/layout、`[N]` 标签、8 块/1,800 字符预算、取消、持久化和非日语路径不变。新增 `scripts/test-v3364-japanese-sfx-scope-contract.py`，工程版本 `3.364`，CI 已接入；候选 full 待验证，`test/2.png` 仅作混合漫画参考，`test/3.png` 未提供，不伪造质量证据。
