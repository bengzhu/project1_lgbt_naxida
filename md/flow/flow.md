...
当前 v3.367 Translation numeric token Unicode QA（实现已完成，验证进行中）：`OCR text -> shared block QA -> narrow fullwidth numeric canonicalization -> numberMismatch`；QA 只将全角数字及数值分隔符映射到 ASCII 后比较，保留数字顺序、分隔符和前导零差异，避免合法 `１２３`/`123` 表示被误拒，同时继续拒绝缺失、合并、拆分或改写数字。OCR/layout、预算、标签、取消、持久化和非日语路径不变。新增 `scripts/test-v3367-translation-numeric-token-unicode-contract.py`，工程版本 `3.367`，CI 已接入；`test/3.png` 未提供，不伪造质量证据。
