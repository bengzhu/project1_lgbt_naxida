...
当前 v3.384 Local GGUF Japanese prompt-shape boundary（待云端验收）：普通图片 `test/2.png` 仍从 ordinary image OCR→日语翻译入口进入；批量和所有日语 standard 单块回退都使用短双语任务指令、明确的 `Text to translate`/`待翻译文本` 标记和有限只读 compact context，批量生成上限为 256 token，为 `LlamaRuntime` 的 1,024-token context 留出空间。新增 `scripts/test-v3384-japanese-prompt-shape-contract.py`，工程版本 `3.384`，workflow 已接入；云端入口会保存真实逐块译文、LLM trace 和结果列表截图，云端重跑待完成。完整 context 仍只供 QA，sampler state、v3.380 的 exact shared-Han QA、逐块质量门、取消/持久化边界不变；不合成质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
