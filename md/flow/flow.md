...
当前 v3.365 Japanese kind-style scope（候选，待云端验证）：`mixed tagged manga batch -> exact global ordinals -> per-kind scoped style guidance -> existing per-block QA`；对白、旁白、拟声词、标题和其它文字类型的提示分别绑定对应的全局 `[N]` 块，单块请求保留局部类型提示。OCR/layout、`[N]` 标签、8 块/1,800 字符预算、取消、持久化和非日语路径不变。新增 `scripts/test-v3365-japanese-kind-style-scope-contract.py`，工程版本 `3.365`；候选实现与云端证据待补。本地只运行安全静态检查，`test/2.png` 仅作已有混合漫画参考，`test/3.png` 未提供，不伪造质量证据。
