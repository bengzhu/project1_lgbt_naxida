...
当前 v3.366 Japanese fallback context scope（当前研发分支，待云端验证）：`mixed tagged batch -> exact per-block kind -> single-block context scope -> kind-specific output limit -> existing block QA`；批量失败后的 plain-text 回退不再携带其它编号块的风格 metadata，保留 confirmed 术语和已完成上一批只读摘要；无 `textKind` 的旧块按对应 batch offset 或 dialogue fallback，不继承批次第一块。OCR/layout、标签、8 块/1,800 字符预算、取消、持久化和非日语路径不变。新增 `scripts/test-v3366-japanese-fallback-context-scope-contract.py`，工程版本 `3.366`，CI 已接入，待安全静态回归与 exact-SHA full；Koharu/GGUF、授权语料和目标设备不阻塞，`test/3.png` 未提供，不伪造质量证据。
