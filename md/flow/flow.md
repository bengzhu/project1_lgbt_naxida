...
当前 v3.378 translation leading-label boundary（研发中）：`plain translation -> finite leading label + explicit delimiter + payload -> strip label only -> shared cleanup/QA`；无分隔标签、嵌入正文、既有 `翻译是：` 歧义内容、严格漫画 `[N]` 标签、OCR、geometry/layout、预算、逐块回退、取消、持久化和非日语路径不变。新增 `scripts/test-v3378-translation-leading-label-boundary-contract.py`，工程版本 `3.378`，workflow 已接入；云端 exact-SHA full 待完成。`test/3.png` 未提供，不合成样图或质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
