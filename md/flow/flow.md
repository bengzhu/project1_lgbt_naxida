...
当前 v3.382 Local GGUF prompt budget boundary（待云端验收）：普通图片 `test/2.png` 仍从 ordinary image OCR→日语翻译入口进入；漫画批量和长上下文日语请求使用有限的只读 compact context，并把批量生成上限设为 256 token，为 `LlamaRuntime` 的 1,024-token context 留出空间。新增 `scripts/test-v3382-local-gguf-prompt-budget-contract.py`，工程版本 `3.382`，workflow 已接入；云端入口会保存真实逐块译文、LLM trace 和结果列表截图，云端重跑待完成。sampler state、v3.380 的 exact shared-Han QA、逐块质量门、取消/持久化边界不变；不合成质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
