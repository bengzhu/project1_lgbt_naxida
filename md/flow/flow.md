...
当前 v3.359 Japanese shared-Han QA exactness（候选验证中）：`Japanese image batch -> source-leakage QA -> exact shared-Han exception -> block-only retry`；只有规范化后完全相同的至少两字符纯汉字共享词免于泄漏失败，`日本→日本人`、单字、假名/拉丁/数字和其它语言对仍拒绝。OCR、geometry/layout、预算、tag、取消、持久化和非日语路径不变；新合同 `scripts/test-v3359-japanese-shared-han-qa-exactness-contract.py` 已接入，云端候选 receipt 待补录，`test/3.png` 不伪造。
