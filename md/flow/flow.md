...
当前正式版本：`3.19`（图片风险结果行提供独立完成并继续/撤销复查动作，主行仍只定位；复查入口区分开始与继续，风险标签适配窄 inspector 与 Dynamic Type。v3.18-v2.2 能力与 v3.3 Koharu mask 拓扑 gate 仍保留；真实竖排图片 corpus、Speech corpus 与 Koharu 真实四件套运行态仍待提供）。
