...
当前 v3.373 translation term width QA（进行中）：`confirmed term source/output -> Unicode canonical + case/width folding -> exact containment -> existing block QA`；全角/半角与规范化等价的术语不再被误判为缺失，标点仍保持语义边界，revoked 术语继续 fail-closed。OCR、geometry/layout、预算、标签、逐块回退、取消、持久化和非日语路径不变。新增 `scripts/test-v3373-translation-term-width-qa-contract.py`，工程版本 `3.373`，workflow 已接入；cloud full/merge receipt 待执行。`test/3.png` 未提供，不伪造质量证据；Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
