...
test/1.png
  ...
  -> external TextBox orientation-aware shadow OCR（真实 artifact ready 后对竖排 / 近 90 度倍数旋转 TextBox 执行有上限 rotation OCR；v1.92 对合法凸四点 line polygon 透视校正后逐行 OCR；v1.97 隔离单行失败并保留成功行，partial / fallback / 全部失败与任意角度 rotation 继续进入 convergence blockers）
