...
v3.241 日语 detector TextRegion → 严格 quad gate → Manga OCR bbox 主 crop；仅当 bbox 结果弱时才执行 line quad fallback，四点 quad 先按 Koharu 长短轴生成 bounded target canvas → `rotate270` → Manga OCR，目标／旋转失败保留自然 warp，投影失败回到 bbox。layout／ownership `rect`、batch、12-48 预算、取消、Vision fallback、批翻译、竖排渲染与非日语路径不变。
