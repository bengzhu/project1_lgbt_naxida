...
当前 v3.355 Japanese geometry-owner balance（研发中）：`geometry-only line pool -> existing text-backed coverage/strict single-owner gates -> owner-only bounded selector -> reserved <=2 geometry slots -> Manga line OCR`；多 owner 超预算时不让一个 owner 占满 geometry 名额，under-budget/single-owner/ownerless 行为、8/2 上限、Vision/Manga OCR、OCR/layout、翻译 QA、取消和持久化边界不变。
