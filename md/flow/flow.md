...
当前 v3.341 Japanese vertical tile round-robin：`uncovered vertical reconnaissance -> height-band round-robin across right-to-left strips -> existing coverage/crop/meaningful-text gate -> bounded Vision OCR fusion`；同一高度窗口先轮询各列，避免长图的 18 个既有 tile 名额被最右列独占，列内窗口仍按上到下，窗口几何、方向补读、去重、翻译 QA、取消与持久化边界不变。
