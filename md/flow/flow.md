...
当前 v3.353 Japanese vertical crop risk balance（研发中）：`vertical layout blocks -> risk-class partition -> same-class spatial round-robin -> bounded 16-block crop queue`；弱 block 先在风险类内获得既有限额，剩余名额再覆盖非风险 band，未超预算/单 band/无风险队列保持历史选择，16/8 上限、line/pixel/tile/frontier、owner、OCR/layout、翻译 QA、取消和持久化边界不变。
