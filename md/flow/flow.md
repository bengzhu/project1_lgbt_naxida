...
test/1.png
  ...
  -> TextRegion crop OCR 候选诊断和 subregion / bubble / content / mask coverage 审计
...
- TextRegion crop 当前是结构化候选层；v13 会优先尝试可信 block-local subregion clamp，再回退到 bubble bbox 或 content rect；v14 额外记录 crop bbox 与 BubbleMask 近似的覆盖关系。只有通过 ground-truth-free 护栏才可替换主翻译输入，本轮 0 块采用。
- BubbleMask 当前是 bbox/rounded-rect 近似实例 ID mask，用于 seed 归属、mask-safe layout、crop coverage 和渲染碰撞诊断；不是 Koharu 真实分割 mask，不能把布局收益冒充 OCR 提升。
