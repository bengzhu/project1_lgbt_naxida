...
当前 v3.309 日语图片翻译流程：`OCR/layout blocks -> mixed-script candidate fidelity selection -> mixed-script-safe Japanese OCR normalization -> bounded Japanese OCR recovery -> scoped Manga/Vision candidate gate -> accepted scoped provenance handoff -> reconcile conservative SFX kind hint -> stable identity batch plan -> completed-only read-only context normalization -> strict tagged translation -> per-block QA/retry -> correction/retry context parity`。同一 scoped block 的 Manga OCR 保持基线，只有同一 bounded Vision crop 通过日语密度、有限 confidence、文字数量/密度或 confidence margin 门槛才替换；接受的 scoped OCR 候选同时更新该 block 的诊断 provenance，避免新文字携带旧引擎、旧 crop 或旧旋转来源；方向候选仍有界，平局保留 Manga。跨 batch 摘要只有在只读、非 pending、来自 completed blocks、含非空 source/target excerpt 且 ordinal 有效时才进入 prompt；无效摘要同时从 QA 泄漏判定中移除。人工修正、单块重试、scoped OCR 复读和忽略/恢复只更新 block 值，候选比较不进入 block identity、布局、持久化或取消边界。
