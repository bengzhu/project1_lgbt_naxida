# 项目流程图
v3.355 Japanese geometry-owner balance（研发中）：`geometry-only candidates -> existing coverage/owner gate -> owner-balanced <=2 geometry reserve -> bounded 8-request Manga line OCR`；同一 known owner 不再独占两个 geometry 名额，under-budget/single-owner/ownerless、OCR/layout、翻译 QA、取消和持久化边界不变。
