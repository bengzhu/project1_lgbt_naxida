# 项目流程图
v3.356 Japanese compact spatial balance（研发中）：`compact candidates -> existing y/x priority -> owner-independent multi-band selector -> reserved <=4 compact recovery -> bounded 12 pixel crops`；跨 band 的短候选获得有限复读机会，under-budget/single-band、OCR/layout、翻译 QA、取消和持久化边界不变。
