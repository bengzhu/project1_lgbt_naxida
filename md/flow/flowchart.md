# 项目流程图
v3.383 Local GGUF standard fallback prompt boundary（待云端验收）：`compact read-only context -> Japanese standard fallback -> QA keeps full context`；普通 `test/2.png` 通过 ordinary image OCR→日语翻译路径，并保存真实 LLM trace、逐块 JSON 与结果列表界面。新增合同 `scripts/test-v3383-japanese-standard-compact-context-contract.py`，工程版本 `3.383`，CI 已接入；重跑待完成，不合成质量证据。sampler state、v3.382 的 256-token manga budget、v3.380 shared-Han QA 与既有图片预算、逐块 QA/取消/持久化边界保持不变；可选 Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
