# 项目流程图
v3.378 translation leading-label boundary（研发中）：`plain output -> explicit leading label delimiter recovery -> shared cleanup/QA`；无分隔或嵌入正文保持原文，严格 `[N]` 批译、OCR/layout、预算、逐块 QA/回退、取消、持久化与非日语路径不变。新增合同 `scripts/test-v3378-translation-leading-label-boundary-contract.py`，工程版本 `3.378`，CI 已接入；等待安全静态回归与 exact-SHA full。`test/3.png` 未提供，不合成质量证据；可选 Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
