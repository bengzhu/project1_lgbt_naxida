# 项目流程图
v3.357 Japanese kana SFX translation recall（研发中）：`Japanese OCR block -> conservative kana-shape classifier -> existing text-kind metadata -> tagged translation prompt -> short SFX/action wording`；只扩展高信号假名声效形状，OCR 请求、候选预算、geometry/layout、tag/QA、取消、持久化和非日语路径保持不变。
