# 项目流程图
v3.381 Local GGUF sampler state boundary（待云端验收）：`prompt -> llama sampler sample -> accept token -> decode next token`；普通 `test/2.png` 通过 ordinary image OCR→日语翻译路径，并保存真实 LLM trace、逐块 JSON 与结果列表界面。新增合同 `scripts/test-v3381-llama-sampler-state-contract.py`，工程版本 `3.381`，CI 已接入；重跑待完成，不合成质量证据。v3.380 shared-Han QA 与既有图片预算、逐块 QA/取消/持久化边界保持不变；可选 Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
