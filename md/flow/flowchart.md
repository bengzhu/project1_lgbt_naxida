# 项目流程图
v3.359 Japanese shared-Han QA exactness（候选验证中）：`image block translation -> normalized source-leakage gate -> exact shared-Han exception -> existing block-only retry`；`日本→日本` 仍可按既有窄 policy 放行，`日本→日本人` 与单字共享汉字不放行；OCR、layout、请求预算、标签、取消、持久化和非日语路径保持不变，云端 full/PR/merge receipt 待补录。
