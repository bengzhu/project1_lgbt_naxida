# 项目流程图
v3.354 Japanese direction-confidence owner domain（研发中）：`vertical candidate/line owner geometry -> finite [0,1] domain -> >=.25 owner gate -> known-owner or ownerless compatibility`；非法方向分数不取得跨块 owner，合法门、既有预算、line/pixel/tile/frontier、OCR/layout、翻译 QA、取消和持久化边界不变。
