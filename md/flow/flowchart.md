# 项目流程图
v3.380 Japanese shared-Han cloud QA boundary（待云端验收）：`source substring -> exact ja→zh-CN shared-Han equality exception -> otherwise sourceLeakage`；仅规范化后完全相等、源文长度大于 1 且源文为共享汉字时保留不变输出，`日本→日本人` 等扩展输出继续 fail-closed。新增合同 `scripts/test-v3380-japanese-shared-han-cloud-qa-contract.py`，工程版本 `3.380`，CI 已接入；`test/2.png` 已纳入 bundle 资源，独立云端入口会走真实普通 OCR→翻译并捕获日语原文/简体中文译文列表界面，云端验收待完成，不合成质量证据；可选 Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
