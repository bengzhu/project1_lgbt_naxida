# 项目流程图
v3.352 Japanese direction-confidence risk domain（研发中）：`vertical recovery -> finite closed [0,1] direction confidence -> risk gate/order -> bounded crop/recovery`；`NaN/±∞` 与有限越界方向值按弱候选处理，合法 `[0,1]` 与 `.45` 门、16/8/4 上限、pixel/line/tile/frontier、owner、OCR/layout、翻译 QA、取消和持久化边界不变。
