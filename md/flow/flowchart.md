# 项目流程图
v3.342 Japanese line owner balance：`risk-first text-backed line queue -> over-budget owner-balanced selection -> reserved geometry-only slots -> Manga/Vision fusion`；只在 text-backed 预算会漏掉 known vertical TextRegion owner 时替换已覆盖 owner 的重复候选，保留风险顺序、8/2 请求边界、ownerless 历史兼容与下游布局/翻译/取消/持久化边界。
