...
v3.387 Local GGUF Japanese bare-prompt fallback boundary（待云端验收）：`context-aware bilingual attempt -> context-free English minimal attempt -> context-free Chinese-shaped Japanese fallback -> exact bare Chinese template -> strict per-block QA`；漫画批译同样保留 `[N]` 标签顺序候选。普通 `test/2.png` 通过 ordinary image OCR→日语翻译路径，并保存真实 LLM trace、逐块 JSON、结果列表界面和基于真实终态会话重开的整图 OCR 原文框截图/坐标清单。新增合同 `scripts/test-v3387-japanese-bare-prompt-contract.py`，工程版本 `3.387`，CI 已接入；重跑待完成，不合成质量证据。sampler state、256-token manga budget、shared-Han QA 与既有图片预算、逐块 QA/取消/持久化边界保持不变；可选 Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
v3.379 standalone English label boundary（已完成）：`plain output -> exact standalone Here is/are the translation(s) label -> remove only metadata line -> preserve natural payload -> shared cleanup/QA`；带正文的同名自然句、严格 `[N]` 批译、OCR/layout、预算、逐块 QA/回退、取消、持久化和非日语路径不变。新增合同 `scripts/test-v3379-translation-standalone-english-label-contract.py`，工程版本 `3.379`，CI 已接入；本地安全回归 `378/378` 通过、27 个进程/编译/runtime 入口跳过（405 总计），Python AST `405/405`、JSON `144/144`、YAML `3/3`、shell `32/32`、plist `4/4` 与 diff 检查通过；实现 SHA `7c6d5cf952e74f641f6ee9c439545fb158982601` 的 exact full [33367420463](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33367420463)、PR [#443](https://github.com/bengzhu/project1_lgbt_naxida/pull/443) checks [33368408182](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33368408182)、merge SHA `6454b3179a2706287037db57dc092ac20092627e` 与合入后 push [33368493788](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33368493788) 均成功，发布 `AITRANS CI/full-validation=success` receipt。候选 Xcode 26.6/JUnit `10/10` 与 Japanese benchmark 通过，UI evidence/probe 按参数跳过。`test/3.png` 未提供，不合成质量证据；可选 Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
v3.378 translation leading-label boundary（已完成）：`plain output -> explicit leading label delimiter recovery -> shared cleanup/QA`；无分隔或嵌入正文保持原文，严格 `[N]` 批译、OCR/layout、预算、逐块 QA/回退、取消、持久化与非日语路径不变。新增合同 `scripts/test-v3378-translation-leading-label-boundary-contract.py`，工程版本 `3.378`，CI 已接入；本地安全回归 377/377、27 个进程/编译/runtime 入口按约束跳过（404 总计），Python AST 404/404、tracked JSON 144/144、workflow YAML 3/3、shell 32/32、plist 4/4 与 diff 检查通过；exact-SHA full [33365044114](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33365044114)、PR [#442](https://github.com/bengzhu/project1_lgbt_naxida/pull/442) checks [33366094900](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33366094900)、merge SHA `763b27c1f9d02f1e1358803e92cd297556764f8f`、合入后 push [33366187576](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33366187576) 与 receipt 均成功。`test/3.png` 未提供，不合成质量证据；可选 Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。

v3.377 translation batch inline preamble（已完成）：`known same-line preamble -> strip only before first [N] -> preserve tagged payload -> existing parser/QA/fallback`；未知前缀、标签、逐块 QA、OCR/layout、预算、取消、持久化和非日语路径边界不变。新增合同 `scripts/test-v3377-translation-batch-inline-preamble-contract.py`，工程版本 `3.377`，CI 已接入；本地安全回归 `376/376`、Python AST `403/403`、tracked JSON `144/144`、YAML `3/3`、shell `31/31`、plist `4/4` 与 diff 检查通过；最终实现 SHA `9ae1205f3123203fc91c7075b635bcb2c75379f1` 的 exact full [33362823753](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33362823753)、PR [#441](https://github.com/bengzhu/project1_lgbt_naxida/pull/441) checks [33363194227](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33363194227)、merge SHA `8b63ff223b260303e9e3efb24fc779a6fb00d5c6` 与合入后 push [33363279607](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33363279607) 均成功；`test/3.png` 未提供，不伪造质量证据，Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。

v3.376 translation metadata content boundary（已完成）：`leading content-sensitive marker + natural payload -> preserve translated content; standalone/punctuation-delimited marker -> remove prompt metadata -> existing validation/QA`；产品只把 `translation engine`、`输出风格` 的独立或标点分隔行首形态当作 prompt metadata，明确指令 marker、表格、标签、OCR/layout、预算、逐块回退、取消、持久化和非日语路径不变。新增合同 `scripts/test-v3376-translation-metadata-content-boundary-contract.py`，工程版本 `3.376`，CI 已接入；本地安全合同 `375/375` 通过、27 个进程/编译/runtime 入口跳过（`402` 总计），Python AST `402/402`、JSON `144/144`、YAML `3/3`、shell `32/32`、plist `4/4` 与 `git diff --check` 通过；实现 SHA `418f86e9e502bb57e831c77eb34380ff0f97fd60` 的 exact-SHA full [33359531013](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33359531013)、PR [#440](https://github.com/bengzhu/project1_lgbt_naxida/pull/440) checks [33360556432](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33360556432)、merge SHA `6d5422e0bc223b4a22a3000374be4b0810f647f8` 与合入后 push CI [33360620974](https://github.com/bengzhu/project1_lgbt_naxida/actions/runs/33360620974) 均成功；`test/3.png` 未提供，不伪造质量证据，Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
