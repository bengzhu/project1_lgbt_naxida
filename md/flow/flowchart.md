# 项目流程图
v3.362 Japanese translation context-echo boundary（候选验证中）：`previous batch summary -> exact normalized echo + different source -> existing block-only QA retry`；合法嵌入短语与重复同一原文不再被宽泛 context-leak gate 误拒绝，既有 OCR、layout、预算、标签、取消、持久化和非日语路径不变。新增合同 `scripts/test-v3362-japanese-translation-context-echo-contract.py`，`test/3.png` 不伪造。
