# 项目流程图
v3.348 Japanese pixel recovery eligibility：`vertical provenance/role -> nonempty Japanese + finite confidence >=.48 + letter/script density >=.5 -> existing vertical coverage`；弱/空/非日语读数不再遮蔽 bounded 12-crop pixel-first recovery，可靠读数仍抑制重复 geometry，compact owner 例外、方向 4、line/tile frontier、翻译 QA、取消和持久化边界不变。
