# 项目流程图
v3.353 Japanese vertical crop risk balance（研发中）：`vertical blocks -> risk-class-first queue -> same-class band round-robin -> bounded 16 crops`；dense weak band 不再被 strong other bands 先挤出，剩余名额仍按空间均衡补齐，单 band/未超预算/无风险行为、8 次方向 fallback、line/pixel/tile/frontier、owner、OCR/layout、翻译 QA、取消和持久化边界不变。
