# 项目流程图
v3.367 Translation numeric token Unicode QA（实现已完成，验证进行中）：`block translation -> shared QA -> canonicalize fullwidth numeric digits/separators -> exact numberMismatch`；`１２３` 与 `123` 视为同一数字 token，前导零、token 顺序、分隔符以及缺失/合并/拆分数字仍保持严格失败。OCR/layout、预算、取消、持久化和非日语路径不变。新增合同 `scripts/test-v3367-translation-numeric-token-unicode-contract.py`，工程版本 `3.367`，CI 已接入；`test/3.png` 未提供，不伪造质量证据。
