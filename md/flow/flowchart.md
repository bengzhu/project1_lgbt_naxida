# 项目流程图
v3.379 standalone English label boundary（待云端验收）：`plain output -> exact standalone Here is/are the translation(s) label -> remove only metadata line -> preserve natural payload -> shared cleanup/QA`；带正文的同名自然句、严格 `[N]` 批译、OCR/layout、预算、逐块 QA/回退、取消、持久化和非日语路径不变。新增合同 `scripts/test-v3379-translation-standalone-english-label-contract.py`，工程版本 `3.379`，CI 已接入；本地安全回归与 exact-SHA full 待完成。`test/3.png` 未提供，不合成质量证据；可选 Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
