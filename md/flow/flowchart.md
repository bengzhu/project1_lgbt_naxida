# 项目流程图
v3.386 Local GGUF Japanese Chinese-shaped fallback boundary（待云端验收）：`context-aware bilingual attempt -> context-free English minimal attempt -> context-free Chinese-shaped Japanese fallback -> strict per-block QA`；漫画批译同样保留 `[N]` 标签顺序候选。普通 `test/2.png` 通过 ordinary image OCR→日语翻译路径，并保存真实 LLM trace、逐块 JSON 与结果列表界面。新增合同 `scripts/test-v3386-japanese-chinese-prompt-contract.py`，工程版本 `3.386`，CI 已接入；重跑待完成，不合成质量证据。sampler state、256-token manga budget、shared-Han QA 与既有图片预算、逐块 QA/取消/持久化边界保持不变；可选 Koharu/GGUF、授权语料和目标设备证据不阻塞普通路径。
