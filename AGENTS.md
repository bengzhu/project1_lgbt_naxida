...
- 当前正式版本号 `3.36`：开发控制台在漫画探针已有 `MangaOverlayProbeReport.externalArtifactReadinessReport` 时，由 View 私有 `MangaKoharuArtifactReadinessSummary` 以可访问 status 区分 `readyForShadowOCR && externalTextBoxesShadowOCRAllowed`、`manifestMissing` / `artifactFilesMissing` 和其他无效状态，并复用可复制 code block 显示 source、verdict、nextAction、四件套 presence / missing、parse errors、generatedBy 与计数。摘要本身不持有新的 Store／probe state、不运行 probe、不加载／写入 active artifact，也不改 Vision OCR、模型、renderer/export、普通图片路径或质量基线；缺真实四件套时仍只显示 `manifestMissing / stopUntilArtifactsProvided`，不能作为 shadow OCR、真机 UI 或质量提升证据。v3.35 的局部预览修正入口仍以同一 busy / active-block guard 登记 revision-scoped `onDismiss` fallback，非成功关闭回同一局部窗、结果行入口回结果行，成功／忽略仍覆盖为既有下一块、完成态或忽略行目标。v3.26 CI receipt 传播规则不变，artifact 继续记录父 SHA、state 和元数据判定；传播路径不是新的 Swift/Xcode 编译证据。仓库尚无真实 Koharu 四件套、Speech 音频或真实竖排图片 corpus，不声称 OCR、翻译或识别质量提升。
