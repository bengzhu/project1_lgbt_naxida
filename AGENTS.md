...
- 候选版本号 `3.7`：图片输入语言先检查 Pro 再更新全局/content/pending 状态，拒绝时由图片菜单显示明确 Alert，不再静默污染文本页语言；v3.4 的免费目标语言规则保持不变。v3.6-v2.2 的 Retry、Koharu gate 与图片生命周期能力仍保留。仓库尚无真实 Koharu 四件套、Speech 音频或真实竖排图片 corpus，不声称 OCR、翻译或识别质量提升。
