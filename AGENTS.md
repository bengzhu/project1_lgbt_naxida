...
v3.390 新增独立 OCR 检测工作台：`OCR 检测` 作为新的 AppTab 和 Store 状态树，支持图片/相册/拍照/粘贴、自动/日语/中文/英语、自动/横排/竖排/漫画竖排、原图 bbox overlay、点击高亮、低置信筛选、单块复读、手动编辑、复制/TXT/JSON、取消/重试和折叠诊断。automatic 语言使用 Vision 自动语言集合；显式日语竖排才启用 90°/270° 方向复读，OCR-only 路径不进入图片翻译或 LLM。test2 CI capture 继续生成兼容别名 `test2-ocr-full-overlay-v3388.png`；本地按约束只做静态合同/语法/diff，待云端基础 build、UI evidence 和 receipt，未声称通用 OCR 质量提升。
