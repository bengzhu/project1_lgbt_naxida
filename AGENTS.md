...
- 当前候选版本号 `3.15`：完整图片预览中的 OCR 覆盖块可直接点选或取消并打开局部放大；当“待复查”筛选隐藏被点块时回到“全部”，保持列表、位置与前后导航一致。点选目标为命名 Button 且至少 44pt，选择仍是 View 私有状态，不改变 Store blocks、OCR、翻译或导出。v3.14-v2.2 的预览、交互、Retry、Koharu gate 与图片生命周期能力仍保留。仓库尚无真实 Koharu 四件套、Speech 音频或真实竖排图片 corpus，不声称 OCR、翻译或识别质量提升。
