...
- 候选版本号 `3.11`：图片数据已载入但预览仍在生成或生成失败时，显示 revision-scoped 反馈而不是“选择图片”；失败可只重试屏幕预览，旧 revision 预览不得回显。v3.10 的后台 ImageIO 有界下采样及 v3.9-v2.2 的交互、Retry、Koharu gate 与图片生命周期能力仍保留。仓库尚无真实 Koharu 四件套、Speech 音频或真实竖排图片 corpus，不声称 OCR、翻译或识别质量提升。
