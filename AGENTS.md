...
- 当前正式版本号 `3.18`：图片风险块存在时显示当前 revision 的 View 私有复查完成/总数/剩余进度；DEBUG 图片成功 fixture 固定覆盖低置信与方向待定风险，UI evidence 增加 wide iPad 图片成功态。v3.17-v2.2 的队列完成/撤销、预览、交互、Retry、Koharu gate 与图片生命周期能力仍保留。进度不写 Store 或持久化，不改变 OCR、翻译、renderer 或导出。仓库尚无真实 Koharu 四件套、Speech 音频或真实竖排图片 corpus，不声称 OCR、翻译或识别质量提升。
