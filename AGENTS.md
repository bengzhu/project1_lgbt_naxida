...
- 当前正式版本号 `3.17`：图片待复查队列增加当前 revision 的 View 私有复查进度；在局部放大中完成当前风险块后，从待复查队列移除并自动定位下一个未复查块，可撤销或在全部完成后重新开始。图片 revision 变化即清零，不写 Store 或持久化，不改变 OCR、翻译、renderer 或导出。v3.16-v2.2 的队列入口、预览、交互、Retry、Koharu gate 与图片生命周期能力仍保留。仓库尚无真实 Koharu 四件套、Speech 音频或真实竖排图片 corpus，不声称 OCR、翻译或识别质量提升。
