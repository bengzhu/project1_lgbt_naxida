...
- 候选版本号 `3.12`：OCR 结果行使用 View 私有选择状态联动图片预览覆盖高亮；图片 revision 变化或筛选隐藏选中行时清除选择，不改变 Store blocks、OCR、翻译或导出。v3.11-v2.2 的预览、交互、Retry、Koharu gate 与图片生命周期能力仍保留。仓库尚无真实 Koharu 四件套、Speech 音频或真实竖排图片 corpus，不声称 OCR、翻译或识别质量提升。
