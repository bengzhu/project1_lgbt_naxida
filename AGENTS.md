...
- 当前正式版本号 `3.14`：新选中 OCR 结果行后通过唯一 workspace anchor 把图片工作区带回视口，Reduce Motion 下不执行滚动动画；局部放大窗提供按当前筛选顺序的前后导航、位置与首尾禁用。选择和导航仍是 View 私有状态，不改变 Store blocks、OCR、翻译或导出。v3.13-v2.2 的预览、交互、Retry、Koharu gate 与图片生命周期能力仍保留。仓库尚无真实 Koharu 四件套、Speech 音频或真实竖排图片 corpus，不声称 OCR、翻译或识别质量提升。
