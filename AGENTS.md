...
- 正式版本号 `2.4`：普通图片稳定导出由 Store 统一持有；启动、新任务、清空与模式重渲染会安全清理被替代 PNG，删除失败保留私有 ownership 后续重试，source / staging / escape / symlink 不会误删。v2.3 的取消后 Retry、v2.2 的图片 transfer/run isolation、v2.1 的 Koharu trusted geometry coverage gate 与 v2.0 的稳定一对一匹配仍保留。仓库尚无真实 Koharu 四件套和 Speech 音频，不声称 OCR、翻译或识别质量提升。
