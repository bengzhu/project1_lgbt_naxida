...
- 当前正式版本号 `3.32`：已人工修正 block 的“恢复 Vision OCR” destructive confirmation 成功后，只在 `ImageTranslationPanel` 的 View 私有状态保存结果行焦点目标和图片 revision；恢复保持 confirmation target 直到 `confirmationDialog` 关闭的 binding 回写，只有其后 revision 仍匹配才发布焦点，取消或新图不会让旧 dialog 抢焦点。v3.31 的修正 sheet 返回焦点、v3.30 的 sheet 关闭交接、v3.29 的误识别 block 忽略／恢复、v3.28 已复查 block ID 和 v3.27 局部对照能力仍保留。v3.26 CI receipt 传播规则不变，artifact 继续记录父 SHA、state 和元数据判定；传播路径不是新的 Swift/Xcode 编译证据。本版不改变 Store、Vision OCR、模型翻译、renderer/export、漫画探针或质量基线。仓库尚无真实 Koharu 四件套、Speech 音频或真实竖排图片 corpus，不声称 OCR、翻译或识别质量提升。
