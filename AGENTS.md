...
- 候选版本号 `3.9`：清空图片翻译前显示破坏性确认，取消不调用 Store 清理，确认后才删除当前图片与结果。v3.8 的来源选择前置 Pro 门控与 v3.7-v2.2 的 Retry、Koharu gate、图片生命周期能力仍保留。仓库尚无真实 Koharu 四件套、Speech 音频或真实竖排图片 corpus，不声称 OCR、翻译或识别质量提升。
