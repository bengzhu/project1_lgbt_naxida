...
- 当前正式版本号 `3.39`：`ImageOCRCorrectionSheet` 的 `normalizedCorrectedOriginal` 仍是 View 私有的唯一首尾空白规范化值，`canSave`、`hasUnsavedChanges`、`requiresRetranslation` 与交给既有 `correctImageTranslationBlock` 的参数都复用它，严格对齐 Store 的 trim-before-no-op 判断。`trim` 后仍等于当前原文的编辑保持“确认无误”、可直接取消／交互式关闭；真正改字继续保留 v3.24 的弃改保护、v3.25 的确认无误分流、只重译当前 block 和 v3.30–v3.35 的 revision-scoped 关闭后焦点交接。v3.38 的 View 私有 `@FocusState correctedOriginalFocused`、键盘工具栏“完成”及取消、打开忽略确认、保存前清焦点继续作为明确收起入口；v3.39 在同一 sheet 的 `Form` 增加 `.scrollDismissesKeyboard(.interactively)`，让滚动也只改变键盘可见性。它不新增 Store／持久化状态，也不改变 Vision OCR、模型翻译、renderer/export、漫画探针、Koharu 主路径、ground truth、metrics 或 `output/`，不能作为 OCR、翻译、识别或 Koharu 质量提升证据。v3.36 的开发控制台 readiness 摘要仍只读已有报告；缺真实四件套时仍显示 `manifestMissing / stopUntilArtifactsProvided`。v3.26 CI receipt 传播规则不变，artifact 继续记录父 SHA、state 和元数据判定；传播路径不是新的 Swift/Xcode 编译证据。仓库尚无真实 Koharu 四件套、Speech 音频或真实竖排图片 corpus，不声称 OCR、翻译或识别质量提升。
