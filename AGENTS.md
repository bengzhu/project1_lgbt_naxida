...
- 当前正式版本号 `3.35`：从普通图片 OCR 的 16:9 局部放大窗 44pt“修正识别文字”铅笔入口进入 sheet 时，View 私有 `beginCorrectionFromFocusPreview` 与结果行共享运行／导出和 active-block guard，却把 v3.33 revision-scoped `onDismiss` fallback 设为同一局部预览；取消、放弃未保存修正和无修改时交互式关闭均在 sheet 关闭后回到该局部窗。结果行的 `beginCorrection` 继续回到结果行；成功修正／忽略仍覆盖为既有下一块、完成态或忽略行目标；新图清空 pending，旧 sheet 不能抢焦点。v3.34 的直接修正入口、v3.32 的恢复 Vision OCR confirmation 关闭后交接、v3.31 的成功修正返回、v3.30 的 sheet 关闭交接、v3.29 的误识别 block 忽略／恢复、v3.28 已复查 block ID 和 v3.27 局部对照能力仍保留。v3.26 CI receipt 传播规则不变，artifact 继续记录父 SHA、state 和元数据判定；传播路径不是新的 Swift/Xcode 编译证据。本版不改变 Store、Vision OCR、模型翻译、renderer/export、漫画探针或质量基线。仓库尚无真实 Koharu 四件套、Speech 音频或真实竖排图片 corpus，不声称 OCR、翻译或识别质量提升。
