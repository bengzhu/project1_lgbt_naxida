...
- 正式版本号 `3.4`：图片失败或取消且 Store-owned source 仍可重试时，修改目标语言会更新 task-scoped Retry 凭据而不立即启动任务；已完成图片继续即时重译，运行中任务仍冻结语言，免费目标语言不再被额外 Pro guard 阻断。v3.3 的 Koharu mask 拓扑 gate、v3.2 载荷校验、v3.1-v2.2 的图片复查与生命周期能力仍保留。仓库尚无真实 Koharu 四件套、Speech 音频或真实竖排图片 corpus，不声称 OCR、翻译或识别质量提升。
