...
- 正式版本号 `3.10`：图片预览在后台通过 ImageIO 生成最大边 2048px 的方向校正缩略图，旧 revision 或取消任务不得发布；Store 原始 Data 继续供 OCR 与导出使用。v3.9-v2.2 的交互、Retry、Koharu gate 与图片生命周期能力仍保留。仓库尚无真实 Koharu 四件套、Speech 音频或真实竖排图片 corpus，不声称 OCR、翻译或识别质量提升。
