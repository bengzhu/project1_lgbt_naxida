...
- 正式版本号 `1.98`：普通图片的 Vision OCR bbox、SwiftUI 预览和 PNG 导出统一使用顶左坐标，旁贴/覆盖模式一致；render ID 独占 staging 与发布前身份校验阻止旧 detached render 覆盖当前导出。v1.97 的 Koharu 多行 polygon 逐行失败隔离与 handoff exact-SHA 硬校验仍保留。仓库尚无真实 Koharu 四件套和 Speech 音频，不声称 OCR、翻译或识别质量提升。
